...
/**
 * Slice a payload into base64 segments small enough that each generated shell
 * command stays far below argv limits. The executor contract runs every
 * command as a single `sh -c <script>` argv element, and Linux caps one argv
 * element at MAX_ARG_STRLEN (128 KiB) — a whole-file `printf %s <b64>` script
 * therefore fails with `spawn E2BIG` once encoded content plus command
 * overhead crosses the cap (the overhead includes the absolute target path,
 * which is why the same file can pass locally and fail on a runner with a
 * longer tmpdir). Large writes must span multiple exec calls instead.
 */
const BASE64_EXEC_SEGMENT = 48 * 1024;
function base64Segments(content: Buffer): string[] {
  const b64 = content.toString('base64');
  const segments: string[] = [];
  for (let offset = 0; offset < b64.length; offset += BASE64_EXEC_SEGMENT) {
    segments.push(b64.slice(offset, offset + BASE64_EXEC_SEGMENT));
  }
...
export class SandboxFilesystem implements WorkspaceFilesystem {
  ...
  async writeFile(path: string, content: FileContent, options?: WriteOptions): Promise<void> {
    ...
    const [first, ...rest] = base64Segments(toBuffer(content));
    ...
    if (options?.overwrite === false) {
      ...
      // no exists() pre-check that could race with a concurrent writer. Only
      // the first segment competes for exclusivity; once it wins the file is
      // owned by this writer and the remaining segments append to it.
      const result = await this.exec(
        `${mkdir}{ (set -C; printf %s ${shellQuote(first!)} | base64 -d > ${shellQuote(abs)}) 2>/dev/null || { [ -e ${shellQuote(abs)} ] && exit ${EXIT_EXISTS} || exit 1; }; }`,
      );
      ...
      await this.#appendSegments(abs, rest, `writeFile ${path}`);
      return;
    }
    await this.execOk(`${mkdir}printf %s ${shellQuote(first!)} | base64 -d > ${shellQuote(abs)}`, `writeFile ${path}`);
    await this.#appendSegments(abs, rest, `writeFile ${path}`);
  }
  ...
  async appendFile(path: string, content: FileContent): Promise<void> {
    const abs = this.resolve(path);
    await this.assertContainedDest(abs, path);
    await this.execOk(`mkdir -p ${shellQuote(posixPath.dirname(abs))}`, `appendFile ${path}`);
    await this.#appendSegments(abs, base64Segments(toBuffer(content)), `appendFile ${path}`);
  }

  /** Append pre-sliced base64 segments with one bounded command per segment. */
  async #appendSegments(abs: string, segments: string[], label: string): Promise<void> {
    for (const segment of segments) {
      await this.execOk(`printf %s ${shellQuote(segment)} | base64 -d >> ${shellQuote(abs)}`, label);
    }
  }
