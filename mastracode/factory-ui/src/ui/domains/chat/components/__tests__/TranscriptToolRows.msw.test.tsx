...
describe('TranscriptEntries tool rows', () => {
  it.each([
    ['Approve', true],
    ['Decline', false],
  ])('forwards the parked run identity when the user selects %s', async (action, approved) => {
    ...
    renderWithProviders(
      <MemoryRouter>
        <TranscriptEntries entries={state.entries} onApprove={onApprove} onRespond={() => {}} />
      </MemoryRouter>,
    );

    await userEvent.click(screen.getByRole('button', { name: `${action} execute_command` }));

    expect(onApprove).toHaveBeenCalledOnce();
    expect(onApprove).toHaveBeenCalledWith('run-1', 'call-1', approved, 'approval-call-1');
  });
