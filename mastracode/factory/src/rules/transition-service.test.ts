...
describe('FactoryTransitionService', () => {
  ...
  it('validates and runs phase behavior from an installed custom board', async () => {
    ...
    const board = defineBoard({
      ...
      initialPhase: 'queued',
      phases: {
        queued: { title: 'Queued', next: 'shipped', onEnter: { issue: onEnter } },
    ...
    await expect(
      service.transition({
        ...request(item, { board: 'release', stage: 'queued', identity: 'enter-release' }),
        initialEntry: true,
      }),
    ).resolves.toMatchObject({ status: 'accepted', stage: 'queued' });
    expect(onEnter).toHaveBeenCalledOnce();
    const decisions = await storage.listDeferredDecisions('org-1', PROJECT_ID);
    expect(decisions).toHaveLength(1);
    const decision = validateFactoryRuleDecision(decisions[0]?.decision);
    ...
    assert(decision.type === 'transition');
    const queuedItem = await storage.get({ orgId: 'org-1', id: item.id });
    assert(queuedItem);
    await expect(
      service.transition(
        request(queuedItem, { board: decision.board, stage: decision.stage, identity: 'ship-release' }),
      ),
    ).resolves.toMatchObject({ status: 'accepted', stage: 'shipped' });
    const shippedItem = await storage.get({ orgId: 'org-1', id: item.id });
    assert(shippedItem);
    expect(shippedItem.stages).toEqual(['shipped']);
    await expect(
      service.transition(request(shippedItem, { board: 'work', stage: 'done', identity: 'work-disabled' })),
    ).resolves.toMatchObject({
      status: 'rejected',
      code: 'invalid_transition',
