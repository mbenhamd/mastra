...
pnpm --filter ./mastracode/tui run cli
```
...
# Build the TUI library bundle
pnpm --filter ./mastracode/tui run build:lib
```
...
pnpm --filter ./mastracode/tui run check
```
...
```bash
# Lint mastracode
pnpm --filter ./mastracode/tui run lint
```
...
```bash
# Run unit tests
pnpm --filter ./mastracode/tui test
...
pnpm --dir mastracode/web run web:dev

# With GitHub App integration (starts Postgres first)
pnpm --dir mastracode/web run web:dev:github
```
