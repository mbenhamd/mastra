...
describe('WorkflowsElasticSearch resource-scoped identity', () => {
  it('loads, updates, and deletes a resource-scoped run by its canonical identity', async () => {
    const workflows = new WorkflowsElasticSearch({ client: sharedClient });
    const workflowName = `resource-workflow-${randomUUID()}`;
    const runId = `resource-run-${randomUUID()}`;
    const resourceId = `resource-${randomUUID()}`;
    const snapshot = { runId, status: 'suspended', context: {} } as WorkflowRunState;
    ...
    await expect(workflows.getWorkflowRunById({ workflowName, runId })).resolves.toMatchObject({ resourceId });
