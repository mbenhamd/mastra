...
describe('read/write pools', () => {
  it('uses current writer state for task admission and recovery while the replica is empty', async () => {
    ...
    const schemaName = `replica_tasks_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const store = new PostgresStore({
      id: 'pg-task-decision-replica-test',
      writePool,
      readPool,
      schemaName,
      enabledDomains: ['backgroundTasks'],
    });
    ...
    try {
      ...
      expect({
        globalRunning: await tasks.getRunningCount(),
        agentRunning: await tasks.getRunningCountByAgent('test-agent'),
        recovery: await tasks.listTasks({ status: 'running' }),
      }).toMatchObject({
        globalRunning: 1,
        agentRunning: 1,
        recovery: { total: 1, tasks: [{ id: 'running-task', status: 'running' }] },
      });
      await tasks.updateTask('running-task', { status: 'pending' });
      await expect(tasks.listTasks({ status: 'pending' })).resolves.toMatchObject({
        total: 1,
        tasks: [{ id: 'running-task', status: 'pending' }],
      });
    } finally {
      await store.close();
      await writePool
        .query(`DROP SCHEMA IF EXISTS "${schemaName}" CASCADE`)
        .finally(() => Promise.all([writePool.end(), readPool.end()]));
    }
  });

  it('uses current writer state for notification delivery while the replica still says pending', async () => {
    const writePool = createTestPool();
    const readPool = createTestPool();
    const readQuery = vi.spyOn(readPool, 'query');
    const schemaName = `replica_notifications_${Date.now()}_${Math.random().toString(36).slice(2)}`;
    const store = new PostgresStore({
      id: 'pg-notification-decision-replica-test',
      writePool,
      readPool,
      schemaName,
      enabledDomains: ['notifications'],
    });
    const sendSignal = vi.fn().mockImplementation(signal => ({
      signal,
      accepted: Promise.resolve(),
      persisted: Promise.resolve(),
    }));
    const mastra = { getAgentById: async () => ({ sendSignal }) } as unknown as Mastra;
    const now = new Date();
    ...
    try {
      ...
      const notification = await notifications.createNotification({
        ...
        source: 'test',
        kind: 'reminder',
        summary: 'A due reminder',
        deliverAt: now,
      });
      const stalePending = await writePool.query(`SELECT * FROM "${schemaName}".mastra_notifications`);
      readQuery.mockResolvedValue(stalePending as never);

      for (const status of ['delivered', 'dismissed'] as const) {
        await notifications.updateNotification({
          id: notification.id,
          threadId: notification.threadId,
          status,
          deliveredSignalId: 'original-delivery',
        });
        expect.soft(await notifications.getNotification(notification)).toMatchObject({
          status,
          deliveredSignalId: 'original-delivery',
        });
        expect.soft(await notifications.listDueNotifications({ now })).toEqual([]);
        expect.soft(await dispatchDueNotifications({ mastra, storage: notifications, now })).toEqual({
          delivered: [],
          failed: [],
          signals: [],
        });
      ...
      // Inbox reads retain the optional replica's eventual-consistency behavior.
      await expect(notifications.listNotifications({ threadId: notification.threadId })).resolves.toMatchObject([
        { id: notification.id, status: 'pending' },
      ]);
    } finally {
      await store.close();
      await writePool
  ...
  it('keeps read-modify-write paths on the writer even when the replica lags', async () => {
    ...
    try {
      ...
      const observation = await memory.initializeObservationalMemory({
        scope: 'thread',
        threadId,
        resourceId: 'lag-resource',
        config: {},
      });
      const staleObservation = await writePool.query('SELECT * FROM mastra_observational_memory WHERE id = $1', [
        observation.id,
      ]);
      ...
      // The next observation must build on the completed writer state, even
      // while the replica still returns the record before that observation.
      readQuery.mockResolvedValue(staleObservation as never);
      const currentObservation = await memory.getObservationalMemory(threadId, 'lag-resource');
      readQuery.mockResolvedValue({ rows: [], rowCount: 0 } as never);
      await memory.updateActiveObservations({
        id: observation.id,
        observations: `${currentObservation!.activeObservations}\nsecond observation`,
        tokenCount: 4,
        lastObservedAt: now,
      });
      const persistedObservation = await writePool.query(
        'SELECT "activeObservations" FROM mastra_observational_memory WHERE id = $1',
        [observation.id],
      );
      expect(persistedObservation.rows[0].activeObservations).toBe('first observation\nsecond observation');
