...
function parseWorkflowJson<T>(json: string, field: string, runId: string): ParseResult<T> {
  try {
    return { ok: true, value: JSON.parse(json) };
  } catch (error) {
    return {
      ok: false,
      error: `Failed to parse workflow ${field} JSON for runId ${runId}: ${formatJsonParseError(error)}`,
    };
  }
}

function parseStoredSnapshot(stored: unknown, runId: string): ParseResult<Record<string, any>> {
  try {
    const value =
      typeof stored === 'string'
        ? JSON.parse(stored)
        : JSON.parse(JSON.stringify(stored ?? createEmptyWorkflowSnapshot(runId)));
    return { ok: true, value };
  } catch (error) {
    return {
      ok: false,
      error: `Failed to parse workflow snapshot JSON for runId ${runId}: ${formatJsonParseError(error)}`,
    };
  }
}
...
export async function handleTypedOperation(
  ...
  switch (request.op) {
    ...
    case 'mergeWorkflowStepResult': {
      ...
      const parsedSnapshot = parseStoredSnapshot(existing.snapshot, request.runId);
