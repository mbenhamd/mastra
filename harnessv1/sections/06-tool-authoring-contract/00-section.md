...
**Entity ownership:** Session (via `HarnessRequestContext`); Harness composition
only — [§0 Mental model](../00-mental-model.md).
...
expose the generic `MastraUnion` authority as `context.mastra`. There is no
compatibility facade for raw storage, primitive storage, agent or workflow
registries, provider/channel clients, mutable framework registries, or other
session-bypassing framework capabilities.
