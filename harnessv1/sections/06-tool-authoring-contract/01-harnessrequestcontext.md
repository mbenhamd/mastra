...
`../packages/_internal-core/src/request-context/index.ts:122` (re-exported via
`../packages/core/src/request-context/index.ts:6`); the `'harness'` slot sits
...
`context.mastra` is absent for Harness-managed invocations (§6.3). The remaining fields follow ordinary Mastra semantics
...
Workspace browser contract. `writer?: ToolStream` is not a persistence surface,
and `agent` / `workflow` helpers are not Harness v1 authority: harness v1 routes
tool-emitted events through `emitCustomEvent` (below) and routes tool
suspension through `suspendTool` per §11.1.

`ToolExecutionContext` is the carrier for the v1 contract, not a substitute for
`HarnessRequestContext`. A second internal interface named
...
For MastraCode, new hooks and observability use `sessionId` as the runtime
session identity. `threadId` remains a separate history/transcript attribute.
Harness request context exposes both values, but caller-provided request context
cannot override either one; the harness-created slot is stamped from the owning
`SessionRecord` and thread row. Model/provider headers, trace attributes, hook
manager state, analytics events, and eval lookups that correlate runtime work
must carry `sessionId` and may include `threadId` only for history joins. Current
MastraCode hook stdin used `session_id` for the legacy thread id; migration must
preserve or version that field and add explicit `harness_session_id` and
`thread_id` before changing external hook semantics.
...
// current-code inputs only. A v1 adapter may expose or consume those current
...
// MastraCode's current TUI can queue multiple inline prompts in process-local
// state. V1 migration must either serialize those prompts before they reach this
// durable slot, or intentionally treat the second prompt as a breaking
// `HarnessBusyError`; a process-local prompt queue is not itself v1 recovery
// evidence.
//
...
// When migration code has the current tool's `suspendSchema`, it must
