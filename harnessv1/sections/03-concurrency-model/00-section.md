...
**Entity ownership:** Session (admission coordinator), Storage (leases, queue
rows) — [§0 Mental model](../00-mental-model.md).

**Admission identity:** busy semantics and operation primitives below are
canonical here. `admissionId` / `admissionHash` inputs, serializability rules,
and conflict errors are canonical in [§4.4b](../04-public-api/04-operation-option-types/02-queue-and-skill-options.md);
durable receipts and tombstones are canonical in
[§5.1d](../05-session-persistence/01-what-gets-persisted/10-queue-admission-and-tombstones.md).

The session has one canonical signal primitive, one durable queue primitive, one
explicit sync-generate signal form, plus skill invocation. The model is built on
**one current session owner per `(harnessName, resourceId, threadId)`** (§2.2).
When that owner is Active, it is the admission coordinator for the thread inside
one Harness namespace: it serializes durable queue appends, pending-item
transitions, `currentRun` snapshots, and run-boundary decisions under the session
lease (§5.8), then uses the agent signal/run APIs to execute work. Closing and
Closed owners still reserve the key for terminalization, reopen, or delete, but
they do not admit new work. Harness v1 does not add a separate `ThreadRuntime` or
thread lease because the unique active session record is the thread runtime
owner.
...
Idle thread: Enqueues for the next standalone turn at an idle boundary
...
<svg role="img" aria-labelledby="hx-concurrency-title hx-concurrency-desc" viewBox="0 0 900 360" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
  ...
  <desc id="hx-concurrency-desc">Operation admission selects signal, queue, sync output, or skill. Signal dispatch reaches live or idle runs, queue appends durable FIFO work, sync output fails fast when busy, and every successful path settles by operation identity.</desc>
  <defs>
  ...
  <g transform="translate(455,35)">
    ...
    <text x="72.5" y="27" text-anchor="middle" font-size="11" font-weight="700" fill="#064e3b" font-family="Inter, system-ui, sans-serif">signal</text>
...
**`signal` is busy-independent once admitted.** Multiple
concurrent `signal()` calls that omit `sync: true` / `output` (10 users typing
...
multi-process deployment, leases and claims must not become a blanket lock on
reading, subscribing, admitting authorized `signal` / `queue` / inbox operations,
or dispatching admitted signals. For direct HTTP/SDK `signal(...)` and
`queue(...)`, the route is owner-affinity: the receiving process must forward the
request to the current live owner before accepting it. If deployment routing
cannot reach the current live owner while the lease is still current, the direct
route has not admitted anything yet; it may return
`HarnessSessionLockedError` / `harness.session_locked` as an owner-affinity
routing failure, not as evidence that the lease is a blanket lock on all signal
or queue admission. Source-specific inputs such as channel callbacks or wakeups
may instead record their durable inbox/wakeup row for a worker/owner to claim. A
process must not return an accepted signal/queue response until one of those
boundaries has succeeded, and it must not reject an already-admitted signal
merely because another process owns recovery-sensitive execution. From the model's
perspective accepted signals show up as a sequence of user inputs interleaved
into whatever reasoning context is live. Each accepted call has a stable
`signalId`; settlement is correlated by that signal rather than run-level
lifecycle events, because one run can answer multiple signals. §4.2 owns the
exact `AgentResult` / `AgentStream` promise behavior and admission failure paths.
Signal-driven admission can still fail before acceptance for reasons unrelated
to busy-ness, including an unreachable live owner under the owner-affinity
contract, but never with `HarnessBusyError`.
...
`signal`, `queue`, or untyped `useSkill` per [§4.4b](../04-public-api/04-operation-option-types/02-queue-and-skill-options.md).
Exact duplicate untyped `useSkill` admission is checked before the idle/busy
check because it reuses the `signal(...)` boundary after skill resolution.
Retained signal/receipt evidence, tombstone lifecycle, and post-compaction lookup
are owned by [§5.1d](../05-session-persistence/01-what-gets-persisted/10-queue-admission-and-tombstones.md)
and verified in §15.1/§15.2 without redefining hash inputs here.
`signal({ sync: true, output })` and `useSkill({ output })` bypass this
...
full turn each — they do not merge with concurrent `signal` inputs. §5.1 and
...
interactive `signal()` or `queue()` busy-rejected at admission.
...
calls `session.signal()` for the new content. There is no `session.steer()`, no
...
interactive, non-sync `session.signal(...)` form by default, or
