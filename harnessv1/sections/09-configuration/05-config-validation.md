...
`Agent`. A mode that embeds a
live `Agent` object, a state-dependent agent factory, or any other old
`agent`-shaped field is invalid input and throws `HarnessConfigError` during
`harness.init()`. Migration tooling may rewrite old config before constructing
the Harness, but init never derives registry entries, calls a mode-local factory,
inspects generic session state to choose an agent object, or treats a live
`Agent` reference on a mode as a committed run surface.
...
rather than silently retargeting. If `signal(...)` would drain into an already
...
Channel bridge delivery plans are validated at the same boundary: any
`deliverySemantics` returned by `resolveProviderOperation(...)` must use the
known mode vocabulary, and any bridge that can snapshot `lookup-reconcile` for a
row must provide `reconcileDelivery(...)` for the stored operation. A delivery
plan with `operationKind: 'custom'` must include a stable `operationName`.
