...
below remain authoritative for fields, defaults, envelope types, and bridge
contracts):
...
<figure>
  <svg role="img" aria-labelledby="hx-channel-config-title hx-channel-config-desc" viewBox="0 0 1040 560" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <desc id="hx-channel-config-desc">Each registered channel has identity, provider-owned bridge capabilities, an ingress policy, and per-worker recovery configs. Delivery semantics are snapshotted on outbox rows from trusted enqueue/provider-operation data and default to at-least-once. Envelopes flow from raw transport requests to verified ingress and action shapes.</desc>
    <defs>
    ...
    <text style="font: 600 16px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="390" y="176" text-anchor="middle">bridge capabilities</text>
    ...
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="390" y="218" text-anchor="middle">send · reconcileDelivery</text>
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="390" y="236" text-anchor="middle">resolveProviderOperation</text>
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="390" y="254" text-anchor="middle">calls existing provider owner</text>
    ...
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="630" y="218" text-anchor="middle">dms · mentions · sharedPlatformThreads</text>
    ...
    <text style="font: 600 14px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="40" y="304">Delivery semantics snapshot (per outbox row, decided at enqueue)</text>

    <rect style="fill: #f8fafc; stroke: #94a3b8; stroke-width: 2; rx: 14;" x="40" y="316" width="220" height="60" />
    <text style="font: 600 14px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="150" y="342" text-anchor="middle">resolveProviderOperation</text>
    ...
    <text style="font: 600 14px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="400" y="342" text-anchor="middle">trusted enqueue fields</text>
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="400" y="361" text-anchor="middle">operationKind + optional name</text>

    <rect style="fill: #f8fafc; stroke: #94a3b8; stroke-width: 2; rx: 14;" x="540" y="316" width="220" height="60" />
    <text style="font: 600 14px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="650" y="342" text-anchor="middle">provider capability hint</text>
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="650" y="361" text-anchor="middle">only if returned by bridge</text>
    ...
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="895" y="361" text-anchor="middle">spec default</text>
    ...
    <text style="font: 600 14px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="40" y="408">Envelope flow (bridge boundary; raw payloads never reach storage)</text>
...
interface ChannelIngressPolicy {
  ...
  // Platform-native shared conversation threads only; this never means shared
  // Harness `Thread` identity across resources.
  sharedPlatformThreads?: 'shared-resource' | 'reject';
  ...
  resolveResource: (ctx: ChannelIngressContext) => Promise<{
    ...
    // These fields are ordinary `session.signal(...)` / `session.queue(...)`
...
interface ChannelOutboxDeliveryPlan {
  ...
  // Bridge-normalized provider operation name. Required for `custom`, and
...
interface HarnessChannelBridge {
  // This is a durable bridge over provider-owned capabilities, not a second
  // channel-provider framework.
  // Provider lifecycle, OAuth/provisioning, route installation, credentials, and
  // platform SDK ownership remain with the existing `ChannelProvider` /
  // `AgentChannels` integration layer (§14.7). Harness calls only narrow
  // verification/normalization, provider-operation resolution, reconcile, and
  // send capabilities after a route or outbox row has already identified the
  // owning `(harnessName, channelId, providerId)`. Implementations must not hide
  // provider installation state, route ownership, provider registration, or live
  // `AgentChannels` handlers behind this interface.
  // Required for externally mounted provider routes. Local/test bridges that
  ...
  // provider exposes the acting user, the bridge returns that verified actor
  // in `ChannelActionEnvelope.actor`; deployment-owned action-token audience
  // policies in §14.5 fail closed if the bridge cannot provide the actor
  ...
  // operation fields and snapshots `deliverySemantics: 'at-least-once'`.
  // Provider capability hints may refine one row, but no bridge-wide defaults
  // or per-operation maps live here; once enqueued, the outbox row is the source
  // of truth.
  ...
  // provider-visible operation. `delivered: false` means the bridge/provider
...
// bridge must ignore them unless they match the route context.
