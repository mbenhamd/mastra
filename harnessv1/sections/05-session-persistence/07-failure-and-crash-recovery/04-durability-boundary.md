...
post-acceptance (every `signal(...)` and every drained `queue(...)`). The
...
`signal(...)`, the accepted `signalId` must eventually map to
`signal_completed` or `signal_failed` and to the signal-result lookup route.
...
`pending`: `signal_failed` / `queue_failed` while evidence is retained, or
...
corresponding `session.signal(...)` signal is accepted, the normal agent-layer
