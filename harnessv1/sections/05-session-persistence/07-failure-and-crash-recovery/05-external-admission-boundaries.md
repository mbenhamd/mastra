...
**Direct HTTP/SDK `signal(...)` and untyped `useSkill(...)`**
...
pre-acceptance record, by design. Remote routes therefore must reach the current
live owner before returning an accepted response; if owner-affinity routing is
unavailable or the owner cannot be reached while its lease remains current, the
operation fails before admission with `HarnessSessionLockedError` /
`harness.session_locked`. Untyped `useSkill(...)` resolves the skill, validates
args, expands the prompt, and then uses the same signal boundary. Exact duplicate
skill retries are checked before the fail-fast busy check while retained
signal/tombstone evidence exists. Callers that need restart survival use
`queue(...)`.

**Direct HTTP/SDK `signal({ sync: true, output })` and `useSkill({ output })`**
...
retries. Remote queue routes use the same owner-affinity rule as direct signal:
the owner appends under its lease, or the route fails before admission rather
than accepting work that no owner can serialize.
...
`ChannelBinding`, then `session.signal(...)` / `session.queue(...)` with
`admissionId`.
...
`runtimeCompatibilityGeneration` to the current runtime domain when the
