...
- _Forward-compatible schema drift._ Unknown fields on a stored `SessionRecord`
...
- _Backward-incompatible schema._ If a required field is missing or malformed,
  ...
  whether to repair or use the explicit operator force-delete path.
- _Corrupted JSON._ Throws `HarnessSessionCorruptError` with
  `reason: 'parse_failed'`.
- _Corrupted pending state (owner repair only)._ If hydration under the owning
...
'accepted')` and channel-originated action receipts to terminal failure with
  ...
  hydration.
- _Pending interrupt with a missing workflow snapshot._ The session hydrates
