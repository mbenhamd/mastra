...
**`signal(...)` in flight, signal not yet accepted by the agent**

After hydration: **Lost.** The signal was never persisted (Slack semantics —
interactive `signal` items aren't on `pendingQueue`). The caller's pending
promise rejects. The user resends if they want the signal delivered.

**`signal(...)` accepted, run started, no suspension**

After hydration: Accepted-signal result evidence is the durable settlement
boundary. The agent thread log may contain the user-side signal message,
assistant turn, and run correlation fields, but it is history/reconciliation
input, not the operation-result ledger. The session may have `currentRun` for
...
against the accepted-signal result evidence plus the agent/thread store. If the
run completed before crash, the signal-result route answers by `signalId` from
that result boundary and `currentRun` becomes terminal or is cleared. If it
didn't, the model output is lost — but the user-side input may survive in the
thread log so they can ask again; unresolved result lookup eventually reports
`failed` rather than `pending` forever (the `currentRun.status` may be
`interrupted`, but the operation result wire state is `failed`), and
`getCurrentRunId()` returns `null` unless the agent layer reports a still-live
run or a persisted pending item keeps the run waiting.
...
**Channel inbound admitted via `signal(...)`, signal not yet accepted**
...
**Product autonomous question response accepted but run resume not yet applied**

After hydration: Same inbox-response recovery boundary. The deterministic
`responseId` and `InboxResponseReceipt.responder` metadata prove which product
responder attempt won the pending question. Recovery retries the resume with the
same `responseId` / `resumeAttemptId`; it does not re-run the responder for that
consumed pending item, and later product state changes do not undo the already
...
`signal(...)`,
`signal({ stream: true })`, untyped `useSkill(...)`, and drained `queue(...)`
operations also record `signal_failed` / `queue_failed` on their existing
...
outbox projection fail as deleted or tenant-hidden. Retrying the explicit
operator force-delete path or the adapter's delete reconciler
resumes the §5.5 cascade, terminalizes any remaining source-specific rows with
...
abandoned-operator-cleanup path, and removes the `SessionRecord` last.
...
a replacement closure or treating the raw task row as completed.
