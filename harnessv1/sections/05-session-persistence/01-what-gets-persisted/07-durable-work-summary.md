...
`DurableWorkKind` and `DurableWorkProofKind` follow the same rule: they group
heterogeneous owner rows for session snapshots and lists, but they do not create
a shared storage table or shared claim loop.
...
**Accepted signal / signal-result evidence**
...
Notes: Result lookup and `signal_completed` remain the settlement authorities.
...
Notes: The ingress row has handed off to signal/queue; later operation status
