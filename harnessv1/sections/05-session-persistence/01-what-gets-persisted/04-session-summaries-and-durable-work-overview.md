...
This group is the bounded read-model layer over the session owner records in
§5.1a. It owns the storage-adapter projection of a session row, durable-work
summary projections for admitted-but-not-yet-complete operations across kinds,
and the list-row shapes the public navigation/reconnect APIs return so callers
...
- `06-background-task-records.md` declares reconstructable background-task row
  shapes plus the `DurableWorkKind` / `DurableWorkStatus` read-model projection
  vocabulary used by background task observation and recovery views. It is not a
  generic storage enum or independent recovery state machine.
