...
These records are the Harness channel bridge's bindings and work rows. They do
not replace `ChannelsStorage` provider installation/config records: credentials,
OAuth/provisioning state, provider app configuration, and current
`ChannelInstallation` rows remain owned by the channel storage domain. Harness
rows reference that owner through stable `(channelId, providerId, installation /
route selector)` identity and add only the per-session binding, inbox/action,
outbox, idempotency, claim, and delivery evidence needed for session-first
recovery.
...
```ts
// Channel bridge records are Harness-owned work ledgers. They may live in a
// dedicated Harness channel-bridge domain or in an extension of the existing
// channels storage domain; they must not replace the ChannelsStorage
// provider-installation/config owner. They are separate records instead of
// fields on SessionRecord because channel dispatch workers must be able to claim
// and update delivery state without becoming second writers to the session lease
// (§5.8). See §14.
...
// Provider callback bindings are durable route/installation selector→harness
// mappings. They let the registry resolve provider-owned callbacks to the
// correct harness/channel pair before a per-conversation ChannelBinding exists.
// They reference provider identities owned by the existing channels domain; they
// do not store provider credentials, OAuth/provisioning state, channel app
// config, or duplicate ChannelInstallation rows. They are configuration-level
// records, not claimable work rows.
