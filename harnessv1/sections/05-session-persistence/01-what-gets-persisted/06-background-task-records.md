### 5.1b.2 Background Task Records and Durable Work Projection Vocabulary

```ts
// DurableWork* names are read-model vocabulary only. They are not a persisted
// work table, a generic worker registry, or a recovery state machine. Every
// value below is derived from a source-specific authority: signal/result
// evidence, queue receipts, channel inbox/action/outbox rows, wakeup rows,
// inbox response receipts, goal receipts, current-run projections, or the
// intentionally reconstructable BackgroundTasksStorage rows defined later in
// this section.
...
// Do not reuse `DurableWorkStatus` as a storage-row status. Source-specific
// rows keep their own status literals and project into DurableWorkSummary at
// read time.
...
interface BackgroundTaskReconstructableRow extends BackgroundTaskRowBase {
  ...
  // be claimed as restart-safe worker state inside BackgroundTasksStorage. They
  // are not a generic Harness worker framework. They must not also sit behind an
  ...
  // (§9) or an equivalent existing Mastra static executor registry entry bridged
  // into that config. v1 reconstructable background tasks use `kind: 'tool'`;
  // other executor kinds remain out of scope until a future registry entry
  // defines their reconstruction contract.
  executorRef: {
    id: string;
    kind: 'tool';
    generation?: string;
...
(`../packages/core/src/background-tasks/types.ts:20-62`) and the
`BackgroundTasksStorage` adapter
(`../packages/core/src/storage/domains/background-tasks/base.ts:8-55`) with
...
1. **Storage-level claim/renew/CAS metadata** (`claimId`, `claimOwnerId`,
   ...
   (`../packages/core/src/background-tasks/types.ts:357-370`,
   `../packages/core/src/background-tasks/manager.ts:34`) over a pubsub
   consumer group (`../packages/core/src/background-tasks/manager.ts:19-21,145`).
...
2. **Reconstructable executor and completion-policy references** (`executorRef`,
   ...
   without referencing a live closure. Current Mastra static executor/tool
   registries are valid inputs to this reference model when they can be resolved
   by stable ID and generation after restart; v1 should compose with them instead
   of classifying every missing `TaskContext` closure as diagnostic-only. The
   `runtimeCompatibilityGeneration` token's scope and fail-closed semantics are
   owned by §9.1.
...
The `'dead'` terminal status in `BackgroundTaskRowStatus` is a v1 addition not
present in the current `BackgroundTaskStatus` literal set; `'suspended'`,
`suspendedAt`, and `suspendPayload` are existing Mastra background-task lifecycle
state that v1 must preserve rather than flattening to `waiting` or `failed`.
§11.6a records the literal-set gap on the `BackgroundTaskStatus` ledger entry.
Durable-work summaries may project these statuses, but the background-task row
itself remains owned by `BackgroundTasksStorage`.
