...
Product-level autonomous responders, if a deployment builds them on top of the
normal inbox API, are ordinary `InboxResponseReceipt` rows with deterministic
`responseId` and optional `responder` audit metadata. The metadata is
correlation data only: it does not create a new responder class, does not bypass
first-response-wins, and does not let recovery resume by any boundary other than
