...
```ts
// Reconstructable BackgroundTasksStorage rows follow the same serialization
// rule. A background task row can be the durable boundary only if it carries
// stable references and serialized metadata sufficient to rebuild both the
// executor and completion policy after restart. It must not rely on `TaskContext`
...
// the owning `QueuedItem`, channel inbox/action/outbox row, inbox response
// receipt, or wakeup row; the background task is only an internal worker for
// that durable source.
// These rows live in the background-task storage domain. Harness v1 adds the
// fields needed for reconstructable execution and claims; it does not define a
// second background-task table under the session storage layer.
...
// admission. They do not replace workflow schedule definitions or ticks; they
// record the handoff after a scheduler/product source decides a fire is due.
// They are intentionally not SessionRecord fields because workers must scan and
// claim due work without first becoming a session owner.
interface HarnessWakeupItem {
  id: string;
  harnessName: string;
  // `schedule-handoff` means an existing schedule/workflow/product scheduler has
  // already decided that a fire is due and is handing that occurrence to Harness.
  // It is not a duplicate schedule definition or tick owner.
  source: 'schedule-handoff' | 'proactive';
  sourceId: string;                    // schedule/fire source id, product job id, or other stable source key
...
interface HarnessRunOperationalState {
  ...
  // callbacks, or same-named substitute tools are not enough to rehydrate a
  ...
  // on an old/imported run snapshot means no generation guard was set at
  // run start and hydration falls back to ID-only validation.
  runtimeCompatibilityGeneration?: string;
  // Set to `true` during the synchronous run-start transition (§5.7) when
  // the entry-point overrides included `addTools` — i.e. `signal({ addTools })`
  // (signal-driven, `stream`, or `sync: true, output` forms) or
  // `useSkill({ addTools })` — or when a current-code input path admits any other
  // per-run executable tool surface that cannot be reconstructed from stable
  // persisted tool identities. MastraCode `extraTools` supplied as a function of
  // `RequestContext` is one such current-code input unless the product registers a
  // stable tool identity and recovery-safe factory for the exact run surface.
  // The added tool implementations are process-local
