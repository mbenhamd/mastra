...
interface HarnessDisplaySubagentSnapshotV1 {
  ...
  task: string;
  modelId?: string;
  // Migration/display hint from current forked-thread subagent metadata only.
  // V1 child-session semantics do not expose fork as a lifecycle mode.
  legacyForkedThread?: boolean;
