...
<figure>
  <svg role="img" aria-labelledby="hx-thread-session-title hx-thread-session-desc" viewBox="0 0 1040 480" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <desc id="hx-thread-session-desc">A HarnessThread row joins the current SessionRecord owner through the (harnessName, resourceId, threadId) owner key. The session row composes in-flight pending work and per-session state slots under one lease.</desc>
    <defs>
    ...
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #334155;" x="320" y="62">owner key</text>
...
interface HarnessThread {
  id: string;
  // Logical namespace of the registered Harness. Existing MemoryStorage rows may
  // carry this as a physical column only after migration; adapters may instead
  // enforce it through a table/schema/key prefix or bound storage view. Public
...
type ThreadMetadata = Record<string, unknown> & {
  ...
  // channel adapters, explicit migration/import metadata, and future framework use.
  app?: Record<string, JsonValue>;
  clone?: ThreadCloneMetadata;
};

// Session-first clone writes Harness-owned clone provenance into top-level
// metadata so history readers can identify the source without letting
...
// `harnessName` is the registered Mastra Harness key (`default` in
// single-harness examples). For new v1 Harness-owned ledgers it is assigned when
// the durable row is created and is immutable for that row's lifetime. Existing
// MemoryStorage thread rows are migrated through an adapter-owned namespace
// strategy: physical column, table/schema/key prefix, or bound storage view.
// Session-local nested records inherit the owning `SessionRecord.harnessName`;
// independently loadable Harness-domain rows such as session summaries,
// tombstones, channel callback/binding/inbox/action/outbox rows, and wakeups
// persist it directly so shared storage adapters can enforce the same namespace
// as the server route/registry.
```

Migration may read legacy thread metadata to seed a first `SessionRecord`, but
that read is a one-time import boundary. After the session row exists, v1 must
hydrate from `SessionRecord` and the canonical domain owners below; it must not
dual-read top-level thread metadata as a fallback source for mode/model,
permissions, OM settings, goals, workspace, queue, pending inbox, display, or
subagent state. App-owned thread metadata remains available only under
`metadata.app`.

MastraCode-specific runtime preferences such as active model-pack id,
escape-as-cancel, sandbox allowed paths, project path binding, and UI-relevant
agent settings are serializable application state inside `SessionRecord.state`
...
interface SessionRecord {
  ...
  // is seeded from explicit import output or the immutable HarnessConfig default
  // only when no selected model exists; once written, it is authoritative over
  // config until `session.switchModel(...)` commits. Runtime hydration never
  // falls back to thread metadata for model selection.
  modeId: string;
  modelId: string;
...
// getDisplayState shape, the in-process subscribeDisplayState payload shape, and
// the only shape that may be stored in SessionRecord.displayState or returned
// over HTTP. Implementations may keep richer in-process maps, Date objects, or
// live buffers internally, but they must normalize to this shape before storage,
// HTTP, or portable API delivery. Unknown payload slots are included only when
// they are canonical
