...
- `Active -> Closing` via `session.close()` or an explicit
  server/local-runtime/operator close helper, committed by the stored
  `closingAt` marker and the §5.8 closing write fence.
...
- `Closed -> Active` via explicit session reopen/open through
  `harness.session(...)` or an equivalent session-first route after the caller
  passes the same resource and ownership checks.
...
No implicit backward durable transitions are allowed. A Closed record becomes
Active only through explicit session reopen; thread/resource lookup must not
create a fresh active record just because the owning session was closed. A
Deleted/hidden session is not rehydrated or listed.
...
Active, closing, and closed reopenable records participate in the
`(harnessName, resourceId, threadId)` current-owner uniqueness rule enforced by
`createOrLoadCurrentSessionOwner(...)` (§5.2). A thread has at most one current
session owner; retained audit/operator records, if an adapter supports them,
must not be returned by owner lookup or treated as reopen targets.
...
- `session.close()` (or an explicit server/local-runtime/operator close helper
  ...
  **Terminal close.** When live work settles or `closeDeadlineAt` is reached,
  ...
  signal boundary, records `signal_failed` / `queue_failed` for unresolved
  ...
  session. The committed Closed state is not a deletion and not a dead-end: a
  later authorized reopen may hydrate the same session after proving the thread,
  resource, and storage records still match.
  ...
  process crashes after `closingAt` but before `closedAt`, re-issuing the same
  close helper or hydrating/resolving the closing record for close/delete repair
  resumes the stored cascade; if `closeDeadlineAt` has passed, the new owner
  proceeds directly to terminal close without waiting again. Close remains
  commit-on-return and resolves only after the terminal `closedAt` write for the
...
- `session.delete()` / the explicit server/local-runtime/operator delete helper
  ...
  **Force delete** is for operator deletion, thread-delete cascade, and
  corruption recovery. It is not on the public Harness class. The operator path
  first acquires or fences the owning session/root lease using the
  normal §5.8 lock policy; under default `lockMode: 'fail'`, an unexpired
  different owner blocks the call, while operator-only `'steal'` fences that
  owner. Once fenced, the delete installs a
  ...
  re-running the operator force-delete path
  completes any unfinished
  ...
  Deleting the root/product session does not silently preserve the backing
  transcript as a new app-facing conversation. Unless an explicit, well-named
  preserve/import-retention option is used by operator tooling, the
  session-first delete cascade removes the backing thread, messages,
  session-owned artifacts, per-session workspace materialization, and
  thread-scoped OM for the deleted `(harnessName, resourceId, threadId)`.
...
`session.close()` reaches `closedAt`, the thread remains the backing transcript
for the same session; `harness.session({ threadId, resourceId })` resolves the
closed owning record and reopens it instead of ignoring it and creating a fresh
active record. A Closing record blocks reopen until terminal close commits.
Closed records remain in storage as retained, reopenable session history while
the `SessionRecord` exists: they are surfaced by
`harness.listSessions({ resourceId, includeClosed: true })` and removed only by
an explicit session-first delete, a lower-level thread-delete cascade, or an
...
operation evidence is gone. Routine cleanup should prefer guarded
guarded delete so `HarnessSessionDeleteBlockedError` protects unresolved
dependents; explicit operator force-delete remains available under the delete
rules above. ID-based guarded delete and the operator force-delete path use the
same tenant-safe resource mismatch behavior as close before applying their
cleanup rules.

Detach (proactively flush + drop without closing) is not exposed in v1. It
happens implicitly via eviction. Close remains the explicit non-live/reopenable
lifecycle transition; delete remains the explicit destructive lifecycle
transition.
