...
```ts
    ...
    // Resolves retained duplicate-admission evidence for signals / untyped
    // skills and queued items. It checks retained signal
    ...
    compactOperationResultEvidence(opts: {
      ...
      resourceId: string;
      kind: 'signal' | 'queue';
      // Exactly one public result key must be present: `signalId` for signal /
