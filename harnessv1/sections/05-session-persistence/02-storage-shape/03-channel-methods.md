...
<figure>
  <svg role="img" aria-labelledby="hx-channel-methods-title hx-channel-methods-desc" viewBox="0 0 1040 520" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <text style="font: 600 17px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="520" y="50" text-anchor="middle">Channel bridge storage extensions</text>
    <text style="font: 500 13px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="520" y="70" text-anchor="middle">harness-scoped channel-domain view; the harness layer applies resource/session checks before returning rows</text>
...
```ts
    // Channel bridge records (new in v1). These are Harness-scoped binding and
    // work-ledger capabilities, not provider installation/config rows. Prefer
    // implementing them as an extension of the existing `channels` storage
    // domain. A bound Harness facade may expose them for convenience, but they
    // must not be buried in a monolithic session table or duplicate provider
    // configuration.
    // The existing ChannelsStorage domain remains the owner for credentials,
    // OAuth/provisioning, provider app config, and ChannelInstallation records;
    // these methods reference that configuration through stable provider/channel
    // IDs. The harness storage view scopes rows by `harnessName`, and the harness
