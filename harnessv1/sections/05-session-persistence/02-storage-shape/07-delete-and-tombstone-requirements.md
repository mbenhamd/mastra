...
deleted session subtree: the target `SessionRecord`, the backing thread/message
history for a root/product session delete unless an explicit operator preserve
path is used, thread-scoped OM for that verified conversation, all descendant
