...
<figure>
  <svg role="img" aria-labelledby="hx-event-union-title hx-event-union-desc" viewBox="0 0 1040 440" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <text style="font: 600 16px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="520" y="53" text-anchor="middle">HarnessEventV1</text>
...
// Lifecycle. Session-bearing lifecycle events are session-scoped observer
// events for the affected session and are also live-fanned out to
// `harness.subscribe(...)`. `harness_shutdown` is harness-scoped only and is
// not part of per-session SSE replay. `session_closed` is terminal: it means
// `closedAt` was written. `session_evicted`, `session_hydrated`, and process
// shutdown are non-terminal observer notifications for cache/lease loss,
// rehydration, or process lifecycle; they are grouped here for subscriber
...
// answer several signals, so `agent_end` alone never identifies
// which `signal(...)` call completed. `result` is scoped to the admitted
...
// MastraCode's current `sandbox_access_request` event is not a built-in v1 event
// unless a future owning section adds it here. Path access prompts should migrate
// to `question_pending` or to a dedicated pending kind defined in this union, so
// headless/TUI responders do not depend on a product-local event name.
// Product-level autonomous responders, if any, do not introduce separate
// `goal_*` or `question_answered` events. They consume the same pending item
...
session SSE route. Implementations may keep private non-`HarnessEventV1`
...
`harness.subscribe(...)`, or session SSE `HarnessEventV1` surfaces. The portable
remote display-state contract is `session.getDisplayState()` returning
`HarnessDisplayStateSnapshotV1` (§4.2, §5.1); in-process implementations may
also offer `subscribeDisplayState(...)` with the same snapshot shape. Remote
clients recover display state after reconnect by refetching
`GET /sessions/:sessionId`, not by replaying synthetic display-snapshot events or
opening a second display-specific subscription.
