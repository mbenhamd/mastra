...
- **Per-session FIFO.** Within a single session, events are delivered to every
  ...
  receive future events in order from the moment they subscribe; they do _not_
...
- **Per-operation event identity.** Accepted `signal(...)`
  terminal operation events are scoped to the accepted `signalId`:
  `signal_completed` or `signal_failed`. Admitted `queue(...)` terminal
  ...
  `session_closed`, and `session_evicted` are observer events, not per-operation
  settlement boundaries. `harness_shutdown` is also an observer event, but it is
  harness-scoped and never part of per-session replay. §3, §4.2, §4.4, §5.7,
  §13.2 through §13.4, and §15 own exact promise settlement, stream outer-promise
  admission timing, result lookup, retention, tombstone, recovery
  terminalization, and verification rules.
