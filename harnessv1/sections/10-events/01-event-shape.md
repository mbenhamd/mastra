...
type HarnessEventV1 = HarnessEventBase & (
  | LifecycleEvent
  | StateEvent
  | TurnEvent
  | OperationEvent
...
`HarnessEventV1` is the v1 public/wire projection. Current core already exports
a different `HarnessEvent` during the migration window; implementations must
project legacy events into `HarnessEventV1` instead of reusing the legacy type
name for an incompatible payload set. A package may later alias
`HarnessEvent = HarnessEventV1` only at the cutover where the legacy current
Harness event surface is no longer exported under that name.
