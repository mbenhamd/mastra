...
**Entity ownership:** Session and Harness subscribers; stream/pubsub inputs are
live-layer only and are not durability boundaries — [§0 Mental model](../00-mental-model.md).
...
<figure>
  <svg role="img" aria-labelledby="hx-events-overview-title hx-events-overview-desc" viewBox="0 0 1040 540" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <desc id="hx-events-overview-desc">Sources project into the closed HarnessEventV1 union through a Harness-owned event adapter. Session-scoped events are stamped with an epoch-seq id, buffered in-memory for SSE replay, and fanned out to session and harness subscribers. Harness-scoped events are live-only.</desc>
    <defs>
    ...
    <text style="font: 500 12px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="880" y="90" text-anchor="middle">session lifecycle · shutdown · storage error</text>
    ...
    <text style="font: 500 13px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="60" y="188">projects into the closed HarnessEventV1 union (§10.2); custom events covered in §10.3</text>
...
`HarnessEventV1` union, stamps §10.1 identity in the target session or harness
...
1. **Agent stream chunks.** `AgentChunkType` at
   `../packages/core/src/stream/types.ts:760` is the runtime stream of
   ...
   events — they are current-code inputs only, and adding per-chunk public
   ...
   **after** the corresponding `PendingToolSuspension` row commits under the
   ...
   `raw`, `abort`) are current-code inputs for display state, message
   ...
   not directly to `HarnessEventV1`.
2. **Workflow events.** Workflow runs (`../packages/core/src/workflows/`)
   ...
   public `HarnessEventV1` union and do not cross the SSE boundary.
3. **Pubsub substrate.** Mastra's pubsub primitives
   ...
   `../packages/core/src/agent/durable/stream-adapter.ts:78` package
...
The four sources are current-code inputs; the v1 contract is the closed
`HarnessEventV1` union below and the §13.3d SSE envelope. Stream-chunk
identifiers, workflow-step IDs, pubsub topic offsets, and current event
