...
**Entity ownership:** Harness config policy only — [§0 Mental model](../00-mental-model.md).
...
This "sandbox" term is narrower than MastraCode's current `/sandbox` command
and `request_access` tool, which manage filesystem path allowlists in
`sandboxAllowedPaths`. V1 command policy must not be used as a replacement name
for that path-access state; path access remains workspace/filesystem policy.
MastraCode migration must choose one durable owner for approved path grants
(normally session/workspace state committed through the SessionRecord) and route
both `/sandbox add` and model-initiated `request_access` through that owner. A
tool-updated process-local allowlist is not evidence that a grant survives
restart, clone, or session reopen. If a requested path grant is intentionally
turn-local, the event/pending item must say so and must not be imported as
persisted `sandboxAllowedPaths`.
...
- Compatibility adapters for the current model-facing
  ...
  that adapter shape. Products that remap core workspace tool names, such as
  MastraCode exposing `execute_command`, apply this classification after their
  remapping/normalization step; permission keys and hooks still use the
