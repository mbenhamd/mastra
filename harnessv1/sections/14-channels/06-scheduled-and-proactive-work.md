...
Process-local `harness.registerHeartbeat(...)` is not the primitive for durable channel
...
should build on that shape without duplicating the schedule-definition owner.
The scheduler remains responsible for cron definitions, due-fire calculation,
and workflow publication until a Harness target is explicitly added. The
`HarnessWakeupItem` row is the durable handoff from that source into Harness
session admission, not a Harness-owned cron row, workflow schedule row, or
second scheduler. By itself, the scheduler is not end-to-end durable channel
delivery: the workflow or background task must still resolve a `ChannelBinding`,
queue the session work with idempotency, and rely on the outbox for outbound
effects. A claim-then-publish scheduler that advances
...
binding/workspace intent when needed, the outbox-producing completion policy, and
...
behind a source-specific wakeup, queue, inbox, or outbox row that remains
retryable. A background task
...
Therefore the implementation choice is composition, not duplication:
WorkflowScheduler owns time and schedule definitions, BackgroundTasksStorage owns
reconstructable execution rows, and Harness owns only the wakeup/session/outbox
handoff that proves user-visible work can recover.
