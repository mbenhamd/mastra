...
`sendProviderOperation(...)` and `reconcileDelivery(...)` are restart-safe bridge
operations over provider-owned capabilities:
...
`{ delivered: false }` only after the bridge/provider authoritatively proves
that the provider-visible operation was not delivered. Provider lookup ambiguity,
...
`channelId`; the internal dispatch runtime injects the harness name and cannot
dispatch another harness's rows. Cross-harness dispatch is the Mastra Server
`mastra.harnessChannels.dispatchOutbox(...)` operator API (§13.1), not a public
per-harness bridge API. This matters because a Mastra Server can share one
...
the bridge or defaulted from the operation kind, and a snapshotted
`deliverySemantics`. Bridges whose `reaction-remove` operation is
provider-idempotent SHOULD return `native-idempotency` or `client-message-id`
from `resolveProviderOperation(...)` for that row rather than relying on the
`'at-least-once'` default, because retried removes after a worker crash can
otherwise produce a provider-visible duplicate-remove error.
...
`signal(...)` fan-in is only as restart-safe as the agent-layer
