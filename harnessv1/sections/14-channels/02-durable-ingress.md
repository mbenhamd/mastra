...
replacement policy. The bridge constructs `SignalOptions` / `QueueOptions` from
...
controls that change session defaults or permissions are not inbound signal
...
`model` on an active `signal` admission is not retried unchanged; the bridge
either switches the row to `queue` if policy permits queue handoff, or marks it
`dead` for operator repair. `HarnessSessionClosedError` is interpreted against
the row's already-resolved binding generation: an inbox row tied to a binding
that has since closed marks that same row `dead` with row
`lastError.code = 'session_closed'` (bare `HarnessRowErrorCode`, §4.5d; wire
projection is `harness.session_closed`) unless the row had already durably
resolved a replacement before admission. A later provider event with a new
idempotency key is not this failed row's retry; it runs the normal binding policy
from §14.1, which may reopen the retained owning session or intentionally create
a fresh replacement binding when product policy requires a new conversation.
...
This requires `session.signal(...)` and `session.queue(...)` to honor
...
channel ingress. A live-only `agent.stream(...)` path is not Harness mode.

`signal` remains the default for interactive channel fan-in. `queue` is used
