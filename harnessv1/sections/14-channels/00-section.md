...
**Entity ownership:** Harness internal channel runtime, Storage
(inbox/action/outbox), Session (admission), and existing Mastra
`ChannelProvider`/`AgentChannels` provider ownership; live `AgentChannels` is
fenced in §14.7 — [§0 Mental model](../00-mental-model.md).

Mastra already has per-agent channel transports (`packages/core/src/channels`).
`AgentChannels` is one bot identity across platforms and can wire provider
adapters to a live path: platform webhook -> channel thread mapping ->
`agent.stream(...)` -> platform post/edit. Harness v1 does not use that live
path for harness-bound conversations. It specifies the control-plane bridge that
lets channel traffic preserve Harness session identity, permissions, durability,
approvals, replay boundaries, and proactive outbound delivery.
...
`harness.session(...)`, `session.signal(...)`, `session.queue(...)`, or the
...
```
Channel inbound -> resolveChannelBinding(...) -> harness.session(...) -> session.signal(...) / session.queue(...)
...
<figure>
  <svg role="img" aria-labelledby="hx-channel-flow-title hx-channel-flow-desc" viewBox="0 0 1160 680" width="100%" style="max-width: 1160px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <text style="font: 600 17px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="390" y="213" text-anchor="middle">Signal or queue</text>
...
Before the row is written, the bridge snapshots an operation delivery plan.
`kind` remains the Harness projection category, while `operationKind` /
`operationName` identify the bridge-normalized provider operation that will be
attempted. The bridge first asks `bridge.resolveProviderOperation(...)` when
present. Otherwise it uses trusted enqueue operation fields and snapshots
`deliverySemantics: 'at-least-once'`. If neither the bridge nor trusted enqueue
path provides an operation kind, the bridge snapshots
...
bridge/provider boundary, not the stored shape. Before computing `payloadHash` or
