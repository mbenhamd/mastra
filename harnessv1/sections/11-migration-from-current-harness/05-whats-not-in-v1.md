...
- **First-class read-state and notification APIs.** Harness v1 has no
  ...
  v1 path: products may keep local or application-owned read state keyed by
...
- **First-class durable agent-produced artifact records.** Agent-produced files,
  ...
  outbox kind. Safe v1 path: tools that produce durable outputs include stable
...
- **First-class durable heartbeat API.** `registerHeartbeat(...)` is process-local.
