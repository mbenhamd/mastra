...
```ts
  ...
  // the corresponding pending shape on `SessionRecord` (§5.1) by first
  // verifying the addressed item, persisting an accepted response receipt,
  // then clearing the pending field / updating display as part of the resume
  // attempt for the underlying Mastra workflow:
  ...
  //   method persists `InboxResponseReceipt(status: 'accepted')`, then clears
  //   the pending field, marks the current run `resuming`, updates the display
  //   snapshot from durable state, and calls the Required Agent Resume Boundary
  //   below. Two-phase
  ...
  //
  // Fresh sessions start with `SessionRecord.permissionRules` copied from
  // `HarnessConfig.initialPermissionRules` when configured; otherwise the rules
  // object is empty. `sessionGrants` always starts empty. Product defaults that
  // require per-category behavior, such as MastraCode's read=allow and
  // edit/execute/mcp=ask defaults, must use explicit initial rules instead of
  // relying on the scalar fallback below.
  ...
  // `yolo` over policy `ask`, the default policy, unmatched `ask`, and any
  ...
  //
  // These gates are adapter hooks around the existing Mastra tool-surface and
  // agent execution path; they do not introduce a parallel tool runtime or a
  // second approval DSL. In MastraCode, treating `/yolo` as policy-ask-only
  // rather than "auto-approve every tool-owned approval" is a v1 behavior
  // change and must be migrated deliberately.
  //
  ...
  // any signal on this session. Useful for browser drag-drop with progress
  // UIs and large files. Inline attachments on `signal`/`queue`/`useSkill`
  ...
  // same projection and must not fall back to old thread metadata after an
  // active `SessionRecord` exists; §11.2 old metadata is bootstrap/import input
  // only.
  ...
  // caller already has a `Session` reference. The Session method is the normal
  // app-facing API. ID-based close/delete helpers exist only behind server
  // route, local runtime, or operator/admin boundaries that apply the same §5.5
  // semantics; callers iterating `harness.listSessions(...)` summaries should
  // pass the summary's `resourceId` to that boundary rather than resurrecting a
  // broad public Harness lifecycle surface.
  ...
  // `signal_failed` / `queue_failed` boundary before `closedAt` commits.
  // Closed sessions remain reopenable through the session resolver; close is not
  // a delete and does not rotate to a fresh session record.
  close(): Promise<void>;

  // Destructive session lifecycle. Deletes this closed session and the durable
  // conversation/artifacts it owns after the §5.5 guarded delete checks and
  // cleanup complete. A `preserve` option is intentionally absent in v1; callers
  // that want a copy must use `clone` before deletion. Force delete is not a
  // hidden Session option; it is a separate operator/admin Harness surface.
  delete(): Promise<void>;

  // User-facing session label. This is the session-first wrapper around the
  // backing thread title because app code should not need to drive ordinary
  // lifecycle through thread-first Harness helpers.
  rename(opts: { title: string }): Promise<void>;

  // Copy this conversation into a new usable session. Copies committed message
  // history and, when requested, app-owned thread metadata only. It never copies
  // live process handles, active streams, stale leases, pending queue/inbox
  // state, channel bindings, wakeups, workspace handles, OM rows, or in-flight
  // tool execution ownership.
  clone(opts?: { title?: string; copyAppMetadata?: boolean }): Promise<Session<TState>>;

  // Fork is intentionally absent from v1 until §5 defines partial-history copy
  // semantics and the server route contract. Products that need a branch point
  // must create a fresh session and replay/import the intended history through
  // an explicit product-owned operation.
}
```
