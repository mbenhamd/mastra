...
**Entity ownership:** Harness and Session for product callers; thread
import/history, channel dispatch, background diagnostics, workspace
administration, and force-delete helpers live behind internal or operator
boundaries rather than the public Harness class (§4.1,
[§0](../00-mental-model.md)).
...
- §4.1 Harness — top-level `Harness` API: lifecycle, concrete session
  resolution/listing, catalogs, local heartbeats, and cross-session
  subscriptions.
...
- §4.3 Per-turn overrides — the rules for `model` / `mode` / `addTools` /
  `yolo` overrides that apply to `signal`, `useSkill`, and the serializable
  subset accepted by `queue`.
- §4.4 Operation option types — the option-bag declarations used across the
  Harness/Session surface: list/signal options, queue and skill options,
