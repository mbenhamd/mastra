...
interface SignalOptions<S extends PublicSchema | undefined = undefined> extends HarnessOverrides {
  ...
  // Optional caller-supplied idempotency key for signals from
  ...
  // sync-generate signal form: it calls agent.generate() on a fresh runId,
  // bypasses the signal pathway, is not retry-safe in v1, and is the only
  // signal form that can throw HarnessBusyError.
  output?: S;
  sync?: boolean;
