...
```ts
// Thrown by fail-fast forms (`signal({ sync: true, output })`, `useSkill(...)`)
...
// `signal()` and `queue()` are busy-independent and never throw this error at
...
// Thrown at admission for malformed options (e.g. `signal({ output, stream: true })`,
...
// Thrown at admission when `signal(...)` carries `model`, `mode`, `addTools`,
