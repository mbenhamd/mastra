...
// already has a different current owner. Harness v1 permits only one current
// owner per Harness/thread/resource pair across Active, Closing, and Closed
// reopenable records (§2.2/§5.5).
...
// Thrown by guarded session delete when the closed session still
// has dependent work or descendants that are not terminal. The row IDs are
// diagnostic/audit detail; callers should branch on the error class and either
// retry after cleanup or use the explicit operator force-delete path.
...
//
//   'session_closed'  — session close/lifecycle teardown is in progress. The session is going
