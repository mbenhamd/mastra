...
The v1 scope is deliberately narrow: one active goal per session, no nested goal
graph, no product-specific planner, no alternate judge strategy registry, no
goal dashboard storage, and no independent goal work queue. Products that need
those behaviors build them above `Session` or defer to §11.5. Core Harness owns
only the continuation invariant that must be linearized with the session FIFO
and recovered under the session lease.
...
interface GoalState {
  ...
  maxTurns: number;
  judgeModelId: string;
  // Optional reference to judge memory owned by this session goal. The
  // reference is durable goal state; Memory remains the notebook, not the
  // authority for whether the goal exists, what turn was judged, or what
  // continuation must recover.
  judgeMemory?: GoalJudgeMemoryRef;
...
type GoalJudgeMemoryRef = {
  kind: 'session-goal-memory';
  sessionId: string;
  goalId: string;
  resourceId: string;
...
contract as any other `PendingInboxItem`; a goal judge may decide `waiting` after
such a turn, but it does not become a second inbox responder. MastraCode or
another product may build an opt-in autonomous question-answering controller on
top of the normal inbox response APIs, using deterministic `responseId` values
and first-response-wins conflict rules, but that controller is product policy and
...
admitted under the normal `signal(...)` rules from §3. The eventual
...
Goal judge memory is session-owned goal support. If an implementation uses a
separate memory/history record for judge context, `GoalState.judgeMemory` stores
the durable reference under the owning session and goal. The memory record may
carry summaries or recall material, but it is not the source of truth for goal
existence, decision receipts, continuation recovery, or child ownership. Current
MastraCode-style `${threadId}-${goalId}` memory threads and `forkedSubagent` /
`goalJudge` metadata are legacy import inputs only.

Clone never copies `GoalState` or shares `GoalState.judgeMemory` with the source
session. A cloned session starts with no active goal; callers that want the same
objective on the clone must call `setGoal(...)` explicitly after clone creation,
which creates fresh session-owned judge state and receipts. This prevents copied
sessions from sharing judge context, continuation receipts, or stale child
ownership. Partial-history fork is deferred from v1, so there is no goal fork
policy.
