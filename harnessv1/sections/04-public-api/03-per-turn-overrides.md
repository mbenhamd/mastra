...
`signal` and `useSkill` accept the full per-turn override set. `queue`
...
the resulting `LanguageModel` to the selected Agent call for `signal(...)`,
...
payloads. When a `signal(...)` or `useSkill(...)` entry point starts a run with
...
Product extension hooks that produce runtime-only tools, such as MastraCode
`extraTools` functions derived from `RequestContext`, follow the same rule unless
they are lowered to stable registered tool identities with a recovery-safe
factory and compatibility generation.
...
Callers who need a one-shot custom tool surface should use `signal(...)` on an
