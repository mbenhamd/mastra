...
harness has no "execute a skill" method because there is no session for it to
execute against. `session.useSkill(ref)` is the only public caller-facing
invocation surface. Model-facing skill activation through agent tools is
implementation material (§11), and when a v1 session exposes that path for
activation it must resolve from the same code + workspace catalog rather than an
alternate view. The harness has no `harness.skills` surface, no
`session.skills.*` namespace, and no separate per-deployment skill list; the flat
session surface is the public inspection and invocation boundary.
...
catalog projection, args validation, and prompt expansion; it does not redeclare
operation options.
...
**Resolution.** When `session.useSkill('triage')` is called, the harness first
...
- Discovery runs asynchronously on the first `session.useSkill`,
...
- The in-session refresh path is `await session.refreshSkills()`. It clears
...
**Model-facing activation.** The v1 `useSkill(...)` operation resolves the
skill, validates declared args schema and supplied args, injects args, then
delegates to the `signal(...)` path. It does not depend on the model calling a
skill tool. If a v1 deployment exposes model-facing skill tools or processors,
activation/catalog behavior that claims to represent the session's available
skills must use the same resolution chain as `session.listSkills()` /
`session.getSkill(...)` / `session.useSkill(...)`. Workspace file-oriented
helpers such as skill search across workspace content or reading `references/`,
`scripts/`, and `assets` remain workspace-only.
...
- **Session surface** — discovery view of code + workspace-resolved skills. This is
  ...
  - `session.refreshSkills(): Promise<void>` — drops the cached discovery
    generation.
  - `session.useSkill(ref: string, opts?: UseSkillOptions): Promise<AgentResult>`
...
(§6). It dispatches through `session.useSkill(ref, opts)` against the owning
