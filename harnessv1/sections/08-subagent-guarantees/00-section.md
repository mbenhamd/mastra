...
**Entity ownership:** Session (child rooms), Harness (`HarnessConfig`) —
[§0 Mental model](../00-mental-model.md).
...
- **Relationship to Mastra agent delegation primitives.** Mastra core
  ...
  sessions through `@mastra/core/harness`; they are old implementation inputs,
  not substitutes.
- **Relationship to current forked subagent threads.** Current Harness can clone
  a thread for a forked subagent through `cloneThreadForFork` and stores
  metadata such as `forkedSubagent` / `parentThreadId` on cloned threads. V1
  child sessions do not reuse that as the durable child-session model. During
  migration, fork metadata may seed display/import provenance only; ownership,
  routing, pending inbox, permissions, recovery, and depth checks are proven by
  `SessionRecord.parentSessionId` and the child session's own records.
