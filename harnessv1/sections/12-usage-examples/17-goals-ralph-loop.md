...
// continuation reason as the next queued user-message signal via session.queue(...).
await session.signal({ type: 'user-message', contents: 'Begin.' });

// User changes their mind mid-loop. This signal is admitted under normal
// signal semantics; it does not rewrite the durable FIFO order of any
// continuation that was already queued.
await session.signal({ type: 'user-message', contents: 'Actually, focus on the proration bug first.' });

// Pause without losing the goal — useful for triaging an unrelated bug.
await session.pauseGoal();
await session.signal({ type: 'user-message', contents: 'Quick — what does line 42 of pricing.ts do?' });
