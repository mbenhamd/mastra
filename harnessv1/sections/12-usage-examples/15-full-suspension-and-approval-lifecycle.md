...
```ts
import { Harness, HarnessSessionNotFoundError } from '@mastra/core/harness';
...
session.queue({
  contents: 'Refactor the billing service. Plan first, ask before destructive changes.',
});
...
trusted `resourceId` through the explicit server/runtime close helper.
