...
```ts
import { Harness, LocalWorkspace } from '@mastra/core/harness';
...
// Tearing down a per-resource workspace (e.g. user deleted their account) is an
// operator/admin action, not a public Harness method. The admin surface must
// reject while any session for that resource is still live.
```
