...
Multiple producers sending into the same session concurrently through
`signal(...)` calls. §3 owns the concurrency and settlement
rules.
...
const [a, b, c] = await Promise.all([
  session.signal({ type: 'user-message', contents: 'I think the auth flow is broken' }),
  session.signal({ type: 'user-message', contents: 'Yeah, I just got logged out too' }),
  session.signal({ type: 'user-message', contents: 'Same here, started ~3 minutes ago' }),
]);
...
await session.queue({ contents: 'Step 1: investigate' });
await session.queue({ contents: 'Step 2: write a postmortem' });
await session.queue({ contents: 'Step 3: file a follow-up ticket' });
```

See §3 for the canonical `signal` vs. `queue` behavior.
