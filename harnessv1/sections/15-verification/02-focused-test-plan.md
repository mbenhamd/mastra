...
**Implementation status (Mastra `main`).** Rows in this section are v1
acceptance criteria, not claims that the contract is already implemented or
tested on current `main`. As of the latest spec pass against
`../packages/**/src`: there is no `HarnessStorageDomain`, no v1 `Session`
class, no `/harness/*` route family, and no `HarnessWakeupWorker` (or harness
channel ingress/outbox workers) in packages — only the thread-first `Harness`,
agent-level signals, platform `BackgroundTaskWorker`, and live-layer helpers
(pubsub, display state, `AgentChannels`). Source-evidence notes below mark target
evidence to add at cutover, current-source gaps where `main` already has a known
hole, or no-current-implementation cases where no harness-v1 test exists yet. Do
not treat those notes as proof that v1 storage, workers, or server routes already
ship.
...
Source evidence target: add storage-domain contract tests for inbox de-dupe once
`HarnessStorageDomain` and `createOrLoadChannelInboxItem` exist. Externally
reachable provider callback routes and ingress workers are not on current
`main`; bridge ingress remains a cutover requirement.
...
Source evidence target: `HarnessWakeupWorker` readiness/recovery and Harness
`/harness/*` route startup tests are cutover requirements. Channel inbox/action
callback route workers are not in current `main`. Platform
`BackgroundTaskWorker` tests must not be substituted for harness wakeup recovery
rows.
...
composition: newly created sessions copy `initialPermissionRules` when
configured, per-tool rules override category rules, `defaultPermissionPolicy`
fills missing rules, unmatched effective policy is `ask`, effective `deny` blocks without creating
...
`addTools`, processors, ToolSearch, workspace wrappers, child-session/subagent
...
Session/server tests prove authorized `signal({ yolo: true })` on an
...
`signal({ yolo: true })` return the original signal metadata instead of
re-running the active-run conflict check, and idle `signal({ yolo: true })`,
`queue({ yolo: true })`, and `useSkill({ yolo: true })` still bind the
authorized approval bypass to their fresh run boundary.

**Direct route owner-affinity**

Server tests prove direct HTTP/SDK `signal(...)`, untyped `useSkill(...)`, and
`queue(...)` requests received by a non-owner process are forwarded to the live
session owner before any accepted response is returned. If the live owner cannot
be reached while its lease remains current, the route returns
`409 harness.session_locked` before creating signal, queue, result, or tombstone
evidence. Exact duplicate requests with retained accepted evidence still return
that evidence without rerunning owner routing.
...
valid scoped subscription tokens when that token mode is enabled; rejects expired,
...
Server/SDK tests prove ordinary clients do not receive detailed background-task
list/get/live-event routes from Harness v1. They read only bounded
`durableWork` projections on session list/snapshot models. Operator diagnostic
routes must reject unauthorized callers, apply the selected resource scope before
reading, counting, snapshotting, or streaming tasks, and return tenant-safe `404`
for direct task-ID mismatches under a resource scope. Tests also prove rows
without a trusted owner are hidden from resource-scoped operators unless they use
an explicit unscoped diagnostic capability. Tests cover the v1 diagnostic
...
directly through diagnostic routes.
...
**Explicit legacy import boundary**

Session/storage tests prove opening an old thread with the v1 Harness performs
...
active `SessionRecord`, imports only owner-defined valid old fields, retains
unknown top-level old metadata and unrelated `metadata.app` keys, ignores
malformed or ownerless fields for v1 runtime state, and uses the existing
`SessionRecord` rather than re-reading old runtime metadata on later
hydrations.

**Current behavior characterization before rewrite (pre-v1 `main`, not v1 contract)**

These rows verify the migration-sensitive MastraCode rules owned by §11.6. They
characterize **current MastraCode product behavior on pre-v1 `main`** and the v1
controller rewrite that must re-express the user-visible workflows through
`Session.*`. They are not a bridge layer, not a second source of migration
semantics, and not evidence that v1 already ships. Passing them must fail if
thread-first Harness APIs remain on the product path.

Migration tests pin the current user-visible MastraCode behaviors named by §11.6
before the v1 rewrite. This row characterizes product behavior only; it does not
preserve removed Harness methods or old runtime entry points. The v1 controller
tests then prove the same behaviors route through
`Session.signal(...)`,
`Session.queue(...)`, `session.clone(...)`, `session.rename(...)`, and session
list/read models rather than old
`harness.switchThread(...)`, `harness.setResourceId(...)`, or process-local
current-thread mutation.

**MastraCode session identity propagation**

MastraCode construction tests prove `createMastraCode(...)` creates/opens a
startup `Session` before admitting user input, passes `sessionId` to hook
manager state, analytics/observability correlation, request context, and display
state, and still exposes `threadId` only as durable history identity. Tests
prove thread events from legacy import do not overwrite a v1 session hook id,
request context rejects caller-supplied top-level `harness.sessionId` /
`harness.threadId` overrides, and subagent/tool contexts preserve the owning
session identity rather than the parent TUI's selected thread row.

**MastraCode session controller boundary**

Controller tests prove TUI/headless product APIs depend on a MastraCode session
controller interface, not removed Harness methods. The controller may keep local
UX state such as selected resource, staged `/new`, optimistic pending rows, and
local overlay state, but storage-backed Session state remains authoritative for
admitted signals, queue order, pending inbox, goals, subagent ownership, model
settings, operation result lookup, clone safety, and recovery. Tests fail if
product code calls `harness.switchThread(...)`, `harness.setResourceId(...)`,
`harness.sendMessage(...)`, `harness.sendSignal(...)`, `harness.followUp(...)`,
or `harness.memory.*` as the normal v1 lifecycle/admission path.
A static/import-boundary test proves MastraCode TUI/headless command modules
depend on the MastraCode session controller interface, not on `Harness` directly,
except construction/bootstrap files. Runtime tests then prove all user-visible
workflows still pass through `Session` operations and durable receipts.

**MastraCode headless session selectors**

Headless tests prove `--thread <id|title>` is a MastraCode product selector:
title lookup is resource-scoped and deterministic. Duplicate title matches fail
loudly before calling the Harness resolver; this is an intentional breaking
behavior from the pre-v1 duplicate-title fallback. The selector resolves to
`sessionId` or `{ threadId, resourceId }` before §5.3 session resolution.
`--continue` opens the most recent active/reopenable session for the resource;
`--clone-thread`
maps to `session.clone(...)` and returns a fresh
`sessionId`; `--resource-id` scopes resolution without mutating a global Harness
resource; and `--title` commits through `session.rename(...)` before or after
the submitted prompt according to the documented CLI behavior. Machine-readable
output includes enough identity to disambiguate `sessionId` and `threadId`
during migration, and stale/closed/deleted/cross-resource selectors surface the
normal v1 session errors without hidden thread auto-creation.

**MastraCode headless preflight atomicity**

Headless tests prove the target session is resolved or created before applying
model, thinking-level, title, resource, clone, or prompt side effects. A failed
`--model`, `--thread`, `--clone-thread`, `--resource-id`, `--title`, attachment
admission, or output-schema preflight leaves no orphan `SessionRecord`, no queue
item, no message, no operation tombstone, no workspace materialisation, and no
global Harness state mutation. Successful preflight applies model/thinking/title
only to the resolved session and then admits the prompt through the normal
session operation boundary.

**MastraCode TUI session lifecycle commands**

TUI command tests prove `/threads` lists session summaries and renders
thread-history context; `/thread` opens through §5.3; `/new` keeps the product's current
pending-new semantics by staging a fresh-session intent until first admission, or
explicitly documents and tests an intentional immediate-create behavior change;
startup with no suitable thread stages the same fresh-session intent; `/clone`
uses `session.clone(...)` without copying live handles, leases, pending UI rows, or
in-flight ownership; `/resource` changes the selected resource scope and opens
or stages the expected session; and `/name` commits through
`session.rename(...)`. Tests also prove close/delete hide or reject subsequent
UI admissions according to §5.5 and that pending UI state from one session
cannot bleed into another after navigation.

**MastraCode startup project/resource resolution**

Startup tests prove project-scoped session selection replaces current
thread-first heuristics without losing behavior: legacy `projectPath` metadata
is imported only as bootstrap input, directory/project matching chooses an
eligible session for the current resource, locked or unavailable legacy threads
do not become v1 blanket locks, resource reset changes only the selected
resource scope, and no suitable session stages the same fresh-session intent as
`/new`. Once a `SessionRecord` exists, project metadata is not a runtime source
of truth for mode/model, queue, goal, pending inbox, or recovery.

**MastraCode session-keyed queue and optimistic signal UI**

TUI/headless tests prove immediate signals and queued follow-ups are keyed by
accepted operation identity (`admissionId`, `signalId`, `queuedItemId`) and
session id. Two identical text submissions are distinct operations unless the
same `admissionId` and hash are intentionally reused. Optimistic pending
messages confirm from accepted operation evidence rather than text matching, and
queued work does not drain into a different session after `/thread`, `/new`,
`/clone`, `/resource`, `close`, `delete`, process restart, or resource switch.
Mixed MastraCode controller queued actions preserve current FIFO semantics across
plain messages, built-in slash commands, custom slash commands, skill
activations, and goal/custom command expansions, but raw local slash text is not
itself a core `Session.queue(...)` row. The controller normalizes each action
before durable admission; active/idle delivery chooses `session.signal(...)`,
`session.queue(...)`, or `session.useSkill(...)` by action type without losing
operation identity.

**MastraCode restart and pending UI reconstruction**

TUI restart/reconnect tests prove ask-user prompts, submit-plan prompts, tool
approvals, pending slash/custom command rows, queued actions, optimistic
messages, and tool/status components are reconstructed from `SessionSnapshot`,
pending-inbox read models, thread message pages, and signal/queue result
evidence. They are not reconstructed from memory messages alone, old local
arrays, live PubSub buffers, or text matching. Accepted operations settle after
reattach through result lookup; unaccepted local drafts may disappear.

**MastraCode observability and runtime correlation**

Tracing, hook, analytics, model-header, and eval tests prove runtime correlation
uses `sessionId` while `threadId` remains history identity. Harness request
context, hook manager session id, observability span attributes, provider/model
request headers, analytics events, and eval trace lookup include the owning
`sessionId` and resource; `threadId` may still be present for transcript/history
joins but is not the runtime/session correlation key.

**MastraCode subagent model overrides**

TUI/session tests prove `setSubagentModelId` / `getSubagentModelId` migrate to
session-owned subagent model overrides. Applying a model pack stores mode model
choices, active model-pack id, and subagent model choices in their v1 owners
rather than raw thread metadata; subagent tool execution and child-session
creation use the effective session override; global settings remain bootstrap
defaults only; and overrides survive restart while clone behavior follows the
explicit session clone copy policy.

**MastraCode old metadata import destinations**

Import-boundary tests prove each current MastraCode top-level thread metadata key
family either has an explicit v1 owner or is ignored before §15.2's thread
metadata namespace protection rejects raw writes: goals commit to §4.7
...
Skill command tests make the v1 decision explicit. If `/skill` becomes
`session.useSkill(...)`, it uses skill admission/result evidence, duplicate
handling, restart-safe lookup, and session queue ordering from the skill
contract. If MastraCode intentionally preserves transcript-style skill
activation by rendering a skill block into the conversation, that path still
uses durable `session.signal(...)` / `session.queue(...)` operation identity and
does not bypass result lookup, attachment admission, or restart-safe queueing.

**MastraCode image and attachment submissions**

TUI/headless tests prove pasted images and other attachments are admitted to
Harness-owned storage before provider-visible delivery for immediate signals,
queued follow-ups, slash/custom command payloads, and headless prompts. Restart
after accepted attachment admission preserves the operation; duplicate image
submissions follow admission-id/hash semantics; session/resource switches cannot
move staged or queued attachments into another session; and missing/corrupt
attachments fail through the §4.5 attachment errors instead of substituting
memory-local bytes or temporary paths.

**SignalsPubSub remains live-only**

MastraCode multi-process tests prove `SignalsPubSub` / Unix socket delivery can
fan out live same-session activity but does not provide admission dedupe, result
lookup, queue durability, or recovery. Dropped PubSub messages, socket
unavailability, subscriber restart, and process exit after accepted admission
still leave `Session.signal(...)` / `Session.queue(...)` result lookup and SSE
replay correct from storage evidence.

**Thread lock is not a blanket v1 session lock**

Import-boundary tests prove current core `HarnessConfig.threadLock` and
MastraCode filesystem `threadLock` / `ThreadLockError` are old-runtime thread
coordination and are not used as v1 leases. Multi-client tests prove
read/list/subscribe, authorized message
append, and admitted `Session.signal(...)` are not blocked merely because
another process owns a stale local thread lock, while recovery-sensitive queue
drain, pending-item resume, provider-visible delivery, workspace materialisation,
and side-effect execution still require the storage lease/claim owner from §5.2
and §5.8.

**MastraCode same-session multi-TUI attach**

Multi-process TUI tests prove a second local client opening the same resource
and thread attaches to the same active session instead of creating a replacement
conversation because another process is present. Both clients can read,
subscribe, render snapshots, answer authorized prompts, append authorized input,
and dispatch admitted `Session.signal(...)`; only recovery-sensitive queue
drain, pending-item resume, workspace materialisation, provider-visible
...
Goal tests prove the current separate goal judge memory thread is replaced by
session-owned judge memory or an explicit child-session/history record linked to
`sessionId`, goal id, resource, and parent session. Judge memory survives
restart, does not rely on `forkedSubagent` / `goalJudge` top-level metadata for
ownership, does not collide across clone/reopen, and is cleaned up or
retained according to the same goal/session lifecycle policy as the owning
goal.

**MastraCode goal command ordering**

Goal command tests prove `/goal set`, `/goal pause`, `/goal resume`,
`/goal clear`, plan-started goals, and goal-enabled custom commands commit
`GoalState` under the owning session before any continuation is admitted.
Resume/continue work enters the normal session FIFO; user-queued actions already
in the queue run before goal continuations, and later user actions queue behind
according to commit order. Goal command failures roll back local UI projections
without writing legacy thread metadata as the source of truth.

**MastraCode abort and local cleanup projection**

Ctrl-C/Esc/abort tests prove local UI cleanup is only a projection over session
state. Accepted signals, queued items, pending inbox receipts, and provider
visible work still terminalize or recover through session result evidence and
worker paths; only unaccepted local drafts, staged `/new` intents with no
admission, and purely live UI components may disappear without durable
settlement.

**Package export resolution for `@mastra/core/harness`**

Build/package tests prove the §11.1 `./harness` export resolves to the v1
contract from the package entry map in ESM `import`, CJS `require`, and
TypeScript declaration contexts; the runtime builds emit `dist/harness/index.js`
and `dist/harness/index.cjs`; the declaration-generation path emits
`dist/harness/index.d.ts`; and the ESM, CJS, and declaration targets expose the
expected v1 public names without a `/v1` alias.
...
**Tool suspension migration path**

Tool-runtime and recovery tests prove current Mastra
`context.agent.suspend(...)` / `context.workflow.suspend(...)` migration
...
**Duplicate session-owner corruption**

Resolver/storage tests prove multiple current owners for one
`(harnessName, resourceId, threadId)` fail closed with
`HarnessSessionCorruptError(reason: 'duplicate_session_owner')`.
...
Storage/API tests prove the explicit workspace-admin/operator teardown for a
resource rejects while any persisted active session for that resource exists,
including sessions evicted from memory, and cannot race concurrent session
creation. The public Harness class does not expose this as an app-facing method.
...
or corrupted bytes fails the owning signal/queue/channel/wakeup operation with
...
original `SignalAdmissionResponse` or the original `QueueAdmissionResponse` for
...
that `loadSignalResultEvidence(...)` / `loadQueueResultEvidence(...)` make
...
`SignalAdmissionResponse` / retained terminal result evidence without admitting
...
Server/SDK tests prove `signal({ sync: true, output, admissionId })` and
...
local `Session.signal({ sync: true, output })`,
`RemoteSession.signal({ sync: true, output })`, and typed
...
point, and `signal(...)` posted while the judge evaluates follows normal
`signal(...)` admission rather than queue preemption.
...
resume on a later user `signal(...)` until the next assistant turn drives a
...
**Product autonomous responder races with human or channel question response**

Session/server tests prove an optional product autonomous responder uses a
deterministic `responseId`, writes a normal `InboxResponseReceipt` with
`responder` metadata only if it wins the owning-session lease, and obeys
first-response-wins: human/channel responses that commit first consume the
pending question and the product response is discarded; product responses that
commit first make later human/channel responses observe the normal
stale/consumed/conflict result.

**Product state changes while autonomous response is in flight**

Session tests prove closing/closed-session state, or a missing/changed pending
question before the receipt commit, prevents the autonomous responder from
writing an `InboxResponseReceipt`, emits no extra goal event, and leaves any
still-present pending question answerable by normal inbox callers.

**Crash after accepted autonomous question response before resume**

Recovery tests prove `InboxResponseReceipt(status: 'accepted', responder: ...)`
retries the run resume with the same deterministic `responseId` /
`resumeAttemptId`, does not run the responder again for the consumed pending
item, and does not apply a second resume after the receipt becomes `applied`.
...
operation-scoped `signal_completed` or `signal_failed`, and that `agent_end`,
lifecycle events, or unrelated signal completions do not settle the wrong
promise. Recovery tests prove this remains true when the current agent runtime's
process-local pending signal maps are lost after acceptance: retained
signal-result evidence, not persisted message text or PubSub delivery, settles
result lookup for each `signalId`.
...
and does not by itself settle signal/queue operations or mark the session
...
schedule/proactive routes) is refused before `mastra.shutdown()`. Worker-owned
scopes return `503 harness.worker_unavailable` with
`reason: 'server_draining'`; ordinary session/attachment routes return
`503 harness.server_draining`; a deployment may also use a layer-7
connection-stop. Admitted in-flight turns, SSE consumers, and route handlers
...
Current-source evidence: shutdown may stop the platform background task manager
on current `main`; harness wakeup workers and logbook drain proofs are not on
main. A complete drain proof still needs row-level evidence for every externally
reachable durable-ingress route family at cutover, especially channel callback
routes/workers.
...
runtime-drift fail-closed paths record `signal_failed` / `queue_failed` for
...
`signal`, `queue`, `useSkill`, inbox responses, resume calls, descendant
...
`signal_failed` / `queue_failed`, pending prompt fields are cleared, active
...
`closedAt` leaves a visible Closing record that reserves the owner key, rejects
new work, and is completed by a later `closeSession(...)` owner using the stored
`closeDeadlineAt` without resetting the timeout. Retrying
`POST /sessions/:sessionId/close` after an HTTP timeout resumes the same marker
and does not start a second close; if `closedAt` already committed for a
tenant-verified retained session, the retry is a successful no-op close response.
Server/SDK tests prove ambiguous close transport failures recover through
retrying close and/or reading
...
Current-source gap: agent/server stream tests on current `main` do not
substitute for §13 harness session routes, Last-Event-ID replay, stale/gap
`412`, snapshot recovery, or harness signal/queue result lookup. Final release
evidence must map each replay/result row to harness server and SDK assertions
after §13 lands.

**Embedded `AgentResult` identity consistency**

Event/storage/SDK tests prove that when `AgentResult` appears in
`signal_completed`, `queue_completed`, `QueueAdmissionReceipt.result`,
...
signal/queue, wakeup -> queue, goal decision -> continuation queue receipt,
...
`SessionRecord.displayState`, `GET /sessions/:sessionId`, and `412` recovery use
...
into `Workflow.startAsync(...)`. This is a known pre-v1 hole, not proof that v1
session storage already exists.

**Reserved request-context key rejection**

Server and in-process admission tests prove `signal`, `queue`, `useSkill`, and
...
retryable or dead-lettered. Hydration also proves the explicit old-row import
rule: run snapshots without a persisted generation fall back to ID-only
validation.
...
rows; diagnostics do not settle signal/queue operations; they redact/bound
...
execution for that run. Coverage spans `signal`, `signal({ stream: true })`,
`signal({ sync: true, output })`, `useSkill`, and current-code input paths for
per-run executable toolsets/client tools after registry loss. Tests prove
metadata-only tool snapshots and same-named registered substitute tools are not
...
Delete tests prove guarded `deleteSession(...)` rejects with
...
Delete tests prove the §5.5 explicit operator force-delete lifecycle fences the
owner, walks active and closed descendants bottom-up, marks
dependent `ChannelInboxItem`, `ChannelActionReceipt`, `ChannelOutboxItem`, and
`HarnessWakeupItem` rows terminal `dead` with `session_deleted`, revokes retained
`ChannelActionToken` rows for the deleted session subtree with `session_deleted`,
invalidates or waits out any active worker claim before completion, closes
bindings with `closedReason: 'session_deleted'`, and skips already-terminal rows
without re-emitting close settlement.
...
before serialization. The current `HarnessEvent` projector rejects unknown
current strings before serialization and requires an explicit §13.3f.1 mapping
before any new current code can cross the v1 boundary.

**`AgentChunkType` to `HarnessEventV1` selected projection**
...
project through §4.8c / §5.1b.2 durable-work projections or operator
background-task diagnostics rather than `HarnessEventV1`. Tests also prove the
...
Storage/session/memory tests prove `Session.signal(...)`, drained `queue(...)`,
...
**Session-first thread-delete cascade lifecycle**

Lifecycle/thread tests prove lower-level thread delete and
`DELETE /harness/:name/operator/threads/:threadId` resolve the thread inside
...
accepted as the public v1 lifecycle. Only the Harness operator thread-delete
...
is used; no delete affects another Harness, another resource, or resource-scoped
OM.
...
`signal_failed` / `queue_failed` is recorded only for unresolved operations.
...
**Local heartbeat validation**

Unit tests reject duplicate IDs and invalid `intervalMs`, verify `immediate`
defaults to `false`, skip overlapping ticks, and prove the async unsubscribe
returned by `registerHeartbeat(...)`, `stopHeartbeats()`, and shutdown await
in-flight handler plus shutdown hook.
...
rejected at init. Provider-owned capabilities may be reused only behind durable
outbox projection/dispatch, not by rewriting live channel tools in place.
...
Storage/server tests prove `session.clone(...)` loads the source only inside the
authenticated `(harnessName, resourceId)`, creates a new same-resource thread
plus fresh `SessionRecord` ownership, preserves message order, writes clone
provenance, optionally copies only `metadata.app`, strips reserved top-level
runtime/fork/channel metadata, leaves source thread/session state unchanged, and
copies no session/channel/queue/pending/wakeup/outbox/tombstone/workspace/memory
rows. Separate operator tests prove `POST /operator/threads/:threadId/clone`
performs only the lower-level history/import thread clone, returns
`HarnessThread`, and never creates or switches a live session. Both paths reject
same-resource `newThreadId` collisions, preserve each cloned attachment ref's
original `ownerSessionId` / `attachmentId` / `sha256`, reject before writing when
a source ref is missing or digest-mismatched or the adapter cannot atomically
register cloned message-history references, reject cloned historical refs as live
inputs to another session, and keep source attachment bytes guarded through
source-session delete while cloned message refs remain.
...
`workspace_execute_command` migration wrapper keeps shell-string inputs in
