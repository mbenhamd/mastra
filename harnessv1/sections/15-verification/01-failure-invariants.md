...
**Admission identity in verification rows:** hash inputs and option-field rules
are canonical in
[§4.4b](../04-public-api/04-operation-option-types/02-queue-and-skill-options.md);
durable receipts, tombstones, and retention are canonical in
[§5.1d](../05-session-persistence/01-what-gets-persisted/10-queue-admission-and-tombstones.md).
Rows below state promises only; they must not introduce parallel hash algorithms
or storage shapes.

**Direct interactive `signal(...)`**
...
`signal_failed` or result lookup, and unresolved accepted signals settle
...
Promise: `signal({ sync: true, output })` and `useSkill({ output })` call the
...
**Explicit legacy thread import**

Authoritative record: explicit import receipt plus imported `SessionRecord`

Promise: Normal v1 `harness.session(...)` resolution does not silently create a
session row from legacy `HarnessThread` metadata. Legacy thread conversion is an
operator/import path only: the caller chooses the target `(harnessName,
resourceId, threadId)`, the importer validates resource ownership and the
current-owner key, and imported fields are limited to deterministic mappings
owned by §11.2 and their canonical v1 owners. Existing active v1 records win over
legacy input. Malformed, ambiguous, unsupported, or ownerless legacy fields are
preserved only as import audit/source metadata and are ignored for v1 runtime
state. V1 mutators preserve unknown top-level legacy metadata when the import
policy keeps it, but never consult `metadata.app` as runtime state.
...
user/assistant message commits from `signal(...)`, drained `queue(...)`,
...
Promise: `session.clone(...)` creates a new usable session in the same
`(harnessName, resourceId)` namespace and copies committed message history plus
explicitly requested app-owned thread metadata as a point-in-time snapshot with
fresh ownership. The v1 clone implementation may call or wrap lower-level thread
clone only after applying the v1 field policy: the resulting session clone never
copies `SessionRecord` state, active runtime ownership, queue/admission receipts,
pending items, channel bindings, channel inbox/action/outbox rows, wakeups,
operation tombstones, per-session workspace state, goals, permissions,
model/mode overrides, sandbox/path grants, or memory/observational-memory rows.
Cloned messages with `PersistedAttachment` refs keep the original
`ownerSessionId`, `attachmentId`, and `sha256` as guarded message-history
references; if the adapter cannot resolve every ref to matching bytes and
register those cloned message refs atomically with the clone, clone rejects
before creating dangling history.
...
Authoritative record: current-owner `SessionRecord` uniqueness on
`(harnessName, resourceId, threadId)` plus `createOrLoadCurrentSessionOwner`

Promise: Concurrent cold starts for one thread/resource inside one Harness
namespace resolve to one current owner. Closed rows remain the reopen owner for
their thread/resource until delete/hidden cleanup or explicit fresh/clone creates
a different thread/session identity; no blind read-miss/create race can
create duplicates or cross into another Harness namespace.
...
`HarnessConfig.initialPermissionRules` at session creation,
...
unmatched `ask`; an effective `deny` is terminal and beats grants, `yolo`, and
tool-owned approval reasons without creating `pendingApproval`.
Fresh-session category defaults are explicit bootstrap data, not implicit
MastraCode compatibility: products that need non-scalar defaults must seed
`initialPermissionRules`, and after creation the session row is the only
...
Non-Harness generic Mastra tool execution remains outside Harness v1.
