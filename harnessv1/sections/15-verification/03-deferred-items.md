...
- **Durable heartbeat as a first-class Harness API.** Deferred by the
  ...
  `registerHeartbeat(...)` remains process-local; restart-safe heartbeat is modeled
...
- **Workspace filesystem event ingestion into memory.** Deferred by §11.5
  ...
  observational-memory ingestion path. Safe v1 path: agents summarize
...
- **First-class durable agent-produced artifact records.** Deferred by
  ...
  routes.
- **Operator repair APIs beyond dispatch.** v1 specifies internal per-harness
  outbox dispatch, the central cross-harness
  `mastra.harnessChannels.dispatchOutbox(...)` operator surface from §13.1,
  dead-letter/undeliverable row states, and explicit migration constraints.
  Read-only channel diagnostics are included only as redacted, side-effect-free
  session-scoped summaries plus optional operator-only channel-wide diagnostics
  (§14.8). Product-specific repair, binding migration UI, manual
  replay/re-projection, per-row retrigger, administrative reconciliation,
  retry/retarget controls, and stuck-session repair routes are deferred; the safe
  v1 path is no automatic retargeting, no diagnostic-surface mutation, and no
  hidden retry after terminal `dead`/`undeliverable`.
...
- **Automatic closed-session garbage collection and session-history
  ...
  cleanup worker for closed `SessionRecord` rows. Safe v1 path: operators
  or product-owned routines call the explicit resource-scoped delete helper or a
  session-first thread-delete cascade after their own retention policy no longer
  ...
  closed session keeps it as the reopen owner for its
  `(harnessName, resourceId, threadId)` tuple; deleting or hiding it is what
  removes that owner from `includeClosed` and makes session result/admission
  routes use tenant-safe not-found. A new conversation uses explicit
  fresh/clone semantics with a different thread/session identity.
  Retained source-specific rows, if any, keep only their own point-duplicate
  semantics.
- **Per-principal read-state and notification model.** Deferred by §11.5.
  ...
  Safe v1 path: products may keep local or application-managed read state
...
- **Richer result, stream chunk, and typed-generate durability schemas.** v1
  ...
  typed synchronous output, or a generate-admission receipt. Safe v1 path:
