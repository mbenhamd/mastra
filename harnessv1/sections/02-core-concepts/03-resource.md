...
`(harnessName, resourceId, threadId)` defines the current session ownership key:
there can be only one current owner across Active, Closing, and Closed
reopenable records (§2.2).
