...
Cardinality: One workspace per session, provisioned on demand, released from the
live process on close, and destroyed only by session delete, explicit
workspace-admin cleanup, or a provider retention policy that preserves the
session's reopen semantics
...
Harness v1 exposes no public harness-level workspace handle. Out-of-session
init scripts, admin tooling, or batch jobs that need a shared workspace use
deployment-owned runtime APIs; product code and tools resolve workspace through
the owning session.
...
- `per-resource` — torn down through an explicit workspace-admin/operator
  ...
  Outlives individual sessions; operators run sweepers for stale resources.
- `per-session` — live handles are flushed and released on `session.close()`;
  durable provider identity/state remain on `SessionRecord.workspace` when the
  workspace is resumable so the closed session can reopen. Destructive teardown
  happens on `session.delete(...)`, explicit operator cleanup, or provider
  retention expiry that also makes future workspace-dependent work fail closed.
...
Per-session durable workspaces are durable across server restarts and across
`session.close()` / reopen while the retained `SessionRecord` exists: provider
state is persisted in the session record (§5.1). On rehydration or reopen, the
provider resumes the workspace from its stored state. `session.close()` must not
destroy durable provider state by default; otherwise close would be equivalent
to a destructive delete for workspace-dependent sessions.
