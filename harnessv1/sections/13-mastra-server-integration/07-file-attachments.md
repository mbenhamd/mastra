...
File attachments are caller-supplied model inputs. Attachment bytes use the
configured storage byte owner (`BlobStore` when available, or an equivalent
object-store implementation behind the Harness attachment view); Harness owns the
session-scoped attachment metadata, durable references, and guarded deletion.
...
`SignalRequest` (with `files` omitted), and each file part is the raw bytes
...
an `attachmentId`, then POSTs the `SignalRequest` as JSON with `kind: 'ref'`
...
await session.queue({
  contents: 'Look at this screenshot',
...
the same flow internally: the harness writes bytes through the configured blob
owner, then persists Harness attachment metadata before any durable record can
reference the file. Guarded delete, reference listing, and retention checks are
owned by §5.2's attachment metadata contract; force-delete cleanup is owned by
§5.5.
...
The narrow exception is committed cloned message history. Session-first
`clone(...)` may retain guarded historical `PersistedAttachment` refs under
§5.2's clone graph. Those refs are read/replay/history evidence for the cloned messages
...
Session close and delete attachment cleanup follow the lifecycle rules in §5.5,
which defer guarded attachment references to the §5.2 storage graph and must
respect cross-thread clone references while they exist. ID-based server/runtime
helpers apply the same lifecycle outside the app-facing Harness class.
