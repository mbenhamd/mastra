...
_does_ cross the wire — is the shared `RemoteSafeSession` interface introduced
in §2.6, and is the only session surface that portable client code should depend
on.

- **Direct workspace access** (`session.getWorkspace()`, raw `Workspace`
  handles). The workspace lives server-side; the client interacts with it through
  tools, events, and the file-attachment routes. There is no remote
  `harness.getWorkspace()` because the public Harness class has no workspace-handle
  method. Attachment routes are for caller-supplied inputs only, not generated-file
  browsing or artifact fetch (§11.5, §15.3). Products that need a generated-output
  browser expose product-specific workspace projections or application datastore
  routes with their own authorization; direct handles would punch through the
  trust boundary.
- **Function-valued `addTools` per-turn override.** Tool implementations are
  ...
  storage can't reproduce. On `SignalOptions` and `UseSkillOptions` it's allowed
...
- **Raw top-level thread metadata writes.** `setThreadSetting(...)` crosses the
  ...
  channel subscription fields, memory/working-memory fields, legacy
  forked-subagent markers, token usage, thread titles, or future Harness/Mastra
  namespaces. Those
...
- **Local default-resource helpers.** `harness.getDefaultResourceId()` is an
  ...
  prove tenant scope.
- **Heartbeat handlers** (`harness.registerHeartbeat`). This is also a Harness
  method and a server-side concern; clients can't register code to run on the
  server.
