...
`POST /signals` and untyped `POST /skills/:skillName` return
`SignalAdmissionResponse` after signal admission. A first accepted signal
...
`GET /signal-results/:signalId` for settlement. The stream route carries the
...
```ts
type SignalResultResponse =
  | { kind: 'signal'; status: 'pending'; signalId: string; runId?: string }
  | { kind: 'signal'; status: 'completed'; signalId: string; runId: string; result: AgentResult }
  | { kind: 'signal'; status: 'failed'; signalId: string; runId?: string; error: HarnessPublicErrorProjection }
  | { kind: 'signal'; status: 'expired'; signalId: string; runId?: string; error: HarnessPublicErrorProjection };
...
`SignalAdmissionResponse` or the original `QueueAdmissionResponse` as
...
result-lookup-only terminal projections in the §13.3 error union, not §4.5 typed
Harness classes. Ordinary failed results carry the same public Harness error
projection so SDK recovery can rehydrate typed subclasses with details when a
typed Harness class exists, and can still branch on a stable discriminated code
when it does not.
