...
**Admission identity on the wire:** DTOs carry optional `admissionId` fields
only. Hash inputs, conflict behavior, and serializability rules are canonical in
[§4.4b](../../04-public-api/04-operation-option-types/02-queue-and-skill-options.md);
durable receipts/tombstones are canonical in
[§5.1d](../../05-session-persistence/01-what-gets-persisted/10-queue-admission-and-tombstones.md).
§13.3b/§13.3e project those contracts; they must not redefine hashing.
