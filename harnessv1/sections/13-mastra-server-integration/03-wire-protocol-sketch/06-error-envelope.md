...
type HarnessErrorResponse = HarnessErrorResponseBase &
  ...
  (
    ...
    | {
        ...
        details: { sessionId: string; attachmentId: string; references: Array<{ source: string; sourceId: string }> };
      }
    | {
    ...
    | {
        ...
        details: { skillName: string; searchedSources: Array<'code-registered' | 'workspace'> };
      }
    | {
    ...
    | {
        ...
        details: { resourceId: string; threadId: string; requestedSessionId: string; activeSessionId: string };
      }
    | {
    ...
    | {
        ...
        details:
          ...
          | { reason: 'duplicate_session_owner'; resourceId: string; threadId: string; ownerSessionIds: string[] }
    ...
    // ── Operation-result lookup terminal projections (no typed Harness class) ─
    | {
        code: 'harness.result_expired'; // Result lookup found compact
        // tombstone identity but no longer
        // has reconstructable result evidence.
        details: { kind: 'signal' | 'queue'; signalId?: string; queuedItemId?: string };
      }
    | {
        code: 'harness.queue_exhausted'; // Queue receipt reached a terminal
        // retry/dead state before producing
        // an AgentResult.
        details: { sessionId: string; queuedItemId: string; attempts: number };
      }
    ...
    | {
        ...
        // drain refusal for this worker scope, so callers retry against
        // the next instance rather than the same one (§13.6).
        retryable: true;
        details: {
          harnessName: string;
          channelId?: string;
    ...
    | {
        code: 'harness.server_draining'; // HTTP 503 drain refusal for ordinary session/attachment routes
        // that are not tied to a worker scope. Not a typed Harness subclass.
        retryable: true;
        details: {
          harnessName: string;
          route: string;
