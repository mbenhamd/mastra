...
of crossing the process boundary directly. Example for `POST /signals`:
...
interface SkillInvocationRequest {
  ...
  mode?: string;
  yolo?: boolean;
  // Same §4.4 request-context allowlist as SignalRequest.
  requestContext?: RequestContextInput;
  // addTools is not sendable over the wire — see §13.5
}
```
...
`payload` part (containing the `SignalRequest` minus `files`) plus one file
part per attachment. The server promotes uploaded files into pre-stored
attachments and rewrites the signal to use `kind: 'ref'` references before
