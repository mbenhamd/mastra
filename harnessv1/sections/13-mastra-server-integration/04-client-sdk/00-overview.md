...
await session.queue({ contents: 'Refactor auth' });
```
...
Existing non-Harness agent/workflow/memory/MCP surfaces remain outside
`HarnessClient` and `RemoteSession`; they are not compatibility paths for
Harness lifecycle.
