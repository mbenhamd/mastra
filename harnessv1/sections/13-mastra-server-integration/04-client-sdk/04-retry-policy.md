...
means `signal(...)`, `queue(...)`, and untyped `useSkill(...)`
when the SDK has supplied or preserved an `admissionId`. The SDK must not
automatically retry `signal({ sync: true, output })` or `useSkill({ output })`
...
For SDK retry capability to apply to `signal(...)`,
...
the same [§4.4b](../../04-public-api/04-operation-option-types/02-queue-and-skill-options.md)
admission hash inputs (normalized content, file references,
...
`RemoteSession.close()` / `POST /sessions/:sessionId/close` is retry-safe
because the route is idempotent against the stored `closingAt` / `closedAt`
close marker (§5.5). After an ambiguous transport failure, the SDK may retry the
same close POST; if it cannot obtain a close response, it refreshes
