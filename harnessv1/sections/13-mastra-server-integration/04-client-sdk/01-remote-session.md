...
- composes the admission route, session event stream, and result lookup routes
  ...
  `signal(...)`, untyped `useSkill(...)`, and `queue(...)` retain the server's
...
local-looking return shape is an SDK wrapper: it does not expose subscription tokens,
...
`setState`, and `refreshSkills` are absent from `RemoteSession`;
`registerHeartbeat` and cross-session subscriptions are absent from the remote
harness type.
