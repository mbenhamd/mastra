...
const session = await harness.session({ threadId: { fresh: true }, resourceId });
```

Single-harness registration for the common case:

```ts
new Mastra({ harness: { default: codingHarness } });

mastra.getHarness('default');
```
...
effect. The server binds each Harness storage view to its immutable
`harnessName` (`default` in the single-harness example above),
...
Per-harness outbox dispatch is internal runtime code that can only claim rows
owned by that harness. Cross-harness channel workers are Mastra Server operator
code and use the registry, not an arbitrary harness instance:
