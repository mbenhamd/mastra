...
additions, not current built-in server routes. Single-harness deployments still
register an explicit Harness name; this spec uses `default`, so its wire routes
are mounted under `/harness/default`.
...
<figure>
  <svg role="img" aria-labelledby="hx-route-families-title hx-route-families-desc" viewBox="0 0 1080 520" width="100%" style="max-width: 1100px; height: auto; display: block; margin: 1.5rem auto; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 18px; padding: 16px; box-sizing: border-box;">
    ...
    <desc id="hx-route-families-desc">Authenticated client routes mount under a Harness name and split into session, history-read, background task, attachment, channel callback, and optional operator route families.</desc>
    <defs>
    ...
    <text style="font: 500 13.5px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="155" y="212" text-anchor="middle">signal / queue / state / inbox</text>

    <rect style="fill: #f8fafc; stroke: #94a3b8; stroke-width: 2; rx: 14;" x="315" y="160" width="200" height="70" />
    <text style="font: 600 17px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #0f172a;" x="415" y="189" text-anchor="middle">History</text>
    <text style="font: 500 13.5px system-ui, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif; fill: #475569;" x="415" y="212" text-anchor="middle">reads, not lifecycle</text>
...
supplies `sessionId` for a pair that already has a different current owner
(`Active`, `Closing`, or retained reopenable `Closed`) in the route's Harness
namespace, returns `409 harness.session_conflict`. A
...
`/channel-diagnostics`, detailed background-task diagnostics remain
operator-only, and signal/queue settlement still uses result lookup.
...
**`GET` `/harness/:name/sessions/:sessionId/messages`**

List committed messages for the session's backing thread after verifying the
addressed session belongs to the authenticated resource. Supports `?cursor`,
`?limit`, and optional `?order=asc|desc`; returns
`WireListPage<HarnessMessage>` over the stable `(createdAt, id)` message key.
This is the ordinary `RemoteSession.listMessages(...)` and reconnect history
route. It reads the shared memory-domain message log through the verified
session boundary; it is not a thread lifecycle API and does not expose raw
thread mutation.

**`PATCH` `/harness/:name/sessions/:sessionId/title`**

Rename the user-facing session label — the wire form of
`RemoteSession.rename({ title })`. Body is `{ title: string }`. The server
derives `resourceId` from auth, verifies the session belongs to that resource,
requires `If-Match` with the session `ETag`, serializes the write under the
session lifecycle/lease authority, and updates the backing thread title only
through this session-first boundary. Stale validators reject with
`409 harness.state_conflict`; closing or deleted sessions reject through the
matching session lifecycle error. Successful writes return a new `ETag`. This is
not an operator/history thread route, does not expose raw `MemoryStorage`
mutation, and does not alter `SessionRecord.state` or app-owned thread metadata.

**`POST` `/harness/:name/sessions/:sessionId/close`**
...
retained session that is already closed as a successful no-op. Closed sessions
remain reopenable through `POST /sessions` or SDK `harness.session(...)`.
Harness v1 has no
separate async close-status route: clients recover ambiguous close transport
failures by retrying close and/or reading `GET /sessions/:sessionId` as
described in §13.4. Deleted or staged-deleted sessions remain tenant-safe
not-found.

**`DELETE` `/harness/:name/sessions/:sessionId`**

Delete a session and the durable conversation/artifacts it owns after the §5.5
guarded delete checks. The server derives `resourceId` from auth; cross-resource
IDs return tenant-safe `404`. Ordinary client delete applies only to a
tenant-verified retained `Closed` session. `Active` or `Closing` sessions reject
with the matching lifecycle conflict (`harness.session_closing` or a delete
blocked/conflict response) rather than implicitly closing and deleting in one
client call. Non-force delete is the ordinary path and may return
`409 harness.session_delete_blocked` while dependent work remains.
Operator-authenticated force delete is a separate protected path and must not be
available to ordinary client routes by sending a hidden flag.

**`POST` `/harness/:name/operator/sessions/:sessionId/force-delete`**

Explicit operator/admin route for the §5.5 force-delete path. The request must
be authorized by an operator capability and must include a human-readable
`reason`; it is not part of `RemoteSession` and is never selected by adding
`force` to an ordinary delete request. It derives or verifies `resourceId` under
the operator policy, applies the force-delete fence and cleanup rules, and
returns no body on success.

**`POST` `/harness/:name/sessions/:sessionId/clone`**

Create a new usable session from this session's committed message history.
Body is `{ title?, copyAppMetadata? }`. The server verifies the source session
belongs to the authenticated resource, composes the lower-level same-resource
thread clone from §5.2, creates a fresh active `SessionRecord` for the cloned
thread, and returns the new `SessionSnapshot` or `RemoteSession` bootstrap DTO
used by §13.4. The clone copies committed messages and optional app-owned thread
metadata only; it never copies live process handles, active streams, stale
leases, pending inbox items, current-run ownership, queue rows, permissions,
token usage, goals, display snapshots, wakeups, channel rows, outbox/action/inbox
rows, workspace handles, or OM rows. It is non-retry-safe without a future clone
idempotency key. This is a v1 migration change from current `Memory.cloneThread`
/ `Harness.cloneThread` behavior, which can copy working memory and
observational-memory records; the v1 route must either call lower-level clone
with OM/working-memory copy disabled or filter those records out before the new
session becomes visible. Fork/partial-history clone is deferred from v1 and has
no route.

**`POST` `/harness/:name/sessions/:sessionId/signals`**

Admit a session signal. Body
`{ type?, contents, files?, attributes?, metadata?, admissionId?, ...overrides }`,
where wire overrides are only serializable fields such as `model`, `mode`,
capability-gated `yolo`, and caller-writable `requestContext.app`; `addTools` is
not sendable. Omitted `type` defaults to `user-message`. Returns
`SignalAdmissionResponse` (§13.3) with `runId`, `signalId`, and
...
§13.3 own retained evidence, tombstones, and lookup responses. The route is
owner-affinity: a non-owner server process forwards to the current live owner
before acceptance, or returns `409 harness.session_locked` before admission if it
cannot reach that owner while the lease remains current. This route never maps
busy state to `409 harness.busy`; admission errors map to
`400 harness.validation`, `409 harness.session_locked`,
`409 harness.session_closing`, `404 harness.session_closed`,
`503 harness.storage`, `409 harness.override_conflict`,
`403 harness.forbidden`, `409 harness.attachment_unavailable`, or
`409 harness.admission_conflict`.

**`POST` `/harness/:name/sessions/:sessionId/signals?sync=true`**

Sync signal with typed output (`signal({ sync: true, output })`). Body uses
...
**`POST` `/harness/:name/sessions/:sessionId/signals?stream=true`**

Stream a turn (SSE, `signal({ stream: true })`). Admission failures use the
...
through the signal-result lookup route.

**`GET` `/harness/:name/sessions/:sessionId/signal-results/:signalId`**

Read status for an accepted signal, including for tenant-verified
closed sessions with retained operation evidence. Returns the
`SignalResultResponse` union from §13.3 (`pending`, `completed`, `failed`, or
...
where wire overrides are only serializable fields such as `model`, `mode`,
capability-gated `yolo`, and caller-writable `requestContext.app`; `addTools` is
rejected. Omitted `type` defaults to `user-message`. Returns
...
responses. The owner appends under its session lease; a non-owner route handler
forwards to that owner before acceptance or returns `409 harness.session_locked`
before admission. This route never maps busy state to `409 harness.busy`;
admission errors map to `400 harness.validation`,
`409 harness.session_locked`, `409 harness.session_closing`,
...
route admits through the `signal(...)` path after skill resolution and
returns `SignalAdmissionResponse`; with `output`, it shares the sync-generate
...
same object-only and failure-mapping rules as `/signals?sync=true`. File
normalization failures use `409 harness.attachment_unavailable` before skill
execution. Untyped skill admission uses the same owner-affinity rule as direct
`signal(...)`; failure to reach the live owner before acceptance maps to
`409 harness.session_locked`. §4.2 owns skill execution and busy semantics; §4.4
owns admission conflicts. Other first-call busy cases may respond
`409 harness.busy`.
...
`tool_call` / `tool_result` message parts, retained signal-result and
...
**`GET` `/harness/:name/operator/threads`**

Optional operator/history helper: list threads for the authenticated resource.
Supports `?cursor` and `?limit`; returns `WireListPage<HarnessThread>` ordered
by `(updatedAt DESC, id DESC)`. This is not the normal session lifecycle
surface, is not part of `RemoteSafeSession`, is not generated into default client
SDKs, and must require an operator/history capability rather than ordinary
session-use permission.

**`POST` `/harness/:name/operator/threads`**

Operator/import history helper only: create a bare thread row without opening a
session room. Normal app and SDK code must create or open conversations through
`POST /sessions` (session-first). There is no matching in-process thread-create
method on `Harness` (§4.1). Servers must not expose this route through default
client SDK lifecycle helpers.

**`GET` `/harness/:name/operator/threads/:threadId`**

Operator/history helper: get a thread after verifying it belongs to the
authenticated resource. This is a transcript/history read, not session open.

**`POST` `/harness/:name/operator/threads/:threadId/clone`**

Operator/import history helper: clone a thread for the authenticated resource.
This is not `session.clone(...)`; it never creates or
switches a live session. Body is `CloneThreadRequest`
...
**`PATCH` `/harness/:name/operator/threads/:threadId`**

Operator/history helper: rename the backing thread label only. Normal clients
must use `session.rename(...)` or
`PATCH /harness/:name/sessions/:sessionId/title` so lifecycle remains
session-first. There is no in-process thread-rename method on `Harness` (§4.1).
This route must require operator/history capability.

**`DELETE` `/harness/:name/operator/threads/:threadId`**

Destructive operator/history helper: delete a thread through the §5.5
session-first delete cascade. Normal clients use `DELETE /sessions/:sessionId`;
this route is only for callers that explicitly intend to remove the whole
backing transcript and have operator/history authority.

**`GET` `/harness/:name/operator/threads/:threadId/messages`**

Operator/history helper: list messages for a thread after verifying it belongs
to the authenticated resource. Supports `?cursor`, `?limit`, and optional
`?order=asc\|desc`; returns `WireListPage<HarnessMessage>` over the stable
`(createdAt, id)` message key.

**`GET` `/harness/:name/operator/threads/:threadId/first-user-message`**

Operator/history helper: return the first user message for a thread after
verifying it belongs to the authenticated resource.

**`POST` `/harness/:name/operator/threads/first-user-messages`**

Operator/history helper: batch first-user-message lookup for
authenticated-resource thread IDs; cross-resource IDs are omitted or returned as
not found without leaking existence.

**`GET` `/harness/:name/operator/background-tasks`**

Operator diagnostic route. List background tasks visible to an operator-selected
scope. Ordinary clients do not get detailed task list/get/stream APIs through
Harness; they observe the bounded `SessionListItem.durableWork` and
`SessionSnapshot.durableWork` projections instead. When an operator narrows the
route to a resource, status/date/order filters apply only inside that
`(harnessName, resourceId)` scope. Any `total` field is best-effort for the
filtered scope and must not be used as a recovery or pagination boundary.
Returned task objects use the redacted §4.8 `BackgroundTask` diagnostic
projection over existing BackgroundTasksStorage rows, not a Harness-owned task
store.

**`GET` `/harness/:name/operator/background-tasks/:taskId`**

Operator diagnostic route. Get one background task after verifying the requested
operator scope and, when a resource scope is provided, the task's §5.1 owner
proof directly or through `ownerRef` to the owning Harness durable row. Missing,
cross-harness, cross-resource, and unscopable rows return tenant-safe `404` for
resource-scoped operators. Returned task objects use the redacted §4.8
`BackgroundTask` diagnostic projection, not the internal storage row.

**`GET` `/harness/:name/operator/background-tasks/events`**

Operator diagnostic live SSE stream of background task events for the selected
operator scope. Both the initial running-task snapshot and each later event are
filtered per event. This is a live diagnostic stream, not the §10.5 replay
contract; operators that miss events reconcile through the operator list/get
routes unless a future durable task-event buffer explicitly upgrades the route.
...
admits through `session.signal(...)` or `session.queue(...)` with
...
fields are never forwarded as `SignalOptions` / `QueueOptions`; `sync`,
...
equivalent inventory for Studio or operator diagnostics, that surface is
operator/admin only: explicit operator authentication, authorization, audit
logging, rate limiting, and deployment-owned pagination or caps are required.
The result is diagnostic inventory, not proof that an ordinary principal may
access any listed resource.
...
the registry boundary before adapter verification or body parsing can select an
alternate path. Provider-owned routes that call into Harness use the same registry
...
Product-level autonomous responders are not a public route and not a
caller-selectable actor. Wire callers cannot submit `responder` receipt metadata,
impersonate an autonomous response through `requestContext`, or answer a
descendant prompt through a parent-owned controller. Human, SDK, channel, and
product responders that race for the same pending question resolve through the
`POST /inbox/:itemId` stale/consumed/conflict rules above.
...
client routes and must not be republished to ordinary Harness clients as
migration wrappers. Public v1 behavior is expressed through the canonical
`/harness/:name/...` routes above. Current `validateThreadOwnership(...)` /
`enforceThreadAccess(...)`, raw `MemoryStorage.getThreadById(...)`,
`deleteThread(...)`, `listMessages(...)`, gateway memory reads, and legacy
channel metadata lookup are implementation material only; retained non-Harness
routes remain legacy/operator/internal surfaces with explicit operator auth, not
a parallel public lifecycle path. Same-resource principal capability failures on
canonical Harness routes still map to `403 harness.forbidden`. Harness-bound
channel callback and `AgentChannels` reuse rules remain governed by §14.7;
metadata lookup is not a substitute for `ChannelBinding` resolution.

For destructive thread delete specifically, the Harness operator thread-delete
route in this section delegates to the §5.5 session-first delete cascade before
calling any raw memory/gateway deletion primitive. Tenant/resource verification
alone is not enough to make a physical delete route a public Harness
thread-delete route.
...
`/background-tasks/stream` routes during migration. Harness v1 ordinary clients
do not get detailed task read/list/stream routes; they use session durable-work
projections. Detailed, unfiltered, cross-resource, or task-ID diagnostics require
operator authorization. Existing flat routes that cannot enforce this contract
are disabled for ordinary clients or exposed only as operator diagnostics under
explicit operator authentication, authorization, audit logging, and rate
limiting; they are not a parallel public bypass around the Harness route
namespace.
...
behavior or downstream-forwarding behavior apply on Harness routes, even when
the lane is implemented by reusing existing auth wrappers.

The only query-parameter auth exception in v1 is an optional scoped-token mode
...
lookup, state, signal, queue, inbox, attachment, and operator routes. It is not
...
state, not authentication. Servers that support this token mode must avoid caching
...
observational-memory snapshots, result lookups, scoped channel diagnostics, and
...
resource, `signal`, `queue`, `useSkill`, attachments, and non-privileged
...
Operation class: Approval bypass: any wire `yolo: true` on `signal`, `queue`,
or `useSkill`.
...
`setThreadSetting`, session rename, and thread rename/delete/clone where
exposed.

Minimum principal rule: Explicit state/metadata write capability. State and
session-title/app-thread-metadata writes use the session `ETag` / `If-Match`
stale-write contract; operator thread title/delete/clone behavior is owned by
the thread routes.
...
`session.signal(...)`, `session.queue(...)`, or `session.useSkill(...)`.
