...
`retryable: true`, and the scope details in section 13.3 before creating new inbox,
...
The eviction policy (section 5.4) applies normally. Sessions that haven't been touched
...
runs. Drain has two parts. First, the server stops accepting new externally
reachable Harness durable ingress for the sections 13.1/13.2 route set that creates,
claims, or dispatches durable work: channel inbound, channel action callbacks,
session signal/queue/inbox writes, attachment uploads, and wakeup-producing
schedule/proactive handoffs. Worker-owned scopes such as channel inbox/action,
channel outbox, wakeups, and reconstructable background tasks may return
`503 harness.worker_unavailable` with `reason: 'server_draining'` and their
worker scope. Ordinary session and attachment routes that are refused only
because this server instance is draining return `503 harness.server_draining`
instead; they must not invent a worker scope. A deployment may also stop
accepting new connections at the listener or load-balancer layer so requests
never reach the instance. In each case callers retry against the next instance
rather than the draining one. Read-only session and diagnostic routes may keep
answering until `mastra.shutdown()` begins storage teardown. Second, already admitted
...
Settled work persists state through the normal section 5 storage path so the next
...
epoch to end and recover through the section 10.5 replay/snapshot path.
...
expire for the next worker. Outbox projection before dispatch follows section 14.4
using the section 5.2 binding-discovery and outbox-claim methods; section 13.6 only owns
