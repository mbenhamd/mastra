...
describe('pubsub subscriber acknowledgement audit', () => {
  ...
  it('drains the PEL for durable-agent abort listeners', async () => {
    ...
    const runtimeBindingId = 'redis-ack-audit-binding';
    const topic = AGENT_CONTROL_TOPIC(runId, runtimeBindingId);

    const listenerPubSub = makePubSub();
    let aborts = 0;
    const unsubscribe = await subscribeToAbortRequests(listenerPubSub, runId, runtimeBindingId, () => {
      aborts++;
    });
