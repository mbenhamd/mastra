...
describe('dynamic workflow route permissions', () => {
  it('requires read access for list and detail routes', () => {
    expect(LIST_DYNAMIC_WORKFLOWS_ROUTE.requiresPermission).toBe('dynamic-workflows:read');
    expect(GET_DYNAMIC_WORKFLOW_ROUTE.requiresPermission).toBe('dynamic-workflows:read');
  });

  it('requires write access for upsert and delete routes', () => {
    expect(UPSERT_DYNAMIC_WORKFLOW_ROUTE.requiresPermission).toBe('dynamic-workflows:write');
    expect(DELETE_DYNAMIC_WORKFLOW_ROUTE.requiresPermission).toBe('dynamic-workflows:write');
  });

  it('exposes only dynamic workflow route paths', () => {
    expect([
      LIST_DYNAMIC_WORKFLOWS_ROUTE.path,
      UPSERT_DYNAMIC_WORKFLOW_ROUTE.path,
      GET_DYNAMIC_WORKFLOW_ROUTE.path,
      DELETE_DYNAMIC_WORKFLOW_ROUTE.path,
    ]).toEqual([
      '/dynamic/workflows',
      '/dynamic/workflows',
      '/dynamic/workflows/:dynamicWorkflowId',
      '/dynamic/workflows/:dynamicWorkflowId',
    ]);
  });
});
