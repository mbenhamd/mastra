...
describe('vNext Workflow Handlers', () => {
  ...
  describe('STREAM_WORKFLOW_ROUTE', () => {
    ...
    it('should reject a terminal run owned by another request-context resource before revealing status', async () => {
      vi.spyOn(mockWorkflow, 'getWorkflowRunById').mockResolvedValue({
        runId: 'test-run-cross-tenant',
        workflowName: 'test-workflow',
        status: 'success',
        resourceId: 'tenant-b',
      } as any);
      ...
      const createRunSpy = vi.spyOn(mockWorkflow, 'createRun');
      const requestContext = new RequestContext();
      requestContext.set(MASTRA_RESOURCE_ID_KEY, 'tenant-a');
