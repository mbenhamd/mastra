...
describe('A2A Handler', () => {
  ...
  describe('getAgentExecutionHandler', () => {
    ...
    it('unregisters resubscribe listeners when the abort signal is triggered', async () => {
      ...
      const taskKey = JSON.stringify(['test-agent', 'task-1']);
      expect(((mockTaskStore as any).listeners.get(taskKey) as Set<unknown> | undefined)?.size).toBe(1);

      abortController.abort();

      await expect(pendingNext).rejects.toMatchObject({ name: 'AbortError' });
      expect(((mockTaskStore as any).listeners.get(taskKey) as Set<unknown> | undefined)?.size).toBe(undefined);
    });
  });
