...
import { MastraServer, type ServerRouteSelector } from './index';
...
function createAdapter(
  opts: { prefix?: string; customRouteAuthConfig?: Map<string, boolean>; routes?: ServerRouteSelector } = {},
) {
...
describe('MastraServer.getFrameworkPublicMatcher', () => {
  describe('built-in public core auth routes', () => {
    ...
    it('does not classify a predicate-excluded built-in route as framework-public', () => {
      const isPublic = createAdapter({
        routes: route => !(route.method === 'GET' && route.path === '/auth/capabilities'),
      }).getFrameworkPublicMatcher();

      expect(isPublic('/api/auth/capabilities', 'GET')).toBe(false);
      expect(isPublic('/api/auth/me', 'GET')).toBe(true);
    });

    it('does not classify a built-in route omitted from an explicit route list as framework-public', () => {
      const routes = SERVER_ROUTES.filter(route => !(route.method === 'GET' && route.path === '/auth/capabilities'));
      const isPublic = createAdapter({ routes }).getFrameworkPublicMatcher();
