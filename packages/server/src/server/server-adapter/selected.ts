...
export type { InferParams, ResponseType, RouteSchemas, ServerContext, ServerRoute, ServerRouteHandler } from './routes';
...
} from './routes/route-builder';
export { deriveAction, derivePermission, extractResource, getEffectivePermission } from './routes/permissions';
 ...
 *
 * Built once at server-adapter registration time from the adapter's selected
 * built-in route registry and the `customRouteAuthConfig` map. Called by each
 * adapter's context middleware to stash a boolean on the per-request context
 * under `MASTRA_FRAMEWORK_PUBLIC_KEY`.
 */
...
export interface MastraServerOptions<TApp> {
  app: TApp;
  mastra: Mastra;
  ...
  /**
   * Built-in server routes to register. Omit to register every route in the
   * adapter's route registry. Array and predicate selectors both preserve the
   * registry's canonical order.
   */
  routes?: ServerRouteSelector;
}

/** Options for the registry-free selected-route adapter entrypoint. */
export interface SelectedMastraServerOptions<TApp> extends MastraServerOptions<TApp> {
  /**
   * Canonical built-in route registry available to this adapter. Import a
   * reviewed per-domain route entrypoint so unrelated route modules never load.
   */
  routeRegistry: readonly ServerRoute[];
}
...
export abstract class MastraServer<TApp, TRequest, TResponse> extends MastraServerBase<TApp> {
  ...
  constructor({
    ...
    routeRegistry,
    routes,
  }: SelectedMastraServerOptions<TApp>) {
    ...
    this.serverRoutes = this.resolveServerRoutes(routeRegistry, routes);
  ...
  private resolveServerRoutes(
    routeRegistry: readonly ServerRoute[],
    routes?: ServerRouteSelector,
  ): readonly ServerRoute[] {
    const builtInsByRoute = new Map<string, ServerRoute>();
    for (const route of routeRegistry) {
      const routeKey = formatRoute(route);
      if (builtInsByRoute.has(routeKey)) {
        throw new Error(`route registry contains duplicate built-in route: ${routeKey}`);
      }
      builtInsByRoute.set(routeKey, route);
    }

    if (!routes) return Object.freeze([...routeRegistry]);
    if (typeof routes === 'function') return Object.freeze(routeRegistry.filter(route => routes(route)));
    ...
    for (const route of routes) {
      ...
      if (selectedRouteKeys.has(routeKey)) {
    ...
    return Object.freeze(routeRegistry.filter(route => selectedRouteKeys.has(formatRoute(route))));
  }
