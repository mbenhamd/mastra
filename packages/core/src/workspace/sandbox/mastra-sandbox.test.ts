...
describe('MastraSandbox Base Class', () => {
  ...
  describe('Lifecycle Methods', () => {
    ...
    it('_stop() joins an in-flight destroy without overwriting the terminal status', async () => {
      const sandbox = new MountableSandbox();
      await sandbox._start();

      await Promise.all([sandbox._destroy(), sandbox._stop()]);

      expect(sandbox.status).toBe('destroyed');
      expect(sandbox.calls.filter(call => call === 'destroy')).toHaveLength(1);
      expect(sandbox.calls).not.toContain('stop');
    });
