...
export abstract class MastraSandbox<THandle = unknown> extends MastraBase implements WorkspaceSandbox {
  ...
  async _stop(): Promise<void> {
    // A completed or in-flight destroy is terminal. Joining the destroy
    // prevents a concurrent stop from overwriting `destroyed` with `stopped`.
    if (this.status === 'destroyed') {
      return;
    }
    ...
    if ((this.status as ProviderStatus) === 'destroyed') return;
    if (this._destroyPromise) return this._destroyPromise;
