...
describe('createScorer', () => {
  ...
  describe('Observability score emission', () => {
    ...
    it('redacts auth and bounds RequestContext values in scorer span input', async () => {
      ...
      const scorerRunInput = (startSpan.mock.calls[0]?.[0] as { input?: Record<string, unknown> }).input;
      expect(scorerRunInput).not.toHaveProperty('requestContext');
    ...
    describe('requestContext persistence on the scorer-run input', () => {
      ...
      it('bounds and redacts nested values selected by the wildcard', async () => {
        const { captured, scorer } = captureScorerRun();
        ...
        await scorer.run({
          ...
          requestContext: {
            userId: 'u1',
            tenant: { apiKey: 'sk-secret', note: 'x'.repeat(20_000) },
          },
          requestContextKeys: ['*'],
        });

        expect(captured.input?.requestContext?.tenant.apiKey).toBe('[REDACTED]');
        expect(captured.input?.requestContext?.tenant.note).toContain('[TRUNCATED]');
        expect(JSON.stringify(captured.input?.requestContext)).not.toContain('sk-secret');
      });
    });
