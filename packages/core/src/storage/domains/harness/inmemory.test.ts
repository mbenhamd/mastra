...
describe('InMemoryHarness admission storage contract', () => {
  ...
  it('reports only the durable unfenced terminal evidence writer as applied', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    const pending = {
      harnessName: 'default',
      sessionId: 'session-1',
    ...
    const competing = await Promise.all([
      storage.writeMessageResultEvidence(completed),
      storage.writeMessageResultEvidence(failed),
    ]);
  ...
  it('settles and reloads completed and failed admitted messages after an adapter restart', async () => {
    ...
    for (const [index, terminal] of terminals.entries()) {
      ...
      const restarted = new InMemoryHarness({ db });
      await expect(
  ...
  it('atomically claims signal dispatch and migrates legacy run evidence', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    const pending = {
      harnessName: 'default',
      sessionId: 'session-1',
    ...
    await expect(
      ...
      status: 'pending',
      runId: 'run-1',
      dispatch: { state: 'dispatching', delivery: 'idle', runId: 'run-1' },
    });
  ...
  it('uses semantic dispatch CAS and fences terminal settlement by attempt', async () => {
    const storage = new InMemoryHarness({ db: new InMemoryDB() });
    const identity = {
      harnessName: 'default',
