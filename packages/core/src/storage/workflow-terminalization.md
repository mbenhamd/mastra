...
`supportsWorkflowTerminalizationJournal()` only answers whether the version 1 journal exists. It does not imply producer outbox, destination receipt, or recovery support. Every capability is negotiated independently, and base adapters return `unsupported` for protocol operations they do not implement. An adapter that advertises `recoveryVersion: 1` must implement `getWorkflowRunTerminalStatus()` from its durable terminal evidence; the runtime uses it to reject a nested start under a terminal parent even when the replaceable parent row has already been deleted.
