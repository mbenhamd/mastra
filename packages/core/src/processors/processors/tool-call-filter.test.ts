...
describe('ToolCallFilter', () => {
  ...
  describe('preserveModelOutput', () => {
    ...
    it('preserves compact model output only for an explicit tool allowlist', async () => {
      const prompt: LanguageModelV2Prompt = [
        ...searchPrompt(),
        { role: 'assistant', content: [toolCallPart('call-private', 'private-file', { path: 'PRIVATE_PATH' })] },
        {
          role: 'tool',
          content: [toolResultPart('call-private', 'private-file', { type: 'text', value: 'PRIVATE_COMPACT_RESULT' })],
        },
      ];

      const result = await runFilter(new ToolCallFilter({ preserveModelOutputFor: ['search'] }), prompt);
    ...
    it('lets an explicit empty model-output allowlist override the legacy preserve-all option', async () => {
      const result = await runFilter(
        new ToolCallFilter({ preserveModelOutput: true, preserveModelOutputFor: [] }),
        searchPrompt(),
      );

      expect(JSON.stringify(result)).not.toContain('Compact search summary');
    });

    it('bounds preserved model output to maxModelOutputBytes', async () => {
      const prompt: LanguageModelV2Prompt = [
        { role: 'user', content: [{ type: 'text', text: 'Search and summarize' }] },
        { role: 'assistant', content: [toolCallPart('call-search', 'search')] },
        {
          role: 'tool',
          content: [toolResultPart('call-search', 'search', { type: 'text', value: 'A'.repeat(500) })],
        },
      ];

      const result = await runFilter(
        new ToolCallFilter({ preserveModelOutput: true, maxModelOutputBytes: 64 }),
        prompt,
      );

      const preserved = textsIn(result).find(text => text.startsWith('search result:\n'));
      expect(preserved).toBeDefined();
      expect(preserved).toContain('[truncated]');
      expect(new TextEncoder().encode(preserved!.replace('search result:\n', '')).byteLength).toBeLessThanOrEqual(64);
    });
    ...
    it('omits media parts from content output instead of serializing them', async () => {
      const prompt: LanguageModelV2Prompt = [
        { role: 'assistant', content: [toolCallPart('call-content', 'screenshotTool')] },
        {
          role: 'tool',
          content: [
            toolResultPart('call-content', 'screenshotTool', {
              type: 'content',
              value: [
                { type: 'text', text: 'visible summary' },
                { type: 'media', data: 'UNSAFE_MEDIA_DATA', mediaType: 'image/png' },
              ],
            }),
          ],
        },
      ...
      expect(textsIn(result)).toContain('screenshotTool result:\nvisible summary');
      expect(JSON.stringify(result)).not.toContain('UNSAFE_MEDIA_DATA');
      expect(JSON.stringify(result)).not.toContain('image/png');
    });

    it('omits unknown output shapes instead of serializing them', async () => {
      const prompt: LanguageModelV2Prompt = [
        { role: 'assistant', content: [toolCallPart('call-unknown', 'providerTool')] },
        {
          role: 'tool',
          content: [
            toolResultPart('call-unknown', 'providerTool', {
              type: 'provider-defined-blob',
              value: { raw: 'UNSAFE_RESULT' },
            }),
          ],
        },
      ];
      ...
      expect(JSON.stringify(result)).not.toContain('UNSAFE_RESULT');
      expect(JSON.stringify(result)).not.toContain('providerTool result');
      expect(result).toEqual([]);
    });

    it('omits json output that hides behind a toJSON hook', async () => {
      const prompt: LanguageModelV2Prompt = [
        { role: 'assistant', content: [toolCallPart('call-hook', 'hookTool')] },
        {
          role: 'tool',
          content: [
            toolResultPart('call-hook', 'hookTool', {
              type: 'json',
              value: { toJSON: () => 'UNSAFE_RESULT' },
            }),
          ],
        },
      ];
    ...
    it('omits json output nested past the traversal depth bound', async () => {
      let deep: Record<string, unknown> = { leaf: 'DEEP_SECRET' };
      for (let depth = 0; depth < 128; depth += 1) {
        deep = { nested: deep };
      }

      const prompt: LanguageModelV2Prompt = [
        { role: 'assistant', content: [toolCallPart('call-deep', 'deepTool')] },
        { role: 'tool', content: [toolResultPart('call-deep', 'deepTool', { type: 'json', value: deep })] },
      ];
