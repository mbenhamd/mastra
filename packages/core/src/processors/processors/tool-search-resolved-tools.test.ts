...
function idlessTool(description: string) {
  return { description, inputSchema: z.object({}), execute: async () => ({}) } as any;
}
...
describe('ToolSearchProcessor includeResolvedTools', () => {
  ...
  it('searches and loads an id-less resolved tool by its record key', async () => {
    const resolvedTool = idlessTool('Fetch a customer invoice');
    const processor = new ToolSearchProcessor({ tools: {}, includeResolvedTools: true });

    const first = await processor.processInputStep(stepArgs({ fetch_invoice: resolvedTool }));
    expect((await search(first, 'invoice')).results.map((r: any) => r.name)).toContain('fetch_invoice');
    expect((await load(first, 'fetch_invoice')).success).toBe(true);

    const second = await processor.processInputStep(stepArgs({ fetch_invoice: resolvedTool }));
    expect(toolsOf(second).fetch_invoice).toBe(resolvedTool);
  });

  it('keeps multiple id-less resolved tools distinct by record key', async () => {
    const resolvedTools = {
      fetch_invoice: idlessTool('Fetch an invoice'),
      send_invoice: idlessTool('Send an invoice'),
    };
    const processor = new ToolSearchProcessor({ tools: {}, includeResolvedTools: true });

    const first = await processor.processInputStep(stepArgs(resolvedTools));
    const names = (await search(first, 'invoice')).results.map((r: any) => r.name);
    expect(names).toEqual(expect.arrayContaining(['fetch_invoice', 'send_invoice']));
    expect((await load(first, 'fetch_invoice')).success).toBe(true);
    expect((await load(first, 'send_invoice')).success).toBe(true);
  });
