...
describe('rehydrate agent/tool step options', () => {
  ...
  it('hard-crashes when an agent step carries a static scorers config (scorers are not serialized)', () => {
    const agent = fixedResponseAgent('a5', 'ok');
    const wf = createWorkflow({
      id: 'bad-static-scorers-wf',
      inputSchema: z.object({ value: z.string() }),
      outputSchema: z.object({ text: z.string() }),
    })
      .agent(agent, { scorers: { relevance: { scorer: 'relevance-scorer' } } as any } as any)
      .commit();

    expect(() => toStorableGraph(wf.stepGraph)).toThrow(/"scorers" does not round-trip/);
  });

  it('hard-crashes when a tool step carries a static scorers config (scorers are not serialized)', () => {
    const wf = createWorkflow({
      id: 'bad-tool-static-scorers-wf',
      inputSchema: z.object({ value: z.string() }),
      outputSchema: z.object({ echoed: z.string() }),
    })
      .tool(echoTool, { scorers: { relevance: { scorer: 'relevance-scorer' } } as any } as any)
      .commit();

    expect(() => toStorableGraph(wf.stepGraph)).toThrow(/"scorers" does not round-trip/);
  });

  it('hard-crashes when a foreach step carries a function concurrency resolver', () => {
    const wf = createWorkflow({
      id: 'bad-foreach-concurrency-wf',
      inputSchema: z.object({ value: z.string() }),
      outputSchema: z.object({ echoed: z.string() }),
    })
      .foreach(echoTool as any, { concurrency: (() => 2) as any })
      .commit();
