...
import type { SerializedSingleStepEntry, SingleStepEntry } from './types';
...
/** The id of a serialized single step-like entry (same keying as {@link getEntryId}). */
export function getSerializedEntryId(entry: SerializedSingleStepEntry): string {
  return entry.type === 'step' ? entry.step.id : entry.id;
}
