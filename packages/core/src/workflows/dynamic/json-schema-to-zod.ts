 ...
 *  - primitive constraints: `minLength` / `maxLength` / `pattern` on strings,
 *    `minimum` / `maximum` / `exclusiveMinimum` / `exclusiveMaximum` /
 *    `multipleOf` on numbers, `minItems` / `maxItems` on arrays — dropping
 *    them would let a rehydrated workflow validate wider than the original
 *
  ...
  if ('const' in schema) {
    ...
    switch (schema.type) {
      ...
      case 'array': {
        ...
        let arr = z.array(walk(schema.items ?? {}, opts));
      ...
      case 'string': {
        ...
        if (typeof schema.pattern === 'string') {
          ...
          try {
            ...
            // An uncompilable pattern must not silently widen to an
            // unconstrained string — treat it like an unsupported keyword.
            return unsupported(
              `Dynamic workflow schema uses an invalid "pattern" (${JSON.stringify(schema.pattern)}): ` +
      ...
      case 'integer': {
        ...
        if (typeof schema.minimum === 'number') {
          // Draft-4 boolean form: `exclusiveMinimum: true` qualifies `minimum`.
          num = schema.exclusiveMinimum === true ? num.gt(schema.minimum) : num.gte(schema.minimum);
        }
        if (typeof schema.exclusiveMaximum === 'number') num = num.lt(schema.exclusiveMaximum);
        if (typeof schema.maximum === 'number') {
          num = schema.exclusiveMaximum === true ? num.lt(schema.maximum) : num.lte(schema.maximum);
        }
