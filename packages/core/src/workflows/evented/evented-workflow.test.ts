  ...
  describe('foreach failure progress (issue #21749)', () => {
    it('does not re-execute successful iterations when time travelling to a failed foreach', async () => {
      ...
      try {
        ...
        const failedSnapshot = await (
          await testStorage.getStore('workflows')
        ).loadWorkflowSnapshot({
          workflowName: workflow.id,
          runId: run.runId,
        });
        const failedForeach = failedSnapshot?.context?.[processItem.id] as any;
        expect(failedSnapshot?.status).toBe('failed');
        expect(failedForeach?.status).toBe('failed');
        expect(failedForeach?.suspendPayload?.__workflow_meta?.foreachOutput).toMatchObject([
    ...
    it('preserves successes across suspend, resume, failure, and time travel', async () => {
      const executions = [0, 0, 0];
      const seed = createStep({
        id: 'evented-sparse-progress-seed',
        inputSchema: z.object({}),
        outputSchema: z.array(z.number()),
        execute: async () => [0, 1, 2],
      });
      const processItem = createStep({
        id: 'evented-sparse-progress-item',
        inputSchema: z.number(),
        outputSchema: z.number(),
        ...
        execute: async ({ inputData, resumeData, suspend }) => {
          executions[inputData]! += 1;
          if (inputData === 1 && resumeData === undefined) {
            return await suspend({ value: inputData });
          }
          if (inputData === 2 && executions[inputData] === 1) {
            throw new Error('transient post-resume failure');
          }
          return inputData;
        },
      });
      const workflow = createWorkflow({
        id: 'evented-sparse-failure-progress',
        inputSchema: z.object({}),
        outputSchema: z.array(z.number()),
        retryConfig: { attempts: 0 },
      });
      ...
      try {
        ...
        expect(executions).toEqual([1, 1, 0]);
        ...
        const recovered = await run.timeTravel({ step: processItem.id });
        expect(recovered.status).toBe('success');
        expect(executions).toEqual([1, 2, 2]);
      } finally {
        await mastra.stopWorkers();
      }
    });

    it('waits for admitted siblings before terminalizing a concurrent foreach failure', async () => {
      const executions = [0, 0, 0];
      let announceSlowStart!: () => void;
      let releaseSlow!: () => void;
      const slowStarted = new Promise<void>(resolve => {
        announceSlowStart = resolve;
      });
      const slowRelease = new Promise<void>(resolve => {
        releaseSlow = resolve;
      });
      const seed = createStep({
        id: 'evented-concurrent-failure-seed',
        inputSchema: z.object({}),
        outputSchema: z.array(z.number()),
        execute: async () => [0, 1, 2],
      });
      const processItem = createStep({
        id: 'evented-concurrent-failure-item',
        inputSchema: z.number(),
        outputSchema: z.number(),
        execute: async ({ inputData }) => {
          executions[inputData]! += 1;
          ...
          if (inputData === 0 && executions[inputData] === 1) {
            await slowStarted;
            throw new Error('fast sibling failure');
          }
          return inputData;
        },
      });
      const workflow = createWorkflow({
        id: 'evented-concurrent-failure-progress',
        inputSchema: z.object({}),
        outputSchema: z.array(z.number()),
        retryConfig: { attempts: 0 },
      });
      ...
      try {
        const run = await workflow.createRun();
        const pending = run.start({ inputData: {} });
        await slowStarted;
        ...
        const recovered = await run.timeTravel({ step: processItem.id });
        expect(recovered.status).toBe('success');
        expect(executions).toEqual([2, 1, 1]);
      } finally {
        releaseSlow();
        await mastra.stopWorkers();
      }
