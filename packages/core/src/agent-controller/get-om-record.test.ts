...
describe('AgentController.getObservationalMemoryRecord', () => {
  ...
  it('restores OM progress from the durable record without scanning messages', async () => {
    ...
    // Seed the durable record through the storage API rather than by mutating the
    // value `initializeObservationalMemory` returned. This fork isolates the memory
    // boundary: reads hand back a frozen deep clone, so assigning onto it would throw
    // and — even unfrozen — would not reach stored state. `insertObservationalMemoryRecord`
    // is the supported write path and stores its own unfrozen clone.
  ...
  it('restores buffered observation counts when chunks are stored serialized', async () => {
    ...
    // Seeded through the storage write path for the same reason as above: this fork
    // returns a frozen deep clone from the memory boundary, so mutating the read value
    // neither succeeds nor reaches storage.
    await memoryStorage.insertObservationalMemoryRecord({
      ...record,
      pendingMessageTokens: 6_000,
