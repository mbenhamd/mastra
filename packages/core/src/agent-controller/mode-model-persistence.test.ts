...
describe('AgentController mode-model persistence across restarts', () => {
  ...
  it.each([true, false])(
    ...
    async hasStorage => {
      ...
      const controller = new AgentController<AgentControllerTestState>({
        ...
        modes: [
          { id: 'plan', name: 'Plan', transitionsTo: 'build', agent: planAgent },
  ...
  it.each(['approve', 'decline', 'abort'] as const)(
    'routes a later tool approval to the plan agent after switching to build (%s)',
    async decision => {
      const execute = vi.fn(async () => 'File updated.');
      let planCalls = 0;
      let buildCalls = 0;
      const planAgent = new Agent({
        id: 'plan-agent',
            ...
            return {
              stream:
                planCalls === 1
                  ...
                  : planCalls === 2
                    ? createToolCallStream({ toolCallId: 'write-call', toolName: 'write_file', input: '{}' })
                    : createTextStream('Plan finished.'),
            };
          },
        }),
        tools: {
          ...
          write_file: createTool({
            id: 'write_file',
            description: 'Update the planned file after approval.',
      ...
      const planApproval = vi.spyOn(planAgent, 'sendToolApproval');
      const buildApproval = vi.spyOn(buildAgent, 'sendToolApproval');
      const controller = new AgentController<AgentControllerTestState>({
        workspace: createMockWorkspace(),
        id: 'test-controller',
        storage,
      ...
      session.subscribe(event => {
        ...
        if (event.toolName === 'write_file' && decision === 'abort') {
          session.abort();
          return;
        }
        session.respondToToolApproval({
          decision: event.toolName === 'submit_plan' ? 'approve' : decision === 'approve' ? 'approve' : 'decline',
          runId: event.runId,
          toolCallId: event.toolCallId,
        });
      });
      await session.sendMessage({ content: 'Create a plan' });
      expect(events).toContainEqual(expect.objectContaining({ type: 'tool_suspended', toolCallId: 'plan-call' }));

      events.length = 0;
      await session.respondToToolSuspension({ toolCallId: 'plan-call', resumeData: { action: 'approved' } });

      expect(events.filter(event => event.type === 'error')).toEqual([]);
      expect(events).toContainEqual({ type: 'agent_end', reason: decision === 'abort' ? 'aborted' : 'complete' });
      expect(planApproval).toHaveBeenCalledWith(
        expect.objectContaining({ toolCallId: 'write-call', approved: decision === 'approve' }),
      );
