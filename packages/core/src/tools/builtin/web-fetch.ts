...
async function requestUrl(
  ...
  return new Promise((resolve, reject) => {
    ...
    const settleResolve: typeof resolve = value => settle(() => resolve(value));
      ...
      response => {
        ...
        void (async () => {
          ...
          if (location && response.statusCode && response.statusCode >= 300 && response.statusCode < 400) {
            ...
            // The redirect hop runs under its own wall-clock deadline.
            clearTimeout(deadline);
            settleResolve(await requestUrl(nextUrl, redirectsRemaining - 1));
            return;
          }
    ...
    // The `timeout` request option above only covers socket *inactivity*: a
    // server dripping one byte per interval keeps the socket busy and never
    // trips it. This wall-clock deadline bounds the whole request — connect,
    // headers, and body — and tears down both directions when it fires.
    deadline = setTimeout(() => {
      const error = new WebFetchError(`Request timed out after ${TIMEOUT_MS}ms.`);
      settleReject(error);
