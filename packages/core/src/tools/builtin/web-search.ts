...
export function normalizeWebSearchProvider(providerOrModel: unknown): WebSearchProvider {
  ...
  // AI SDK model instances report `provider` as `<provider>.<apiFlavor>`
  // (`openai.responses`, `openai.chat`, `anthropic.messages`, ...). Strip the
  // flavor qualifier so those ids resolve to their provider namespace; ids
  // whose namespace is still unsupported (`ollama.responses`,
  // `openai-compatible.chat`, ...) keep failing below.
  const namespaceProvider = getProviderNamespace(routerProvider);
  if (supportedProviders.has(namespaceProvider as WebSearchProvider)) {
    return namespaceProvider as WebSearchProvider;
  }
...
/** `openai.responses` → `openai`: the segment before the first dot names the provider. */
