...
describe('webSearchTool', () => {
  ...
  it('normalizes AI SDK provider-qualified ids to their provider namespace', () => {
    expect(normalizeWebSearchProvider('openai.responses')).toBe('openai');
    expect(normalizeWebSearchProvider('openai.chat')).toBe('openai');
    expect(normalizeWebSearchProvider('anthropic.messages')).toBe('anthropic');
    expect(normalizeWebSearchProvider('xai.chat')).toBe('xai');
    // AI SDK LanguageModel instances surface the qualified id via `provider`.
    expect(normalizeWebSearchProvider({ provider: 'openai.responses', modelId: 'gpt-5-mini' })).toBe('openai');
  });
  ...
  it('throws for unsupported or ambiguous providers', () => {
    ...
    // Qualified ids whose namespace is unsupported must still fail.
    expect(() => normalizeWebSearchProvider('ollama.responses')).toThrow(MastraError);
    expect(() => normalizeWebSearchProvider('cerebras.chat')).toThrow(MastraError);
    expect(() => normalizeWebSearchProvider('openai-compatible.chat')).toThrow(MastraError);
  });
