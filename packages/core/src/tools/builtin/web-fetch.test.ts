...
describe('webFetchTool', () => {
  ...
  it('enforces a wall-clock deadline on slow-drip responses that evade the inactivity timeout', async () => {
    vi.useFakeTimers();
    try {
      allowPublicDns();
      // A response that keeps trickling bytes and never ends: each chunk resets
      // the socket-inactivity timeout, so only a wall-clock deadline can stop it.
      ...
      const request = mockRequest(response);

      const resultPromise = (webFetchTool as any).execute({ url: 'https://example.com/drip' }, {});

      await vi.advanceTimersByTimeAsync(5_000);
      response.push('drip');
      await vi.advanceTimersByTimeAsync(5_000);
      response.push('drip');
      await vi.advanceTimersByTimeAsync(6_000); // crosses the 15s total budget

      // Race against a sentinel so a missing deadline fails fast instead of
      // hanging the test on a never-settling fetch.
      await expect(Promise.race([resultPromise, 'still pending'])).resolves.toEqual({
        content: 'Failed to fetch URL: Request timed out after 15000ms.',
        isError: true,
      });
      expect(request.destroy).toHaveBeenCalled();
      expect(responseDestroy).toHaveBeenCalled();
      // The deadline timer must not leak once the request settles.
  ...
  it('clears the wall-clock deadline timer once a response completes', async () => {
    vi.useFakeTimers();
    try {
      allowPublicDns();
      mockRequest(createResponse(['Hello'], { statusCode: 200 }));

      const result = await (webFetchTool as any).execute({ url: 'https://example.com/fast' }, {});

      expect(result).toMatchObject({ content: 'Hello', ok: true });
      expect(vi.getTimerCount()).toBe(0);
    } finally {
      vi.useRealTimers();
    }
