 ...
 * Wraps child span creation and structured log emission in a null-safe API.
 */
...
export type CoreTool = {
  ...
  /** @internal Present only for object input schemas with an executable Standard Schema validator. */
  approvalInputEditing?: 'object';
...
> extends Partial<ObservabilityContext> {
  ...
  /**
   * Observability helpers for recording child spans and structured logs from
   * inside a tool's execute function. Always provided; when no tracing context
   * is active, `span` runs the function directly and `log` is a no-op.
   */
  observe: ToolObserve;
}
