...
describe('BackgroundTaskManager lifecycle', () => {
  ...
  it('leaves retryable running tasks recoverable across shutdown', async () => {
    ...
    const storage = await manager.getStorage();
    await storage.createTask(makeRunningTask({ startedAt: new Date(Date.now() - 60_001) }));
    const controller = new AbortController();
    manager.activeAbortControllers.set('task-1', controller);

    const recoveringManager = new BackgroundTaskManager({ enabled: true });
    recoveringManager.__registerMastra(mastra);
    recoveringManager.registerStaticExecutor('agent-1:slow-tool', { execute: async () => undefined });
    try {
      await manager.shutdown();
      expect(controller.signal.aborted).toBe(true);
  ...
  it('does not consume a retry when a redelivered dispatch was declined before starting', async () => {
    ...
    const updateTaskIfStatus = vi.fn(
      async (_taskId: string, _expectedStatus: BackgroundTask['status'], updates: Partial<BackgroundTask>) => {
        Object.assign(pendingTask, updates);
        return true;
      },
    );
    ...
    expect(updateTaskIfStatus).toHaveBeenCalledWith(
      'task-1',
      'pending',
      expect.objectContaining({ status: 'running', retryCount: 0 }),
    );
  ...
  it('still counts a crashed attempt when a redelivered dispatch finds the task running', async () => {
    ...
    const updateTaskIfStatus = vi.fn(
      async (_taskId: string, _expectedStatus: BackgroundTask['status'], updates: Partial<BackgroundTask>) => {
        Object.assign(crashedTask, updates);
        return true;
      },
    );
    const workflowRun = { start: vi.fn(async () => ({ status: 'success' })) };
    ...
    expect(updateTaskIfStatus).toHaveBeenCalledWith(
      'task-1',
      'running',
      expect.objectContaining({ status: 'running', retryCount: 1 }),
    );
    expect(ack).toHaveBeenCalled();
    await manager.shutdown();
    await pubsub.close();
