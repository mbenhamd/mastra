...
export interface TaskContext {
  ...
  /** @internal Whether the caller is awaiting this task's authoritative result. */
  awaited?: boolean;
...
export type BackgroundTaskResumeOptions = {
  /** Wait until the suspended-to-running admission is accepted by a worker. */
  waitForAdmission?: boolean;
  /** Abort while waiting for admission. */
  abortSignal?: AbortSignal;
};
...
export interface BackgroundTaskHandle {
  ...
  /** Resume the task */
  resume(resumeData?: unknown, options?: BackgroundTaskResumeOptions): Promise<BackgroundTask>;
  /** Restarts a task */
  ...
  waitForCompletion(options?: {
    ...
    abortSignal?: AbortSignal;
    /** Resolve when the task suspends, instead of waiting for a terminal state. */
