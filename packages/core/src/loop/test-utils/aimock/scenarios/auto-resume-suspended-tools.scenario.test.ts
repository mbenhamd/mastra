...
describeForAllEngines(
  ...
  engine => {
    ...
    it('auto-resumes a suspended tool when the user sends a follow-up message on the same thread', async () => {
      ...
      // The resume must carry the suspended call's identity coordinates: a re-emitted
      // args resume without them cannot be matched to the persisted suspension.
      const suspendedChunk = suspendedChunks[0] as Extract<
        (typeof suspendedChunks)[number],
        { type: 'tool-call-suspended' }
      >;
      const suspendedRunId = suspendedChunk.runId;
      const suspendedToolCallId = suspendedChunk.payload.toolCallId;
