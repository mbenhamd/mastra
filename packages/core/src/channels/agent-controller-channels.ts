...
export class AgentControllerChannels extends AgentChannels {
  ...
  private async respondToSessionApproval({
    ...
    if (
      ...
      session.approval.getRunId() !== runId
    ) {
      this.log(
        'info',
        `Ignoring stale tool ${decision === 'approve' ? 'approval' : 'denial'} action (no matching parked approval for runId=${runId}, toolCallId=${toolCallId})`,
      );
    ...
    session.respondToToolApproval({ decision, runId, toolCallId, requestContext });
  }
