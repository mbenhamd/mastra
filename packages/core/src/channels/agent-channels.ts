...
export class AgentChannels {
  ...
  /**
   * Tear down this AgentChannels instance. Safe to call multiple times —
   * useful for tests and for graceful shutdown of long-lived processes.
   */
  close(): void {
  ...
  private async processChatMessage(chatThread: Thread, message: Message, mastra: Mastra): Promise<void> {
    ...
    // Typed to the signal-contents shape (text/file parts only): image/* rides
    // as file parts (the LLM adapter converts), matching AgentSignalContents.
    ...
    // Upstream signals API (#merge): `AgentSignalContents` is now a bare
    // string | parts array, and `providerOptions` rides on the MESSAGE INPUT
