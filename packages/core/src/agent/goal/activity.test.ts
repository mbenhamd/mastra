...
import { clearObjective, GOAL_STATE_TYPE, writeObjective } from './objective';
...
async function readDuration(store: ThreadStateStorage | undefined, threadId: string) {
  const state = await store!.getState<GoalObjectiveRecord>({ resourceId, threadId, type: GOAL_STATE_TYPE });
...
describe('goal activity tracking', () => {
  it('checkpoints execution time and exposes it to display consumers with a stale record', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'run-1', now: () => now });
    now += 500;
    expect(
  ...
  it('excludes repeated approval waits while counting each resumed segment', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'approval-run', now: () => now });
    now = 100;
  ...
  it('serializes concurrent segment checkpoints for the same objective', async () => {
    ...
    await Promise.all([
      beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'run-a', now: () => 0 }),
      beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'run-b', now: () => 0 }),
    ]);
    await Promise.all([
  ...
  it('does not attach elapsed time to a replacement objective', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'replaced-run', now: () => 0 });
  ...
  it('does not resurrect an objective cleared between segment start and checkpoint write', async () => {
    const { mastra, store, threadId } = await setup(objective({ activeDurationMs: 0 }));

    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'cleared-run', now: () => 0 });

    // A concurrent objective clear commits just before the checkpoint write
    // reaches the store's mutation lane. The stale segment must not write the
    // old record back.
    const originalMutateState = store.mutateState.bind(store);
    vi.spyOn(store, 'mutateState').mockImplementationOnce(async (args: any) => {
      await clearObjective(store, resourceId, threadId);
      return originalMutateState(args);
    });

    await stopGoalActivity({ agentId, runId: 'cleared-run', now: () => 500 });

    expect(await store.getState({ resourceId, threadId, type: GOAL_STATE_TYPE })).toBeUndefined();
    // The skipped write must not surface as checkpointed display time either.
    expect(
      getGoalActivityDurationMs({
        agentId,
        resourceId,
  ...
  it('does not overwrite an objective replaced between segment start and checkpoint write', async () => {
    const { mastra, store, threadId } = await setup(objective({ activeDurationMs: 0 }));
    const replacement = objective({ id: 'objective-2', objective: 'A different goal', activeDurationMs: 40 });

    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'raced-run', now: () => 0 });

    const originalMutateState = store.mutateState.bind(store);
    vi.spyOn(store, 'mutateState').mockImplementationOnce(async (args: any) => {
      await writeObjective(store, resourceId, threadId, replacement);
      return originalMutateState(args);
    });

    await stopGoalActivity({ agentId, runId: 'raced-run', now: () => 500 });

    const state = await store.getState<GoalObjectiveRecord>({ resourceId, threadId, type: GOAL_STATE_TYPE });
    expect(state?.id).toBe('objective-2');
    expect(state?.activeDurationMs).toBe(40);
  });
  ...
  it('logs and ignores failures while reading the objective at activity start', async () => {
    ...
    await expect(
      beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'failed-start' }),
  ...
  it('logs and ignores failures while checkpointing activity', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'failed-checkpoint', now: () => 0 });
  ...
  it('does not fail activity handling when the logger throws', async () => {
    ...
    await expect(
      beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'failed-logger' }),
    ).resolves.toBeUndefined();
  });
  ...
  it('does not cache an objective store miss', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'miss-run', requestContext });
  ...
  it('does not start activity for a paused objective', async () => {
    ...
    await beginGoalActivity({ mastra, agentId, resourceId, threadId, runId: 'paused-run', now: () => 0 });
