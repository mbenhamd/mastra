 ...
 */
function snapshotAgentExecutionValueInternal<T>(value: T, seen: WeakMap<object, unknown>, cloneOpaqueRoot = false): T {
  ...
  const copyOwnProperties = (target: object, excludedKeys = new Set<PropertyKey>()) => {
    for (const key of Reflect.ownKeys(value)) {
      ...
      const snapshotDescriptor: PropertyDescriptor =
        'value' in descriptor
          ? { ...descriptor, value: snapshotAgentExecutionValueInternal(descriptor.value, seen) }
          : {
              configurable: descriptor.configurable,
              enumerable: descriptor.enumerable,
              writable: descriptor.set !== undefined,
              value: snapshotAgentExecutionValueInternal(Reflect.get(value, key), seen),
            };
  ...
  if (value instanceof RequestContext) {
    ...
    for (const [key, entryValue] of value.entries()) {
      clone.set(key, snapshotAgentExecutionValueInternal(entryValue, seen));
    }
  ...
  if (value instanceof Map) {
    ...
    for (const [key, entryValue] of value) {
      clone.set(snapshotAgentExecutionValueInternal(key, seen), snapshotAgentExecutionValueInternal(entryValue, seen));
    }
    copyOwnProperties(clone);
    return clone as T;
  }
  if (value instanceof Set) {
    const clone = new Set();
    seen.set(value, clone);
    for (const entryValue of value) clone.add(snapshotAgentExecutionValueInternal(entryValue, seen));
    copyOwnProperties(clone);
    return clone as T;
  }
...
export function snapshotAgentExecutionOptions<T>(value: T, inheritedFields: readonly PropertyKey[] = []): T {
  ...
  const clone = snapshotAgentExecutionValueInternal(value, seen, true);
  ...
  for (const key of inheritedFields) {
    ...
    Object.defineProperty(clone as object, key, {
      ...
      enumerable: true,
      writable: true,
      value: snapshotAgentExecutionValueInternal(Reflect.get(value as object, key), seen),
    });
  }
