...
describe('registerRun thread lease', () => {
  it('acquires the thread lease for a plain thread-bound run and releases it on completion', async () => {
    ...
    // Fork contract: registerRun returns the run's completion watcher (the
    // queue-drain gate), not the registration barrier — poll for the lease
    // appearing instead of awaiting the returned promise. A plain (non-signal)
    // run must own the cross-process thread lease once registration settles —
    // otherwise remote liveness checks treat it as a ghost and contending
    // instances start competing runs. The fork holds the lease under a
    // process-attempt-unique owner token that embeds the public run id.
    await waitForCondition(() => pubsub.owners.get(key) !== undefined);
    expect(pubsub.owners.get(key)).toContain(runId);
