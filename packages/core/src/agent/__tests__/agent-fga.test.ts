...
describe('DurableAgent FGA checks', () => {
  ...
  it('resume() uses only the current call actor for authorization and workflow execution', async () => {
    ...
    try {
      // Fork contract: preparation itself enforces agents:execute fail-closed,
      // so prepare() already authorized the initial actor once. Each resume()
      // below then authorizes exactly once with its own per-call actor.
      ...
      // Fork contract: public execution options are boundary-snapshotted, so
      // the workflow receives this call's actor by value, not by reference.
      ...
      expect(requireActor).toHaveBeenCalledTimes(3);
