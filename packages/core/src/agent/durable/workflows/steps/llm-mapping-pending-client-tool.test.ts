...
const RUNTIME_BINDING_ID = 'runtime-binding-1';
...
async function runMappingStep(
  ...
  const output = await (step as any).execute({
    ...
    getInitData: () => ({ runtimeBindingId: RUNTIME_BINDING_ID }),
  });
...
describe('issue #23295 (durable engine): mapping step stops at a client-executed tool call', () => {
  ...
  it('records a serialized server tool result when execute returns undefined', async () => {
    ...
    const calls: SeededCall[] = [{ toolCallId: SERVER_CALL_ID, toolName: SERVER_TOOL, args: SERVER_ARGS }];
    const toolCallStep = createDurableToolCallStep();
    const dispatcherOutput = await (toolCallStep as any).execute({
      inputData: { toolCallId: SERVER_CALL_ID, toolName: SERVER_TOOL, args: SERVER_ARGS },
      mastra: undefined,
      suspend: vi.fn(),
    ...
    const validated = await (toolCallStep as any).outputSchema['~standard'].validate(dispatcherOutput);
    expect(validated.issues).toBeUndefined();
    const serializedOutput = JSON.parse(JSON.stringify(validated.value));
    expect(serializedOutput.serverExecuted).toBe(true);

    // The mapping step must use the serialized outcome, rather than re-resolving a local tool.
    globalRunRegistry.delete(RUN_ID);
    const { output, stored } = await runMappingStep([serializedOutput], calls);
