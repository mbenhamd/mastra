 ...
 *
 * The approval context handed to `toolApprovalRequirement` (and from there to a
...
const toolApprovalRequirement = vi.mocked(resolveRuntime.toolApprovalRequirement);
...
function approvalContextArg() {
  expect(toolApprovalRequirement).toHaveBeenCalledTimes(1);
  return toolApprovalRequirement.mock.calls[0]![3] as { requestContext?: Record<string, unknown> };
}
...
describe('durable tool-call approval request context', () => {
  ...
  it('prefers the live registry request context over the snapshot when present', async () => {
    ...
    const runtimeBindingId = 'approval-ctx-live-binding';
