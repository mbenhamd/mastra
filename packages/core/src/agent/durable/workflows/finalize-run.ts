...
import { assertTerminalToolResultRetained } from '../stream-adapter';
import type { DurableAgenticWorkflowInput, RunRegistryEntry } from '../types';
import { createDurableRuntimeRequestContext, resolveRuntimeDependencies } from '../utils/resolve-runtime';
...
export interface DurableFinishSideEffectsOptions {
  ...
  /** Canonical terminal envelope, established before processors or persistence. */
  terminalEnvelope?: { id: string; data: any };
  /** Assistant message that owns `terminalEnvelope`. */
  messageId?: string;
}
...
export async function runDurableFinishSideEffects({
  ...
  let registryEntry = getBoundRunRegistryEntry(runId, initData.runtimeBindingId);
  ...
  if ((needsProcessorRebuild || needsMemoryRebuild || !registryEntry || registryEntry.isPlaceholder) && mastra) {
    const resolved = await resolveRuntimeDependencies({
      mastra,
      runId,
      agentId: initData.agentId,
      input: { ...initData, messageListState },
      requestContext,
      logger: effectiveLogger,
    });
    rebuiltSaveQueueManager = resolved.saveQueueManager;
    rebuiltMemory = resolved.memory;
    registryEntry = getBoundRunRegistryEntry(runId, initData.runtimeBindingId);
    if (!registryEntry) {
      registryEntry = {
        runtimeBindingId: initData.runtimeBindingId,
        tools: resolved.tools,
        model: resolved.model,
        messageList: resolved.messageList,
        memory: resolved.memory,
        saveQueueManager: resolved.saveQueueManager,
        inputProcessors: resolved.inputProcessors,
        outputProcessors: resolved.outputProcessors,
        errorProcessors: resolved.errorProcessors,
        processorStates: resolved.processorStates,
      } as RunRegistryEntry;
    }
  }

  const effectiveRequestContext = createDurableRuntimeRequestContext({
  ...
  if (terminalEnvelope) {
    if (!messageId) throw new Error(`[DurableAgent] A terminal envelope for run "${runId}" requires a message id.`);
    messageList.add(
      {
        id: messageId,
        role: 'assistant',
        content: { format: 2, parts: [{ type: 'data-terminal-tool-result', ...terminalEnvelope }] },
        createdAt: new Date(),
      } as any,
      'response',
    );
  ...
  if (terminalEnvelope && messageId) {
    assertTerminalToolResultRetained(messageList, messageId, terminalEnvelope);
  }
  ...
  if (terminalMemoryPersistenceExpected && !terminalPersistenceOwned && !memory) {
    throw new Error(
      `[DurableAgent] Cannot deliver terminal result for run "${runId}" because configured memory could not be resolved.`,
    );
  }
  if (terminalMemoryPersistenceExpected && durableState.observationalMemory && !terminalPersistenceOwned) {
    throw new Error(
      `[DurableAgent] Cannot deliver terminal result for run "${runId}" because observational-memory persistence could not be resolved.`,
    );
  }
  if (terminalMemoryPersistenceExpected && !terminalPersistenceOwned && !saveQueueManager) {
    throw new Error(
      `[DurableAgent] Cannot deliver terminal result for run "${runId}" because the memory save queue could not be resolved.`,
    );
  }
  ...
  if (
    ...
    try {
      ...
      await (terminalEnvelope
        ? saveQueueManager.flushMessagesStrict(messageList, durableState.threadId, durableState.memoryConfig)
        : saveQueueManager.flushMessages(messageList, durableState.threadId, durableState.memoryConfig));
    } catch (error) {
