...
describe('durable agent requestContextEntries across iterations', () => {
  ...
  describe('LLM step input', () => {
    ...
    it('carries the request context snapshot on every iteration', async () => {
      ...
      // The fork persists only request-context keys the agent explicitly
      // declares durable (`snapshotDurableRequestContextEntries`), so the key
      // under test has to be declared for the snapshot to exist at all.
      const durableAgent = createDurableAgent({ agent: baseAgent, pubsub, durableRequestContextKeys: ['tenantId'] });
      new Mastra({
