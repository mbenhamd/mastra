...
describe('DurableAgent tool approval', () => {
  ...
  describe('streaming with tool approval', () => {
    it('checkpoints active goal duration before publishing a durable approval wait', async () => {
      ...
      // Fork contract: objectives are keyed by (resourceId, threadId).
      let durationAtApproval = 0;
      await vi.waitFor(async () => {
        durationAtApproval =
          (await durableAgent.getObjective({ resourceId: memory.resource, threadId: memory.thread }))
            ?.activeDurationMs ?? 0;
      ...
      await vi.waitFor(async () => {
        expect(
          (await durableAgent.getObjective({ resourceId: memory.resource, threadId: memory.thread }))?.activeDurationMs,
        ).toBeGreaterThanOrEqual(durationAtApproval);
      });
    });
