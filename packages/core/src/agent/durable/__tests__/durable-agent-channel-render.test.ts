...
describe('a run on a channel-backed thread posts its answer to the channel', () => {
  ...
  /**
   * Processor instances and state are process-local. The snapshot records only
   * that processors existed, so a fresh process cannot prove that its channel
   * renderer is the exact continuation of the suspended run.
   */
  it('fails closed when a fresh-process resume cannot reconstruct processor state', async () => {
    const storage = new InMemoryStore();
    ...
    expect(secondTenantIds).toEqual([]);
    expect(second.postMessage, 'an unverified processor continuation must not post').not.toHaveBeenCalled();
  }, 30000);
});
