...
describe('DurableAgent.recover(runId)', () => {
  ...
  it('does not let an older cleanup remove a newer recovery of the same run', async () => {
    ...
    // A run whose runtime dependencies are still registered may not be
    // re-registered: recovery never replaces another live execution's binding.
    await expect(agent.recover(runId)).rejects.toMatchObject({ id: 'DURABLE_AGENT_RUN_ID_CONFLICT' });

    // Free the run ID without going through `firstRecovery.cleanup()`, which
    // would latch that handle's `cleanedUp` flag and make the assertion below
    // vacuous. This models the real race the guard exists for: the first
    // recovery's registration is dropped (TTL eviction / terminal cleanup)
    // while the caller still holds a live handle to it.
    ...
    // The stale handle must not tear down the newer recovery it now aliases.
    // Both recoveries share the persisted `runtimeBindingId`, so only the
    // entry-identity check in `cleanupOwnedRegistryState` separates them.
    firstRecovery.cleanup();
