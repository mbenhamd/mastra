 ...
 * cleanup must clear that topic itself (alongside the agent stream and remote
 * control topics), otherwise a persistent CachingPubSub orphans retained keys.
 */
...
describe('DurableAgent workflow.events.v2 topic cleanup (#20786)', () => {
  it('clears workflow.events.v2.<runId> when cleanup() runs', async () => {
    ...
    expect(clearTopic).toHaveBeenCalledWith(AGENT_CONTROL_TOPIC(runId, runtimeBindingId!));
