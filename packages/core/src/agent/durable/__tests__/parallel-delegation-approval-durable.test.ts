...
// FORK DIVERGENCE (PF-2588): the fork's hardened DurableAgent.listSuspendedRuns
// override fail-closes in #durableSnapshotPairIsConsistent on the parallel-
// sibling suspension pair this upstream #20529 scenario produces, so suspended
// parallel delegated approvals are not discoverable here. Whether the check
// should learn this shape (like the terminal-recovery fingerprint learned the
// #20471 entry shapes) or the scenario genuinely violates the pair-consistency
// invariant is a fork-owner decision tracked in PF-2588 — not a test change.
describe.skip('durable parallel delegation approvals', () => {
