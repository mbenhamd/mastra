...
// FORK DIVERGENCE (PF-2040): the fork's `enforceReconstructibleToolSurface`
// fence rejects executable tools ADDED by input processors on durable runs, so
// ToolSearchProcessor meta-tools stay fail-closed here even after upstream's
// registry write-back (#19578) — which this fork adopted for fence-approved
// tools. `durable-agent-tool-hooks.test.ts` pins the rejection contract.
// Unskipping requires a sanctioned framework-processor lane through the fence
// (tracked as PF-2586), not a test change.
describe.skip('DurableAgent ToolSearchProcessor meta-tool resolution (#19571)', () => {
