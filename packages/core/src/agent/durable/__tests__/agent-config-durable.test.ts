...
describe('AgentConfig.durable', () => {
  ...
  it('shares one standalone DurableAgent wrapper across concurrent first calls', async () => {
    const { DurableAgent } = await import('../durable-agent');
    const agent = new Agent({
      id: 'concurrent-stream-a',
      name: 'ConcurrentStreamA',
      instructions: 'test',
      model: makeMockModel(),
      durable: true,
    });
    ...
    try {
      const results = await Promise.all([
        agent.stream('first') as unknown as Promise<string>,
        agent.stream('second') as unknown as Promise<string>,
      ]);
      expect(results).toEqual(['durable-result', 'durable-result']);
