...
describe('durable/evented agent — run-local topics are never written to the cache (#21668)', () => {
  ...
  describe('when mastra.pubsub is already a CachingPubSub (reused, not double-wrapped — #18148)', () => {
    ...
    function makeReusedAgent(id: string) {
      ...
      return { agent, mastra, writes };
    }

    it('does not write workflow.events.v2.* events to the cache', async () => {
      const { agent, mastra, writes } = makeReusedAgent('reused-run-local-agent');
      const runId = 'run-4';
      const topic = `workflow.events.v2.${runId}`;
      const received: Event[] = [];
      const workflow = agent.getDurableWorkflows()[0]!;
      const generation = mastra.__registerInternalWorkflow(workflow, runId);

      try {
        await agent.pubsub.subscribe(topic, event => {
          received.push(event);
        });
        await agent.pubsub.publish(topic, { type: 'watch', runId, data: { big: 'payload' } });
        await new Promise(resolve => setTimeout(resolve, 10));
        ...
        mastra.__unregisterInternalWorkflow(workflow.id, runId, generation);
      }
    });
