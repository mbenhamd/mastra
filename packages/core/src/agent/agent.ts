  ...
  implements SubAgent<TAgentId, TRequestContext>
{
  ...
  async getToolsForExecution(options: {
    ...
    // Public callers do not have an enclosing preparation boundary to hand us
    // a model snapshot. Resolve it once here so every tool source observes the
    // same dynamic selection and workspace/tool fan-out cannot invoke a model
    // factory once per converted tool.
    const executionModel = resolvedModel ?? (await this.getModel({ requestContext }));
  ...
  async listSuspendedRuns(options: AgentListSuspendedRunsOptions = {}): Promise<AgentListSuspendedRunsResult> {
    ...
    // Suspended durable runs are discovered through the DurableAgent
    // listSuspendedRuns override (which queries the durable workflow names with
    // snapshot-pair consistency checks), so the standard lane performs exactly
    // one resource-scoped discovery query.
