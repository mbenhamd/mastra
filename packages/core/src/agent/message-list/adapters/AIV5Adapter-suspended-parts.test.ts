...
describe('AIV5Adapter — suspended tool state rehydration', () => {
  ...
  it('downgrades an output-denied legacy toolInvocations entry when structured parts are absent', () => {
    const dbMessage: MastraDBMessage = {
      id: 'msg-denied-legacy',
      role: 'assistant',
      createdAt: new Date('2024-01-01'),
      content: {
        format: 2,
        toolInvocations: [
          {
            toolCallId: 'tc-denied-legacy',
            toolName: 'find-user',
            args: { name: 'Dero Israel' },
            state: 'output-denied',
            approval: { id: 'tc-denied-legacy', approved: false, reason: 'Legacy denial reason' },
          },
        ],
      },
    };
