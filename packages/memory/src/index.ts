...
function assertWorkingMemoryStorageDataWithinLimit(data: string, maxDataBytes?: number): void {
  if (maxDataBytes === undefined) return;
  if (!Number.isSafeInteger(maxDataBytes) || maxDataBytes <= 0) {
    throw new Error('WorkingMemory maxDataBytes must be a positive safe integer.');
  }
  if (new TextEncoder().encode(data).byteLength > maxDataBytes) {
    throw new Error('Working-memory value exceeds the configured UTF-8 byte limit.');
  }
}

// Local copy for compatibility with core versions that predate this export.
...
export class Memory extends MastraMemory {
  ...
  private async deleteStoredThread(memoryStore: MemoryStorage, threadId: string, resourceId?: string): Promise<void> {
    ...
    // Adapters without atomic retraction can delete messages before rejecting
    // the thread-row delete, so retain enough pre-attempt state to reconcile
    // only when that cascade actually committed.
  ...
  private async writeObserverWorkingMemory({
    ...
    memoryStore: MemoryStorage;
    coordinates: WorkingMemorySnapshotInput;
    ...
    if (coordinates.scope === 'resource') {
      if (observationalMemoryGuard) {
        await updateResourceFromObservationalMemory(memoryStore, {
          resourceId: coordinates.resourceId,
          workingMemory,
          guard: observationalMemoryGuard,
        });
      } else {
        await memoryStore.updateResource({ resourceId: coordinates.resourceId, workingMemory });
      }
      return;
    }

    const thread = await this.getThreadById({ threadId: coordinates.threadId });
    if (!thread || thread.resourceId !== coordinates.resourceId) {
      throw new Error('Working-memory thread was not found in the requested resource.');
    }
    const update = {
  ...
  async updateWorkingMemory({
    ...
    const maxDataBytes = config.workingMemory.maxDataBytes;
    ...
    try {
      ...
      const mutexKey =
        coordinates.scope === 'resource' ? `resource-${coordinates.resourceId}` : `thread-${coordinates.threadId}`;
      await this.withWorkingMemoryMutex(mutexKey, async () => {
        ...
        await this.writeObserverWorkingMemory({
          ...
          workingMemory,
          ...(maxDataBytes !== undefined ? { maxDataBytes } : {}),
          ...(guard ? { observationalMemoryGuard: guard } : {}),
        });
      });
   ...
   * Metadata writers acquire this lock before their thread and resource locks.
...
- The user will not see the working memory data directly`;
  ...
  public override async copyThread(
    ...
    try {
      ...
      // Clone OM before post-copy work so a failure can roll the cloned thread
      // and its remapped OM back without leaving a partially copied memory state.
      ...
      // Read and embed copied rows in bounded batches before copying Working
      // Memory. A failed embedding must not alter the destination's memory.
      ...
      if (memoryStore.supportsAtomicThreadCloneRollback && result.rollbackReceipt) {
        try {
          ...
          } else if (rollback.status === 'rolled_back') {
      ...
      if (rollbackSucceeded && this.vector && this.embedder && config.semanticRecall && result.rollbackReceipt) {
        try {
          const messageIndexes = await this.getMemoryVectorIndexes([this.messageIndexPrefix]);
          const clonedMessageIds = result.rollbackReceipt.clonedMessageIds;
          await Promise.all(
            messageIndexes.map(async indexName => {
              for (let i = 0; i < clonedMessageIds.length; i += VECTOR_DELETE_BATCH_SIZE) {
                const batch = clonedMessageIds.slice(i, i + VECTOR_DELETE_BATCH_SIZE);
                if (batch.length === 0) continue;
                try {
                  await this.vector!.deleteVectors({
                    indexName,
                    filter: { message_id: { $in: batch }, thread_id: result.thread.id },
                  });
                } catch {
          ...
          this.logger.error('Failed to rollback copied thread vectors after embedding failure', rollbackError);
        }
      }
      throw error;
    }
  ...
  private requireCloneSourceResourceId(result: StorageCopyThreadOutput): string {
    if (typeof result.sourceResourceId !== 'string' || result.sourceResourceId.length === 0) {
      throw new Error(
        'Storage adapter advertised atomic clone source ownership snapshots but copyThread did not return sourceResourceId.',
      );
    }
