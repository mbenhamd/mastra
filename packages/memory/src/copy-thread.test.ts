...
describe('Memory.copyThread / cloneThread', () => {
  ...
  it('rolls back the copied thread and its vectors when a later embedding batch fails', async () => {
    ...
    const memoryStore = (await memory.storage.getStore('memory'))!;
    const rollbackThreadClone = vi.spyOn(memoryStore, 'rollbackThreadClone');

    await expect(memory.copyThread({ sourceThreadId: 'src-rollback', newThreadId: 'dest-rollback' })).rejects.toThrow(
      'vector store down',
    );

    expect(rollbackThreadClone).toHaveBeenCalledTimes(1);
    expect(rollbackThreadClone).toHaveBeenCalledWith({
      thread: expect.objectContaining({
        threadId: 'dest-rollback',
        storageGeneration: expect.any(String),
        clonedMessageIds: expect.any(Array),
      }),
    });
    const rollbackReceipt = rollbackThreadClone.mock.calls[0]![0].thread;
    expect(rollbackReceipt.clonedMessageIds).toHaveLength(150);
    ...
    expect(deleteVectors).toHaveBeenNthCalledWith(1, {
      indexName: expect.stringMatching(/^memory_messages/),
      filter: {
        message_id: { $in: rollbackReceipt.clonedMessageIds.slice(0, 100) },
        thread_id: 'dest-rollback',
      },
    });
    expect(deleteVectors).toHaveBeenNthCalledWith(2, {
      indexName: expect.stringMatching(/^memory_messages/),
      filter: {
        message_id: { $in: rollbackReceipt.clonedMessageIds.slice(100, 150) },
        thread_id: 'dest-rollback',
      },
    });
  ...
  it('preserves a concurrent destination message when conditional rollback detects a write conflict', async () => {
    const { upsert } = setupSemanticRecallMemory();
    await seedThread('src-concurrent-rollback', 150);
    upsert.mockClear();
    const memoryStore = (await memory.storage.getStore('memory'))!;
    const rollbackThreadClone = vi.spyOn(memoryStore, 'rollbackThreadClone');
    const deleteVectors = vi.mocked(memory.vector!.deleteVectors);

    upsert.mockResolvedValueOnce(undefined).mockImplementationOnce(async () => {
      await memoryStore.saveMessages({
        messages: [
          {
            id: 'concurrent-destination-message',
            threadId: 'dest-concurrent-rollback',
            resourceId,
            role: 'user',
            content: { format: 2, parts: [{ type: 'text', text: 'Concurrent write' }] },
    ...
    await expect(
      memory.copyThread({ sourceThreadId: 'src-concurrent-rollback', newThreadId: 'dest-concurrent-rollback' }),
    ).rejects.toThrow('vector store down');

    expect(upsert).toHaveBeenCalledTimes(2);
    expect(rollbackThreadClone).toHaveBeenCalledTimes(1);
    await expect(memoryStore.getThreadById({ threadId: 'dest-concurrent-rollback' })).resolves.not.toBeNull();
    const { messages } = await memoryStore.listMessages({ threadId: 'dest-concurrent-rollback', perPage: false });
    expect(messages).toHaveLength(151);
    expect(messages.some(message => message.id === 'concurrent-destination-message')).toBe(true);
  ...
  it('embeds every copied message exactly once for adapters that return no messageIdMap', async () => {
    ...
    upsert.mockClear();
    const memoryStore = (await memory.storage.getStore('memory'))!;
    // A custom adapter that omits the messageIdMap while retaining the native receipt.
    const originalCopy = memoryStore.copyThread.bind(memoryStore);
    vi.spyOn(memoryStore, 'copyThread').mockImplementation(async args => {
      const { messageIdMap: _messageIdMap, ...copyWithoutMessageIdMap } = await originalCopy(args);
      return copyWithoutMessageIdMap;
    });
    makeTieOrderUnstable(memoryStore);
