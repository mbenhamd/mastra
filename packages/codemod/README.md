...
Each codemod invocation has a five-minute execution limit. When that limit is reached, the CLI terminates the codemod process tree and exits with a timeout error instead of leaving jscodeshift workers running in the background.
