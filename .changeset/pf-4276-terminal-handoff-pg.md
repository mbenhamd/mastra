...
- Session incarnations are minted and preserved whenever terminal handoff is enabled, even without the session-record projection, and a session row stored before incarnations existed is repaired with a persisted incarnation on first load or update so the feature can be adopted without recreating the session.
