...
Abort intent now travels over pubsub. The executing process picks the request up and flips its own controller, so the run unwinds the same way an in-process abort does and still emits its terminal stream event — consumers waiting on the stream are released instead of hanging. Retained aborts are scoped to the run's immutable runtime binding, so reusing a run ID cannot replay or clear another execution's control signal. Terminal Inngest operations use retry-visible retained-topic cleanup rather than committing after a transient deletion failure. `abort()` now returns a promise so callers can await dispatch; ignoring it preserves the previous fire-and-forget behavior.
