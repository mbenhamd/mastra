...
import { InMemoryChannelsStorage, InMemoryStore } from '@mastra/core/storage';
...
describe('TelegramProvider.connect', () => {
  ...
  it('awaits the replaced AgentChannels close before setChannels', async () => {
    ...
    const existingChannels = new AgentChannels({ adapters: {}, userName: 'agent-1' });
    const closeSpy = vi.spyOn(existingChannels, 'close').mockImplementation(async () => {
      markCloseStarted();
      await closeFinished;
    });
    agent.setChannels(existingChannels);
    const setChannelsSpy = vi.spyOn(agent, 'setChannels');

    stubActiveConnect(BOT_TOKEN);
    const connecting = provider.connect('agent-1', { botToken: BOT_TOKEN });
    await closeStarted;
