...
- Improved Slack channel UX: ([#16937](https://github.com/mastra-ai/mastra/pull/16937))
  ...
  - **Deprecation (not removed)** — the `cards: boolean` and `formatToolCall` fields on `SlackProviderConfig`/`SlackAdapterChannelConfig` are now `@deprecated` but still accepted for backward compatibility: `cards: true` maps to `toolDisplay: 'cards'`, `cards: false` to `toolDisplay: 'text'`, and `formatToolCall` is shimmed onto a `toolDisplay` function. An explicit `toolDisplay` always wins. Migrate `cards: false` → `toolDisplay: 'text'`, and `formatToolCall: (info) => msg` → `toolDisplay: (event) => event.kind === 'result' ? { kind: 'post', message: msg } : undefined`.
