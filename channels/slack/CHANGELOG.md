...
- Improved Slack channel UX: ([#16937](https://github.com/mastra-ai/mastra/pull/16937))
  ...
  - **Deprecation (not removed)** — the `cards: boolean` and `formatToolCall` fields on `SlackProviderConfig`/`SlackAdapterChannelConfig` are now `@deprecated` but still accepted for backward compatibility: `cards: true` maps to `toolDisplay: 'cards'`, `cards: false` to `toolDisplay: 'text'`, and `formatToolCall` is shimmed onto a `toolDisplay` function. An explicit `toolDisplay` always wins. The `formatToolCall` shim posts on `result`/`error` (as in 1.2.x) and renders the built-in Approve/Deny card for tool approvals (the legacy API had no approval rendering), so approvals are never stuck. Migrate `cards: false` → `toolDisplay: 'text'`, and `formatToolCall: (info) => msg` → `toolDisplay: (event) => event.kind === 'result' ? { kind: 'post', message: msg } : undefined`.
