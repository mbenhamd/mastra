...
function approvalMessage(responses: Array<{ request: ApprovalRequest; approved: boolean }>): V6UIMessage {
  return {
    id: 'message-approval-batch',
    role: 'assistant',
    parts: responses.map(({ request, approved }) => ({
...
describe('v6 trailing-message tool approval extraction (e2e)', { timeout: 180_000 }, () => {
  it('resumes multiple exact targets from one trailing assistant message', async () => {
    ...
    const response = await handleChatStream({
      ...
      params: {
        messages: [
          approvalMessage([
            { request: approvalA, approved: true },
            { request: approvalB, approved: true },
          ]),
        ],
      },
    });
