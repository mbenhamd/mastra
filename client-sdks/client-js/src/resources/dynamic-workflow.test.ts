...
describe('DynamicWorkflow resource', () => {
  ...
  it('lists dynamic workflows with filters', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:4111/api/dynamic/workflows?status=active&authorId=user-1',
      expect.any(Object),
    );
  });
  ...
  it('upserts a dynamic workflow definition', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'http://localhost:4111/api/dynamic/workflows',
  ...
  it('gets and deletes an id-scoped dynamic workflow', async () => {
    ...
    expect(fetchMock).toHaveBeenLastCalledWith(
      'http://localhost:4111/api/dynamic/workflows/daily%20summary',
      expect.any(Object),
    );
  ...
  it('drives create, retrieve, execute, replace, and delete through the client resources', async () => {
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (url.pathname === '/api/dynamic/workflows' && init?.method === 'POST') {
        revision += 1;
        stored = {
          ...body,
          status: 'active',
      ...
      if (url.pathname === '/api/dynamic/workflows/daily-summary' && init?.method === 'DELETE') {
        stored = undefined;
      ...
      if (url.pathname === '/api/dynamic/workflows/daily-summary') {
        return new Response(JSON.stringify(stored), { status: stored ? 200 : 404 });
      }

      if (url.pathname === '/api/dynamic/workflows') {
  ...
  it('saves helper workflows alongside the root in one upsert and exposes their ids', async () => {
    ...
    fetchMock.mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      const detailMatch = url.pathname.match(/^\/api\/dynamic\/workflows\/(.+)$/);
      if (detailMatch) {
