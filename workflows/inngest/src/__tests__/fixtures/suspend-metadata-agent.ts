...
import { SharedFileServerCache } from './shared-file-server-cache';
...
export function buildSuspendMetaAgent({
  ...
  // Vitest and the spawned tsx worker transform function source differently.
  // Pin the fixture's durable contract explicitly so this test exercises
  // cross-process rehydration rather than transpiler-specific source text.
  requestApproval.recoveryFingerprint = 'suspend-metadata-test:request-approval:v1';
  ...
  const durableAgent = createInngestAgent({ agent, inngest, cache: new SharedFileServerCache(dbUrl) });
