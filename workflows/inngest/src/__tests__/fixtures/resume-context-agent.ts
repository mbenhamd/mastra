...
export function buildResumeContextAgent({
  ...
  // Vitest and the spawned tsx worker transform function source differently, and the fork
  // hashes the tool's `execute` source into its durable tool binding. Pin the fixture's
  // durable contract explicitly so this test exercises cross-process resume rather than
  // transpiler-specific source text.
  saveNote.recoveryFingerprint = 'resume-context-test:save-note:v1';
  ...
  // The fork persists request-context values into durable workflow state only
  // through an explicit allowlist (`durableRequestContextKeys`); without it both
  // the persisted and the fresh `tenant` entry are stripped at the transport
  // boundary and the resumed tool falls back to 'anon'.
  const durableAgent = createInngestAgent({ agent, inngest, durableRequestContextKeys: ['tenant'] });
