...
let devServer: Awaited<ReturnType<typeof startConnectInngestDevServer>> = null;
let duplicateStepIdObserved = false;
let nestedStepToolingObserved = false;

function signalWorkerTree(proc: ChildProcess, signal: NodeJS.Signals): void {
...
async function terminateWorker(proc: ChildProcess | undefined): Promise<void> {
  if (!proc) return;
  const exited =
    proc.exitCode === null ? new Promise<void>(resolve => proc.once('exit', () => resolve())) : Promise.resolve();
  signalWorkerTree(proc, 'SIGTERM');
  await Promise.race([exited, new Promise<void>(resolve => setTimeout(resolve, 2_000))]);
  signalWorkerTree(proc, 'SIGKILL');
...
function startWorker(): Promise<ChildProcess> {
  return new Promise((resolve, reject) => {
    ...
    const onData = (buffer: Buffer) => {
      ...
      duplicateStepIdObserved ||= output.includes('Duplicate step ID');
      nestedStepToolingObserved ||= output.includes('Nested step tooling detected');
      if (!settled && output.includes('FINISH_SIDE_EFFECTS_WORKER_READY')) {
        settled = true;
        clearTimeout(timer);
        resolve(proc);
      }
...
describe('durable finish side effects on a connect worker', () => {
  ...
  it('persists processed output, generates a title, and recalls it after a worker restart', async () => {
    duplicateStepIdObserved = false;
    nestedStepToolingObserved = false;
    ...
    expect(duplicateStepIdObserved).toBe(false);
    expect(nestedStepToolingObserved).toBe(false);
  });
});
