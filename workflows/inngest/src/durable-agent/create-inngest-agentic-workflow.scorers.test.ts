...
const INNGEST_PROVIDER_STEPS = new Set(['clear-agent-control-topic']);
...
describe('createInngestDurableAgenticWorkflow scorers', () => {
  ...
  describe('when the workflow is built', () => {
    ...
    it('clears binding-scoped abort retention after the final-output checkpoint and before scorers', () => {
      expect(inngestStepIds.indexOf('clear-agent-control-topic')).toBeGreaterThan(
        inngestStepIds.indexOf('map-final-output'),
      );
      expect(inngestStepIds.indexOf('clear-agent-control-topic')).toBeLessThan(
        inngestStepIds.indexOf('execute-scorers'),
      );
    });
  });
  ...
  describe('when compared against the core durable engine', () => {
    it('does not lag core on the steps that run outside the agentic loop', () => {
      ...
      expect(inngestStepIds.filter(stepId => !INNGEST_PROVIDER_STEPS.has(stepId))).toEqual(coreStepIds);
    });
  });
});
