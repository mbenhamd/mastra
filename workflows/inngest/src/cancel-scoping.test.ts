 ...
 *
 * The Inngest function must match the complete lifecycle tuple. Matching only
 * the function or run id lets a stale cancellation terminate a newer execution
 * generation or resume attempt for the same run.
 */
...
describe('inngest cancel event scoping', () => {
  it('scopes cancellation to the run named by the cancel event', () => {
    ...
    expect(config.cancelOn).toEqual([
      {
        event: 'cancel.workflow.scoped-workflow',
  ...
  it('matches on the same field the run sends when cancelling', async () => {
    // `Run.cancel()` sends the same lifecycle tuple carried by the trigger,
    // so Inngest can pair only the exact run generation and resume attempt.
    ...
    expect(cancelOn.if).toBe(
      'async.data.runId == event.data.runId && async.data.executionGeneration == event.data.executionGeneration && async.data.lifecycleResumeAttempt == event.data.lifecycleResumeAttempt',
    );
    expect(config.triggers).toEqual({ event: 'workflow.field-workflow' });
  });

  // The cancellation predicate compares the cancel event against the trigger.
  // A run is cancellable only if the trigger carries the same lifecycle tuple
  // that `cancel()` later sends.
  it('names the run on the trigger event so the cancel event can pair with it', async () => {
    ...
    expect(cancel.data.executionGeneration).toBe(trigger.data.executionGeneration);
    expect(cancel.data.lifecycleResumeAttempt).toBe(trigger.data.lifecycleResumeAttempt);
  });
