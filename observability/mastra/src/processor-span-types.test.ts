...
describe('processor-declared span types', () => {
  ...
  it('does not let a declaration overwrite the runner-owned pipeline facts', async () => {
    ...
    // declaration claimed index 99; this fork keeps a single plain processor
    // on the direct legacy executor, whose runner-owned value must win.
    expect(attributes?.processorIndex).toBe(0);
    expect(attributes?.processorExecutor).toBe('legacy');
  });
