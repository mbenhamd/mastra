...
/** Emits one or more tool-call steps, then returns final text. */
function toolsThenTextModel(toolSteps: Array<Array<{ name: string; args: object }>>, finalText: string) {
  let call = 0;
  return new MockLanguageModelV2({
    doStream: async () => {
      const toolCalls = toolSteps[call];
      call++;
      return {
        stream: convertArrayToReadableStream(
          toolCalls
...
/** Tags tool-result processor spans so multi-tool traces can be asserted exactly. */
class ToolChunkTracingProcessor implements Processor {
  readonly id = 'tool-chunk-tracing';
  readonly name = 'Tool Chunk Tracing';

  async processOutputStream(args: ProcessOutputStreamArgs): Promise<ChunkType | null | undefined> {
    if (args.part.type === 'tool-result') {
      args.tracingContext?.currentSpan?.update({
        attributes: { testToolCallId: args.part.payload.toolCallId } as any,
      });
      // Keep concurrent tool-result calls overlapping long enough to expose a
      // shared-span race when the durable processor pipeline is not serialized.
...
async function runToCompletion(
  ...
  const res = await wrapped.stream(prompt, options);
...
describe('durable-agent observability — full span tree (real exporter)', () => {
  ...
  it.each([
    {
      name: 'sequential tool steps',
      toolSteps: [
        [{ name: 'get_weather', args: { city: 'Paris' } }],
        [{ name: 'get_weather', args: { city: 'Rome' } }],
      ],
      expectedToolCallIds: ['call-1-1', 'call-2-1'],
      options: { toolCallConcurrency: 1 },
    },
    {
      ...
      expectedToolCallIds: ['call-1-1', 'call-1-2'],
      options: { toolCallConcurrency: 2 },
    },
  ])(
    'DurableAgent gives every tool-result chunk its own completed processor span for $name',
    async ({ toolSteps, expectedToolCallIds, options }) => {
      const agent = new Agent({
        id: 'a',
        name: 'a',
        instructions: 'use the tool',
        model: toolsThenTextModel(toolSteps, 'Both calls completed.') as any,
        tools: { get_weather: weatherTool },
        outputProcessors: [new ToolChunkTracingProcessor()],
      });
      const { wrapped } = buildMastra(testExporter, agent, 'durable');
      await runToCompletion(wrapped, 'compare weather', testExporter, options);

      const agentRun = testExporter.getSpansByType('agent_run' as any)[0];
      const toolChunkProcessorSpans = testExporter
        .getAllSpans()
        .filter(
          (span: any) =>
            span.entityId === 'tool-chunk-tracing' &&
            span.attributes?.testToolCallId !== undefined &&
            span.output?.totalChunks === 1,
        );

      expect(toolChunkProcessorSpans.map((span: any) => span.attributes.testToolCallId).sort()).toEqual(
        expectedToolCallIds,
      );
      for (const span of toolChunkProcessorSpans) {
        expect(parentOf(span)).toBe(idOf(agentRun));
        expect((span as any).traceId).toBe((agentRun as any).traceId);
        expect((span as any).output).toEqual({ totalChunks: 1, accumulatedText: '' });
      }
      expect(testExporter.getIncompleteSpans()).toHaveLength(0);
    },
  );
