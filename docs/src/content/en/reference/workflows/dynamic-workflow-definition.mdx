...
`prefixItems` must be non-empty, `items` must be `false`, and `minItems` must equal the number of prefix schemas. If present, `maxItems` must equal the same number. Open tuples, optional tuple positions, and draft-07 tuple-form `items: [...]` aren't admitted.
