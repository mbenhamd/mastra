...
description: "The Step class defines individual units of work within a workflow, encapsulating execution logic, data validation, and input/output handling."
packages:
  - "@mastra/core"
---
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          {
            name: 'suspend',
            type: '(suspendPayload: any, suspendOptions?: { resumeLabel?: string | string[] }) => Promise<void>',
            description:
              'Function to pause workflow execution. Evented resume labels must be unique across concurrently suspended targets, are limited to 256 UTF-8 bytes each, and are bounded to 64 labels and 16 KiB of label metadata per suspended workflow state.',
          },
          {
            name: 'state',
            type: 'z.infer<TState>',
