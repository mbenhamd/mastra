...
description: "The .startAsync() method starts a workflow run without waiting for completion. It returns immediately with the runId, allowing the workflow to execute in the background."
packages:
  - "@mastra/core"
---
...
The `.startAsync()` method starts a workflow run without waiting for completion. It returns immediately with the `runId`, allowing the workflow to execute in the background. This is useful for long-running workflows or scheduled tasks. It also avoids blocking on workflow completion. Built-in workflow runs also return an optional `execution` promise so infrastructure code can observe background completion or rejection without changing the fire-and-forget behavior. Evented or integration-backed engines may omit that promise. Use persisted run status as the portable completion contract.
...
const { runId, execution } = await run.startAsync({
  inputData: {
    value: 'initial data',
  },
})

// Optional: observe an in-process built-in execution without blocking this call.
...
<PropertiesTable
  ...
  {
    name: 'execution',
    type: 'Promise<WorkflowResult> | undefined',
    isOptional: true,
    description:
      'Observable execution promise for built-in workflow runs. Integration-backed engines may omit it; attach a rejection handler when consuming it.',
  },
]}
/>
