...
When a subscriber calls `nack`, the event is republished with the same logical event ID, an incremented `deliveryAttempt`, and then the original is acknowledged. Republishing first ensures a Redis write failure leaves the original pending for reclaim. Once an event reaches `maxDeliveryAttempts`, it's dropped instead of redelivered. Configure that transport cap at or above Mastra's workflow source-event retry budget so the processor can reach its terminalization path. Separately, each subscription periodically reclaims events that an earlier consumer in the group read but never acknowledged, controlled by `reclaimIntervalMs` and `reclaimIdleMs`.
