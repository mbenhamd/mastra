...
description: "CachingPubSub wraps any PubSub implementation and adds event caching and replay."
packages:
  - '@mastra/core'
---
...
<PropertiesTable
  ...
  },
  {
    ...
    properties: [
      ...
      {
        name: 'indexedReplay',
        type: 'CachingPubSubIndexedReplayOptions',
        description:
          'Enables exact indexed replay when the cache supports atomic indexed logs. Sets retained history age and event-count limits.',
        isOptional: true,
      },
      {
        name: 'durableLocalOnly',
        type: "'reject' | 'passthrough'",
        description:
          'Controls local-only publishes with durable exact replay. Passthrough skips retained history and forwards the event to the inner pub/sub.',
        isOptional: true,
        defaultValue: 'reject',
      },
    ],
...
### Compose durable replay with local-only topics

Durable exact replay rejects `localOnly` publishes by default because a process-local event can't also be part of shared retained history. If the same `CachingPubSub` carries intentionally non-replayable local topics, opt into passthrough explicitly:

```typescript
const pubsub = new CachingPubSub(innerPubSub, durableCache, {
  indexedReplay: {
...
With passthrough enabled, `localOnly` events bypass the cache and retain their locality option when forwarded to the inner pub/sub. Other events still use durable exact indexed replay. Only use this option when local-only topics don't require replay.
