...
Publishes an event to a topic. The `id` and `createdAt` fields are assigned by the implementation. Set `options.localOnly` to keep an event in the publishing process when the backend supports process-local delivery.
...
Mastra applies `localOnly` automatically to run-scoped internal workflow events. Custom backends must not send a `localOnly` event to another process or host.
...
`deliveryAttempt` is transport-owned retry metadata. A backend that supports tracked redelivery must keep it with the logical event and start it at `1`. The backend must increment it before each redelivery. Workflow processing uses this value to prevent a worker replacement or process restart from resetting the source-execution budget. Omit it when the backend can't track delivery. Mastra treats both an omitted value and Google Pub/Sub's documented `0` sentinel as the first delivery. Other invalid values are treated as exhausted.
