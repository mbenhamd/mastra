...
When `tools` contains Mastra tools, Mastra binds them to the current agent execution before the model call. Their execution context includes the active Mastra instance, memory, request context, workspace, and agent, thread, and resource identifiers when configured.
