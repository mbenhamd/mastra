...
These lifecycle methods operate on the default `'in-memory'` store. They're no-ops for the `'context'` store: its durable state lives in conversation messages, and its same-run bridge lives in request-local processor state that is released with the request.
...
Releases the in-memory cleanup timer and all processor-owned in-memory state. Call this when a processor instance is evicted or replaced. It is safe to call more than once.
...
Restart safety depends on supplying that persisted history to the next processor run. A configured Mastra MessageHistory processor can do this when it recalls history before ToolSearch runs. Without memory, the caller must provide the prior messages; sending only the newest message leaves no context record to reconstruct.

Within one agent run, request-local processor state bridges a successful load to the next model step before the assistant result appears in persisted messages. This bridge also works without a thread ID. It isn't shared across runs, so discarding or aborting one run doesn't activate tools in another run for the same thread.
