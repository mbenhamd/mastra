...
Unloading a tool changes the tool definitions sent to the model. The cached prefix shifts, causing the next turn to pay a cache write instead of a cache hit.

- In `'in-memory'` mode, unloading happens when `ttl` evicts a thread's state.
- In `'context'` mode, unloading happens when a tool's activation result is absent from the messages, such as after older messages are trimmed.

The model must search for the unloaded tool again before reuse. Removing an unused tool trades one cache write for a smaller prefix on later turns.
