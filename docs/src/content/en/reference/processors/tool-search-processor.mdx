...
Use `filterTool` to apply request-specific policy to dynamic tools. The hook receives the resolved tool ID, tool, request context, and phase.
...
const toolSearch = new ToolSearchProcessor({
  tools: allTools,
  filterTool: ({ toolName, requestContext, phase }) => {
