    ...
    properties: [
      {
        ...
        parameters: [
          ...
          {
            name: 'budget',
            type: '{ maxToolCount?: number; maxToolSchemaBytes?: number }',
            description:
              'Optional cumulative limits for dynamically loaded tools. A search or load that would exceed either limit is refused atomically, while tools already loaded remain available.',
            isOptional: true,
          },
          {
            name: 'budget.maxToolCount',
            type: 'number',
            description:
              'Maximum number of distinct dynamically loaded tools in the complete loaded set. Must be a non-negative safe integer.',
            isOptional: true,
          },
          {
            name: 'budget.maxToolSchemaBytes',
            type: 'number',
            description:
              'Maximum cumulative UTF-8 bytes for dynamically loaded tool definitions. Must be a non-negative safe integer.',
            isOptional: true,
          },
          {
            name: 'storage',
            ...
            description:
              "Where loaded-tool state lives. 'in-memory' (default) tracks loaded tools in an in-memory map per thread with TTL cleanup (see ttl); state is lost on restart and anonymous requests share a 'default' entry. 'context' derives loaded state from canonical assistant tool results: load_tool must report loaded names, while search_tools must include the explicit receipt emitted by autoLoad. It is restart-safe, requires no memory, and de-loads automatically once that result is no longer present in the messages. The 'context' store is opt-in.",
            isOptional: true,
            default: "'in-memory'",
          },
          {
...
Tool IDs must be unique. Tool keys and IDs may differ, but a key may not equal another tool's ID because that makes search/load resolution ambiguous. `search_tools` and `load_tool` are reserved as both keys and IDs. The processor also rejects an always-available input tool that would shadow one of its meta-tools or a loaded dynamic tool.
...
These lifecycle methods operate on the default `'in-memory'` store. They're no-ops for the `'context'` store: its durable state lives in conversation messages, and its same-run bridge lives in request-local processor state that's released with the request.
...
Releases the in-memory cleanup timer and all processor-owned in-memory state. Call this when a processor instance is evicted or replaced. It's safe to call more than once.
...
- `load`: Blocks explicit `load_tool` calls and automatic activation when `search.autoLoad` is enabled.
...
In `storage: 'context'` mode, retained activation receipts are rechecked in the `load` phase before they consume cumulative budget. A receipt denied by current load policy is neither injected nor charged. A load-authorized tool hidden only by the `active` phase still consumes budget because active filtering doesn't unload it.
...
## Cumulative loaded-tool budgets

`search.topK` limits one search result, but tools loaded by multiple searches are additive. Use `budget` when the complete loaded set must stay within a count or serialized-schema limit:
...
Both limits apply only to tools loaded through ToolSearch. They don't count the ToolSearch meta-tools or tools kept eagerly available outside the searchable catalog. `maxToolSchemaBytes` measures the UTF-8 byte length of a JSON array containing each loaded tool's callable `name`, normalized draft-07 input `schema`, and `description` when present. It excludes output schemas, provider-specific options, and executable state. For request-resolved CoreTools, measurement uses the input schema available at the processor boundary, including any model compatibility rewrite applied before input processors run.

Limits are checked against the full loaded set before state is changed. A batch that would exceed either limit is refused as a whole. ToolSearch never loads a prefix of an over-budget batch. In `autoLoad` mode, `search_tools` still returns the discovery results but omits its activation receipt. Both meta-tools return a structured `budget` refusal with the violated `tool_count` or `tool_schema_bytes` metric, configured limit, current usage, attempted increase, and resulting usage. Explicit `load_tool` refusals also return `success: false`.

If a candidate input schema can't be converted or serialized safely, the operation returns `budget.code: 'tool_search_budget_measurement_failed'` and loads nothing. Request-resolved tools may replace the schema of an admitted name. An unmeasurable replacement stops the processor before model input instead of being treated as zero bytes.

Loaded names continue consuming the count budget when they're absent from a later request's searchable catalog. With a schema-byte limit, an unresolved loaded name also stops processing because its cumulative byte usage can't be measured safely.

Loading a tool that's already active remains successful at the limit because it doesn't increase usage. If reconstructed conversation or in-memory state already exceeds a newly configured budget, input processing and resume fail instead of silently removing tools. Set either limit to `0` to prohibit dynamic loading along that dimension. Omitting `budget`, or omitting one of its fields, preserves the unbounded behavior for that dimension.
...
Loaded state is derived from canonical assistant tool results in the conversation messages:

- A `load_tool` result activates only the names in its `loaded` array.
- A `search_tools` result activates tools only when `search.autoLoad` emitted its explicit activation receipt. A discovery-only search result never activates tools.
- Tool-shaped results on user messages are ignored.

This mode:

- Requires no memory configuration.
- It's restart-safe: the durable record is the persisted message history.
- De-loads a tool automatically once that result is no longer present in the messages.

Restart safety requires supplying that persisted history to the next processor run. A configured Mastra MessageHistory processor can recall the history before invoking ToolSearch. Without memory, the caller must provide the prior messages. Sending only the newest message leaves no context record to reconstruct.

Within one agent run, request-local processor state bridges a successful load to the next model step before the assistant result is written to persisted messages. This bridge also works without a thread ID. It isn't shared across runs, so discarding or aborting one run doesn't activate tools in another run for the same thread.
...
Unloading a tool changes the tool definitions sent to the model, which shifts the cached prefix and causes the next turn to pay a cache write instead of a cache hit. In `'in-memory'` mode this happens when a thread's state is evicted by `ttl`. In `'context'` mode it happens when a tool's activation result is no longer present in the messages (for example, when older messages are trimmed). The tool de-loads, and the model must search for it again before reuse. This is expected: removing an unused tool trades one cache write for a smaller prefix on later turns.
