...
The v2 `recognize` call is IAM-authorized and doesn't accept API-key-only authentication. Configure service account credentials on the `listeningModel` (or set `GOOGLE_APPLICATION_CREDENTIALS`) and set `GOOGLE_CLOUD_PROJECT` so the recognizer path can be resolved, even when `vertexAI` isn't enabled.
...
:::note
`listen({ v2: true })` fails with `PERMISSION_DENIED` on `speech.recognizers.recognize` when only `GOOGLE_API_KEY` is set. An API-key request carries no OAuth identity, so granting `roles/speech.client` to a user account doesn't help. The role must be granted to the service account presented in the request. This applies regardless of the `vertexAI` setting; `speak()` and v1 `listen()` still work with an API key alone.
:::
...
Uses a Google Cloud API key for authentication. Covers `speak()` and v1 `listen()`. It doesn't cover `listen({ v2: true })`, which is IAM-authorized and requires service account credentials (see [v2](#v2)).
