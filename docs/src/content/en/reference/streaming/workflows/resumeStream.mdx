...
description: "The .resumeStream() method resumes a suspended workflow run with new data, allowing you to continue execution from a specific step and to observe the stream of events."
packages:
  - "@mastra/core"
---
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    description:
      'The resume label recorded when the step suspended. Use a label to select the matching suspended step, including a step inside a nested workflow, parallel branch, or foreach iteration. Evented workflows accept labels up to 256 UTF-8 bytes. If you also pass forEachIndex, it must match the iteration stored for the label.',
    isOptional: true,
  },
  {
    name: 'resumeData',
  ...
  },
  {
    ...
    type: 'number',
    description:
      'Target a specific iteration of a suspended `.foreach()` step. Pass the zero-based index of the iteration to resume; other iterations remain suspended. Omit to resume all suspended iterations of the step with the same `resumeData`. When label is present, this index must equal the iteration selected by that label.',
    isOptional: true,
  },
  {
    name: 'tracingOptions',
