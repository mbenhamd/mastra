...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    description:
      'The resume label recorded when the step suspended. Use a label to select the matching suspended step, including a step inside a nested workflow, parallel branch, or foreach iteration.',
    isOptional: true,
  },
  {
    name: 'resumeData',
