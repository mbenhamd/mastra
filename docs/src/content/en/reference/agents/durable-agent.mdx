...
<PropertiesTable
  content={[
  {
    ...
    isOptional: true,
    description:
      'Unique identifier for this run. Use it later with `resume()` or `observe()`. A caller-supplied ID remains reserved until the run is cleaned up; starting or preparing another active run with the same ID fails instead of replacing its runtime dependencies.',
  },
  {
    name: 'instructions',
