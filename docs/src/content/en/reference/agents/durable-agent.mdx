...
Resumes a suspended run, for example after a tool approval. Pass the `runId` from the original stream and the exact `toolCallId` the run was waiting on. If the in-memory registry was lost, a configured persistent workflow store can rehydrate the run after verifying its owner tuple and observable runtime bindings. See [`listSuspendedRuns()`](/reference/agents/listSuspendedRuns) for discovery, recovery requirements, and distributed-consumption limitations.
