...
- **Parent agent reused.** The fork runs through the parent agent's `stream(...)` call. The parent's instructions, tools, model, and processors apply; reusing that parent agent surface is what preserves the prompt-cache prefix. The subagent definition's `instructions`, `tools`, `allowedHarnessTools`, `allowedWorkspaceTools`, `defaultModelId`, `maxSteps`, `stopWhen`, `inputProcessors`, and `outputProcessors` are ignored in forked mode. The subagent tool uses fork-specific loop controls so the model can recover if it accidentally calls an inherited-but-disabled tool.
