...
Without `force`, the target session and descendants must already be closed. With `force`, Harness closes the subtree first, then deletes the session rows and owned attachments. The `resourceId` parameter is required so callers do not delete a session from another resource by ID.
