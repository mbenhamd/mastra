...
A `Session` owns all the state tied to a single conversation. The [`Harness`](/reference/harness/harness-class) is the shared host for agents, storage, config, the thread lock, and the event bus. The `Session` holds everything that's per-conversation: identity, the active thread binding and reads, mode and model selection, run and abort state, the live agent stream, tool suspensions, follow-ups, approvals, permission grants, token usage, and the display-state snapshot.
...
<PropertiesTable
  content={[
  {
    ...
    description: 'Resource identity for the conversation. See identity methods below.',
  },
  {
    name: 'thread',
    type: 'SessionThread',
...
Send a user-message signal to the active thread. If a run is active, the signal interleaves into that exact run. If the thread is idle, it wakes a new run. The returned handle contains `id`, `runId`, `willInterleave`, `accepted`, `signal`, and a `result` promise for the eventual `AgentResult`.

Without `admissionId`, `signal()` preserves its optimistic boundary and returns the native routing decision on the first await tick. With `admissionId`, it first persists a reservation and claims dispatch ownership. It then waits for native or broker acceptance and durably records that acceptance. A live competing dispatcher waits for the claim. An expired dispatcher may recover it.

If native acceptance remains ambiguous for 30 seconds, or the caller aborts while waiting for it, the call rejects and stops renewing its dispatch claim. The durable row remains recoverable instead of being marked failed. After that claim expires, retry the same `admissionId` and payload. This acquires a new dispatch attempt with the original signal and run IDs, without reusing a still-pending acknowledgement from the expired attempt. Stable signal-ID deduplication still prevents an already-accepted same-payload signal from being appended twice. A conflicting payload with the same `admissionId` continues to reject before dispatch.

```typescript
const session = await harness.session({ resourceId: 'project-xyz' })
const handle = await session.signal({
  content: 'Use the updated requirements',
  admissionId: 'requirements-v2',
})

const result = await handle.result
```

An exact admitted retry returns the original signal and run identity. Idle recovery always reuses the deterministic idle run ID. Accepted active delivery is pinned to the original active run and fails explicitly if that run disappears. It's never rerouted to a new cold run.

Durable admission evidence records whether the operation came from `message()` or `signal()`. This keeps signal dispatch fencing isolated from normal message settlement across process restarts. Older admitted signal rows are migrated only when they already contain dispatch state or use the stable `harness-channel-signal-` identity. A run ID by itself isn't treated as evidence that the row is a signal.

`admissionId` provides a stable at-least-once execution identity, not exactly-once side effects. After a crash or lost acknowledgement, recovery may repeat provider or tool work under the same run ID. External effects still need their own idempotency keys when exactly-once behavior matters.

### Permissions

Session-scoped grants auto-approve tools without prompting. Grants are ephemeral, so they reset when the session restarts and are never persisted.
...
#### `respondToToolApproval({ approved, editedArgs?, reason?, approvalScope?, ...generation })`

Respond to a pending tool approval. `approved: false` declines execution. Use `reason` to provide an optional explanation. `approvalScope` is `'once'` by default, or `'always'` to grant the pending tool for the rest of the session. The latter requires `approved: true`.

`editedArgs` is an optional shallow JSON object patch for regular agent tool-gate approvals. The merged input must pass the tool's native input validator. Durable agents reject edits. This restriction also applies to delegated tools and approvals raised inside tool execution. See [edit inputs before approval](/docs/harness/tool-approvals#edit-inputs-before-approval) for merge and validation behavior.

Include `responseId`, `itemId`, `runId`, `toolCallId`, and `pendingRequestedAt` to bind the response to the displayed pending generation and receive an inbox receipt. Reuse that complete response for an idempotent retry. A different patch under the same response ID conflicts.
...
await session.respondToToolApproval({
  ...
  approved: true,
  editedArgs: { query: 'reviewed search query' },
})
```
...
Abort the in-flight run. This aborts the live stream and requests abort on the run. It also clears parked tool suspensions.
...
To change the resource ID, use [`harness.setResourceId()`](/reference/harness/harness-class#setresourceid-resourceid-), which also tears down the active thread.
...
Switch to a different mode. This aborts any in-progress generation and saves the current model to the outgoing mode. It then loads the incoming mode's model and emits `mode_changed` and `model_changed` events.
...
`session.state` owns the schema-validated Harness state for the conversation. It holds the current snapshot and validates updates against the `stateSchema` passed to the Harness. It also serializes concurrent writes and emits a `state_changed` event on every change.
...
Merge a partial update into the state. The write queue applies concurrent calls in order after schema validation. State changes emit `state_changed` with the changed keys.
...
Run an updater against the current snapshot and apply its result atomically within the write queue. Use this for read-modify-write changes that must see the latest state. The updater returns `updates` to merge and may include `events` to emit. Its `result` field becomes the resolved value of `update()`.
