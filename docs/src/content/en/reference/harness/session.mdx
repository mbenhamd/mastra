...
If native acceptance remains ambiguous for 30 seconds, or the caller aborts while waiting for it, the call rejects and stops renewing its dispatch claim. The durable row remains recoverable instead of being marked failed. After that claim expires, retry the same `admissionId` and payload: the retry acquires a new dispatch attempt while retaining the original signal and run IDs. The native runtime doesn't reuse a still-pending acknowledgement from the expired attempt. Stable signal-ID deduplication still prevents an already-accepted same-payload signal from being appended twice. A conflicting payload with the same `admissionId` continues to reject before dispatch.
...
Respond to a pending tool approval. `approved: false` declines execution; `reason` provides an optional explanation. `approvalScope` is `'once'` by default, or `'always'` to grant the pending tool for the rest of the session. The latter requires `approved: true`.

`editedArgs` is an optional shallow JSON object patch for regular agent tool-gate approvals. The merged input must pass the tool's native input validator. Durable agents, delegated tools, and approvals raised inside tool execution reject edits. See [edit inputs before approval](/docs/harness/tool-approvals#edit-inputs-before-approval) for merge and validation behavior.
