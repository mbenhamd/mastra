...
Pass a stable `id` to preserve a sandbox's filesystem across sessions or across a `stop()`/`start()` cycle:
...
Call `snapshot()` after a filesystem update to capture the configured recovery checkpoint immediately. It resolves without capturing when the sandbox isn't running. Railway also requires a caller-provided `id`. E2B can capture a provider snapshot without one.

```typescript
await sandbox.snapshot()
```

Lifecycle behavior differs by provider. On Railway, `stop()` tears down the VM but preserves its `id`-keyed recovery checkpoint, while `destroy()` deletes that checkpoint before tearing down the VM. On E2B, `destroy()` kills the sandbox but doesn't delete snapshots that the caller captured separately. Don't use `destroy()` as a provider-portable way to preserve state for a later `start()`.
...
<PropertiesTable
  ...
  },
  {
    ...
    type: '() => Promise<void>',
    description:
      'Tear down the remote sandbox and clear the cached exec lease. For Railway, also release the recovery checkpoint associated with a caller-provided id. E2B snapshots are caller-owned and are not deleted by this method.',
  },
  {
    name: 'stop',
    type: '() => Promise<void>',
    description:
      'Tear down the remote sandbox while preserving its recovery checkpoint. A later start() with the same stable id can restore that checkpoint. This is not an alias for destroy().',
  },
  {
    name: 'executeCommand',
  ...
  },
  {
    ...
    type: '() => Promise<void>',
    description:
      'Capture provider recovery state immediately. Resolves without work when the sandbox is not running; Railway also skips when no caller-provided id exists, while E2B can capture without one.',
  },
  {
    name: 'captureCheckpoint',
    type: '() => Promise<CaptureCheckpointResult>',
    description:
      "Capture provider recovery state and return either captured/coalesced with the checkpoint name, or skipped with no-checkpoint-name-configured/sandbox-not-running. Concurrent callers share one in-flight provider request and receive that request's same result.",
  },
  {
    name: 'getInfo',
