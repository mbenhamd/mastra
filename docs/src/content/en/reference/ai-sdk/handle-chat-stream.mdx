...
When an agent tool approval is declined, `handleChatStream()` emits a semantic AI SDK UI stream part:
...
Use this part to distinguish a declined approval from a tool execution failure. Tool failures continue to emit `tool-output-error`.
