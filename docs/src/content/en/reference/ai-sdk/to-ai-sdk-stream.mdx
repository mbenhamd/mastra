...
## Tool approval denial in UI streams

When a tool approval is declined, `toAISdkStream()` emits a semantic AI SDK UI stream part instead of requiring the client to inspect error text:
...
Mastra emits this part when the source `tool-result` chunk includes `denied: true`. Ordinary tool execution failures continue to emit `tool-output-error`.
