...
| Method | Path | Description |
| - | - | - |
| `GET` | `/api/dynamic/workflows` | List dynamic workflow definitions, filterable by `status` and `authorId` |
| `GET` | `/api/dynamic/workflows/:dynamicWorkflowId` | Get a dynamic workflow definition by ID |
| `POST` | `/api/dynamic/workflows` | Upsert a definition (plus optional helper `dependencies`) and live-register it |
| `DELETE` | `/api/dynamic/workflows/:dynamicWorkflowId` | Delete a dynamic workflow definition and unregister the live workflow |

On authenticated servers, the read routes require the `dynamic-workflows:read` permission and the write routes require `dynamic-workflows:write`. Registered dynamic workflows are executed through the ordinary `/api/workflows/:workflowId` routes above.
