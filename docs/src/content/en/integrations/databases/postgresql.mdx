...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      'When true, automatic table creation/migrations are disabled and lazy schema helpers validate required tables and indexes without issuing DDL. Use a privileged migration role to manage the schema separately.',
    isOptional: true,
  },
  {
    name: 'enabledDomains',
    type: 'ReadonlyArray<PostgresDomainKey>',
    description:
      'Restricts the store to a non-empty list of unique storage domain names. When omitted, all domains are enabled. Empty lists, unknown names, and duplicates throw during construction before PostgresStore creates an internal connection pool.',
    isOptional: true,
  },
  {
...
Use `disableInit: true` when schema changes run separately from the runtime process. `PostgresStore.init()` returns without connecting to PostgreSQL, and lazy Harness and retention helpers read PostgreSQL's catalog to validate their required tables and indexes without issuing `CREATE`, `ALTER`, or `DROP` statements. Missing structure fails with a typed storage error so the runtime does not report a successful operation against an incomplete schema.

Read-only catalog results are reused by each domain instance, so complete an external migration before constructing runtime stores. After an external schema change, reconstruct the affected storage instance so it validates the new schema.

Schema-prefixed native indexes use a deterministic hash suffix when the combined name exceeds PostgreSQL's 63-byte identifier limit. If an older migration created a silently prefix-truncated index name, rerun the privileged schema migration before starting the external runtime; it creates the canonical name, while any old truncated object remains for explicit privileged cleanup.

Run migrations with a role that can change the schema, then start the runtime with a role that has only the table DML and schema privileges it needs:

```typescript
const migrator = new PostgresStore({
  id: 'pg-migrator',
...
await runtime.init() // no startup connection or schema DDL
```

Retention anchor indexes are created lazily when a retention policy first runs and are not included in `exportSchemas()`. If a runtime retention policy is enabled, create its anchor indexes with the privileged migration role before switching to the restricted runtime role; external-schema mode validates those indexes and reports a typed error before pruning when one is missing.

`MASTRA_DISABLE_STORAGE_INIT=true` provides the same no-initialization and no-lazy-DDL behavior for processes configured through environment variables.

The environment flag suppresses automatic initialization while allowing the explicit privileged migration CLI path. `disableInit: true` also disables `PgDB`'s explicit schema migration helpers.

#### Workflow revision migration rollout and recovery

Workflow storage uses durable parent-revision evidence to prevent an old continuation contract from targeting a reused run ID. The first privileged `init()` after upgrading validates existing workflow snapshots and terminalization records, migrates the revision table in one transaction, and records both a migration marker and a private migration-epoch sentinel.

Treat this as a quiesced, forward-only migration boundary:

1. Stop every older process that can initialize or write workflow storage.
1. Back up the complete PostgreSQL schema and workflow data together.
1. Start one upgraded process and let `init()` finish.
1. Verify that initialization succeeds before starting the remaining upgraded processes.

Do not run a rolling mixture of pre-migration and upgraded workflow writers. Older initializers don't participate in the new migration contract and can recreate revision rows that the upgraded runtime must treat as missing durable evidence.

The migration marker row, private epoch relation and row, parent-revision rows, snapshots, and terminalization records form one recovery unit. Back up and restore that complete unit atomically from the same recovery point. Losing only the marker or only the epoch is detected as incomplete provenance and fails initialization; `PostgresStore` doesn't reconstruct the missing record because doing so could make a reused run ID look like its earlier generation. Losing both provenance records is indistinguishable from a schema that has never completed this migration, so provenance tables are not a substitute for a complete database backup.

After this migration commits, don't roll back by starting a pre-migration binary against the migrated database. Roll forward with a corrected binary, or restore the complete pre-upgrade database backup before starting the older version. Applying an exported schema creates neither the migration marker row nor the private epoch relation or row; populated workflow storage must first complete a privileged `init()`.

### Limit initialization to selected domains

Set `enabledDomains` when a PostgreSQL store owns only part of a composite storage configuration. The store constructs, exposes, and initializes only the listed domains. Calls to `getStore()` return `undefined` for domains that aren't selected.

This example limits schema initialization and runtime access to memory tables:
...
const memory = await memoryStorage.getStore('memory')
const workflows = await memoryStorage.getStore('workflows') // undefined
```

Omit `enabledDomains` to preserve the default behavior and initialize every PostgreSQL storage domain. When you provide it, the array must contain at least one unique `PostgresDomainKey`. The constructor rejects empty arrays, duplicate names, and unknown names before it creates an internal pool.

`PostgresStoreVNext` always owns its required vNext observability domain. If you provide `enabledDomains` to `PostgresStoreVNext`, the list must include `observability`.

When `observability` is the only enabled domain, `init()` initializes it through the required observability connection without connecting to or loading a schema snapshot from the otherwise unused primary pool. Selecting any primary-backed domain, or omitting `enabledDomains`, preserves the normal primary-pool initialization path.
...
PostgreSQL thread and message list methods return empty collections only when a database query succeeds with no matching rows. If PostgreSQL rejects the read, the method throws a typed storage error instead of treating the failure as missing history.

Handle or retry these errors at your application boundary. Don't use a caught read failure as evidence that a thread, message, or resource has no stored history.
