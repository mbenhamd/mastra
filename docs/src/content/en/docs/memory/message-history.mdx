...
When you call the agent, messages from a completed turn are automatically saved to the database. You can specify a `threadId`, `resourceId`, and optional `metadata`:
...
`MessageHistory` doesn't commit a turn whose final result has `finishReason: 'error'` or `finishReason: 'aborted'`, even when the provider produced partial output. Those messages remain available to output processors for the current request, but they aren't added to durable conversation history.
