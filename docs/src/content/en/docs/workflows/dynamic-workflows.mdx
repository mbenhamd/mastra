...
Mastra accepts a bounded subset of JSON Schema Draft 2020-12 for dynamic workflow schemas. It rejects a definition before persistence when a schema uses an unknown keyword or a constraint that Mastra can't convert to an equivalent Zod validator. Validation errors include the schema field and a JSON Pointer to the rejected keyword.
