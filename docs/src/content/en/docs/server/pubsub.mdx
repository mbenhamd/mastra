...
If terminal failure publication is temporarily unavailable, Mastra calls `nack` and leaves the source event with the backend. A `workflow.fail` event never publishes another `workflow.fail`; failure processing retries the existing terminal event instead.
