...
- **Harness**: Create remote Harness sessions and reconnect to session events. Settle durable message or queue operations.
...
Authenticated servers require `dynamic-workflows:read` or `dynamic-workflows:write` for definition operations and `workflows:execute` to run the workflow.
