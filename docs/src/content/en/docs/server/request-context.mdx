...
The reserved raw auth token is always represented as `[REDACTED]`. String, number, boolean, `null`, and `undefined` values are preserved. Objects, arrays, functions, and other non-primitive values are replaced with type labels such as `[object]` or `[function]`; their properties and serialization hooks aren't evaluated.
