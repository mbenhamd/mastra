...
## Observability-safe serialization

Use `serializeForSpan()` when a custom observability integration needs to include request context in span input. It returns a bounded snapshot instead of traversing stored objects:

```typescript
import { MASTRA_AUTH_TOKEN_KEY, RequestContext } from '@mastra/core/request-context'

const requestContext = new RequestContext()
requestContext.set(MASTRA_AUTH_TOKEN_KEY, 'Bearer raw-token')
requestContext.set('userId', 'user-123')
requestContext.set('runtimeClient', { connected: true })

const spanInput = requestContext.serializeForSpan()
// {
//   mastra__authToken: '[REDACTED]',
//   userId: 'user-123',
//   runtimeClient: '[object]'
// }
```

The reserved raw auth token is always represented as `[REDACTED]`. Number, boolean, `null`, and `undefined` values are preserved. Strings are limited to 2,048 UTF-8 bytes, the snapshot is limited to 100 keys and a 16 KiB budget, and deterministic `[TRUNCATED]` markers identify omitted content. Objects, arrays, functions, and other non-primitive values are replaced with type labels such as `[object]` or `[function]`; their properties and serialization hooks aren't evaluated.

Mastra applies this bounded representation to scorer run and scorer step span input. `serializeForSpan()` doesn't change the original `RequestContext` passed to scorer execution.
