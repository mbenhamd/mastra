...
The reserved raw auth token is always represented as `[REDACTED]`. Number, boolean, `null`, and `undefined` values are preserved. Strings are limited to 2,048 UTF-8 bytes. Snapshots include at most 100 context entries, plus a `[TRUNCATED]` marker when content is omitted, within a 16 KiB budget. Objects, arrays, functions, and other non-primitive values are replaced with type labels such as `[object]` or `[function]`; their properties and serialization hooks aren't evaluated.
