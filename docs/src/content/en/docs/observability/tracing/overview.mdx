...
Missing usage is not zero usage. Cache-read, cache-write, and reasoning-token
...
each attempt. A failed attempt ends as an error span, while the generation
aggregate retains its measurement. An abort after dispatch closes the active
inference as an error and retains the aggregate. A cache replay or processor
decision that stops before dispatch creates no provider inference and reports
the aggregate and usage as `not_applicable`.
