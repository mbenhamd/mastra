...
Mastra freezes the replacement tool names and executable descriptors for the execution. Input processors can narrow the set for a step, but they can't expand it, replace an allowed implementation, or mutate its executable fields. An empty `toolsets` object creates a tool-free execution. Durable reconstruction fails closed if the original replacement implementations are no longer available after a process restart.
