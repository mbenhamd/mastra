...
## Project tool payloads for UI and transcripts

Use `payloadProjection` when a tool returns raw data your application needs, but browser-facing streams or user-visible transcript messages should receive a smaller or safer shape.

`payloadProjection` is separate from `toModelOutput`. `toModelOutput` shapes the payload sent back to the model. `payloadProjection` shapes tool input, output, errors, approval payloads, and suspension payloads for `display` and `transcript` targets.
...
You can also define a central policy on an agent or on the Mastra instance. Use this when several tools need the same projection rules.

```typescript title="src/mastra/agents/customer-agent.ts"
import { Agent } from '@mastra/core/agent'
...
export const customerAgent = new Agent({
  id: 'customer-agent',
  name: 'Customer Agent',
  instructions: 'Help support teams inspect customer records.',
  model: 'openai/gpt-5.4',
  tools: { customerTool },
  toolPayloadProjection: {
    targets: ['display', 'transcript'],
    projectToolPayload: ({ phase, toolName, input, output }) => {
      if (toolName !== 'customerTool') return undefined

      if (phase === 'input-available') {
        return { customerId: input?.customerId }
      }

      if (phase === 'output-available') {
        return { displayName: output?.displayName }
      }

      if (phase === 'error') {
        return { message: 'Customer lookup failed' }
      }
    },
  },
})
...
If a projection policy is configured and projection fails, Mastra does not fall back to the raw payload for display or transcript targets. Input deltas are suppressed when no safe `inputDelta` projector is available.
