...
:::note
Pass `toolCallId` to target the exact suspended call. Some direct approval methods retain an optional `toolCallId` for compatibility with a uniquely labeled snapshot, but omission is rejected when the stored resume label is missing or ambiguous. Runs returned by [`listSuspendedRuns()`](/reference/agents/listSuspendedRuns) always include the exact label to pass.
:::
