...
Removes tool calls and results from messages sent to the LLM, saving tokens on verbose tool interactions. Optionally exclude only specific tools.

The filter runs on the initial input and on each agentic loop step. During step processing, it keeps the latest completed step by default and changes only the next model prompt, so stored history and final tool results remain intact.
