...
## Return a successful tool result directly

Normally, a model calls a tool and Mastra sends the result back to the model in a second provider request so it can write a final response. When a tool already returns the complete caller-facing answer, use `terminalResult` to skip that redundant request.

```typescript title="src/mastra/tools/compile-document.ts"
import { createTool } from '@mastra/core/tools'
import { z } from 'zod'

export const compileDocument = createTool({
  id: 'compile-document',
  description: 'Compile a document and return its final artifact.',
  inputSchema: z.object({ projectId: z.string() }),
  outputSchema: z.object({
    status: z.enum(['compiled', 'failed']),
    artifactId: z.string().optional(),
    diagnostics: z.array(z.string()),
  }),
  terminalResult: {
    // Required: unsuccessful domain results keep the normal repair loop.
    isSuccess: output => output.status === 'compiled',
    outputSchema: z.object({
      artifactId: z.string(),
      diagnostics: z.array(z.string()),
    }),
  ...
  execute: async ({ projectId }) => compileProject(projectId),
})
```

On success, `await result.text` can be empty because the provider's last response was the tool call. Read the validated value from `result.terminalToolResult` or `await result.getFullOutput()`. A terminal batch can contain projections from different tools, so its aggregate `item.value` type is `unknown`. Parse or narrow that value with the projection schema before typed use. Streaming clients receive the same value as a `data-terminal-tool-result` part before `step-finish` and `finish`.

Terminal delivery is intentionally fail closed. Every call in a parallel tool batch must be a successful opted-in local tool with an explicit `project` function and `outputSchema`. Failed or denied calls, approval-gated calls (including approved calls), resumed suspensions, provider-executed tools, background work, signals already queued at terminal arbitration on engines that support active-run signals, mixed ordinary tools, non-pass-through output processors, and display/transcript payload transforms keep the normal continuation path. A signal accepted after terminal arbitration is delivered as the thread's next turn instead of reopening the committed run.

The projected canonical JSON defaults to a 16 KiB UTF-8 limit and can't exceed 64 KiB. Keep large artifacts in durable storage and return identifiers plus a concise summary.
...
### Replace all tool sources

Set `toolsetsMode: 'replace'` to expose only the supplied per-execution toolsets. Replacement excludes assigned tools, memory tools, client tools, agents, workflows, workspace tools, skills, channel tools, browser tools, and processor-loaded tools.

```typescript title="src/mastra/agents/restricted-run.ts"
...
Mastra snapshots the replacement names, executable functions, and nested tool configuration for the execution. After each input-processor pass it materializes a fresh trusted tool map from that snapshot. Input processors can narrow the set for a step, but they can't expand it or replace an allowed implementation. They also can't mutate the configuration used by later model or execution calls. An empty `toolsets` object creates a tool-free execution. Durable reconstruction fails closed if the original replacement implementations are no longer available after a process restart.

Replacement fences are cleared when runs terminate or resume. To keep caller-retained `RequestContext` instances bounded when suspended runs are abandoned, Mastra allows at most 64 simultaneously retained replacement fences per context. Use a distinct context or finish or resume existing runs before starting more.

See the [`Agent.generate()` reference](/reference/agents/generate) for all runtime options including `toolsets`, `toolsetsMode`, `clientTools`, and `prepareStep`.
