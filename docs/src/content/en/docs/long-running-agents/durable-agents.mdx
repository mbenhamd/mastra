...
Use agent-level [`beforeToolCall` and `afterToolCall` hooks](/docs/agents/using-tools#run-logic-around-tool-calls) for validation, auditing, blocking, and other logic around tool execution. Register those hooks on the wrapped Agent so another worker can rebuild the same executable surface.
