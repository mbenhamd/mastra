...
Replacement is enforced on every agent-loop step and across suspend and resume while the same live Harness instance retains the canonical tool snapshot. After each input-processor pass Mastra materializes a fresh trusted tool map, so processors can remove an allowed tool for a step but can't add a tool outside the replacement surface, replace an allowed implementation, or mutate the configuration used by later model or execution calls.

Harness persists the allowed tool names, but executable tool objects are process-local. After `harness.shutdown()`, process loss, or session rehydration in another Harness instance, a pending replacement run fails closed before it is marked resumed—even when the new Harness has same-named static mode tools—because Mastra can't prove implementation identity from names alone.
