...
Replacement is enforced on every agent-loop step and across suspend and resume while the same live Harness instance retains the original tool implementations. Input processors can remove an allowed tool for a step, but they can't add a tool outside the replacement surface, replace an allowed implementation, or mutate its executable fields.
