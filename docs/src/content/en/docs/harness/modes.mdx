...
Harness persists the allowed tool names, but executable tool objects are process-local. After `harness.shutdown()`, process loss, or session rehydration in another Harness instance, a pending replacement run fails closed before it's marked resumed—even when the new Harness has same-named static mode tools—because Mastra can't prove implementation identity from names alone.
