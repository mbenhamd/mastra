...
A [`Session`](/reference/harness/session) holds the live state of a Harness. It records who the session is for and which thread is active, along with the selected mode and model. It also holds permission grants, queued follow-ups, token usage, application state, and the display snapshot. You reach it through `harness.session`.

## What the Session tracks

The Session is the live state of a single user's interaction with the Harness. It identifies the session owner, current thread, selected mode, and model. It also records what the user has approved, what's queued or running, the application state, and the snapshot to render. A session has one active thread at a time but can list, switch between, and clone many threads over its lifetime. Anything that describes the current state lives here. The Harness owns the shared infrastructure every session runs on.

One Harness can serve many users at once because each session has its own identity and state. A separate Session backs each user, which prevents one session's mode, grants, or active thread from leaking into another.
...
The Harness performs actions that change this state, such as switching threads, modes, or models, because those operations coordinate shared infrastructure and emit events. The Session is where you read the result. The sections below note this split where it matters.
...
The Harness owns permission _policy_, which determines the categories or tools that require approval. The Session owns the _grants_ a user makes during the conversation. A grant lets a tool run for the rest of the session without prompting again:
...
`session.run` tracks the in-flight generation. It reports whether a run is active and exposes its run and trace IDs alongside the abort signal. Use it to reflect run status in your UI:
...
Queue a follow-up with `harness.followUp({ content })`. To redirect the agent mid-run instead of waiting, use `harness.steer({ content })`. Both build on [signals](/docs/harness/signals).

## State

`session.state` holds the conversation's application state, including structured values that agents and the UI share, such as model preferences, feature flags, or progress. The Harness defines the shape with a `stateSchema`. The Session owns the live snapshot and validates updates against the schema. It emits `state_changed` on every write:
...
Fine-grained conversation events report streaming tokens and tool progress. They also report pending approvals and task updates, along with increasing token usage. Subscribing to each event type and reassembling the current picture yourself is tedious and error-prone. `session.displayState` does that work for you.

It's a reducer-maintained snapshot. The Session keeps one `HarnessDisplayState` object and folds every harness event into it as it happens, so the snapshot always reflects the latest state. It captures everything a UI needs to render. This includes whether the agent is running, what it's streaming, and which tools or subagents are active. It also includes anything waiting on the user, such as approvals, the current task list, and running totals like token usage and queued follow-ups.
